  ...
  const stepCustomQuestions = useCallback(async () => {
    ...
    ta.cols = 1
        ...
        // ── likert ──────────────────────────────────────────────
          ...
          ratingRow.className = 'flex gap-1 flex-wrap mt-1.5'
          scores.forEach(score => {
            const sb = document.createElement('button')
            sb.className = 'flex items-center justify-center rounded-xl font-bold text-sm transition-all'
            sb.style.cssText = 'width:40px;height:40px;background:rgba(255,255,255,0.05);border:2px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.8);cursor:pointer;font-family:inherit;'
        ...
        // -- numeric input ---------------------------------------
          ...
          inp.oninput = () => {
            goBtn.style.background = inp.value.trim() ? config.theme.primaryColor : 'rgba(255,255,255,0.15)'
            goBtn.style.color      = inp.value.trim() ? '#fff' : 'rgba(255,255,255,0.4)'
          }
          const goBtn = document.createElement('button')
          goBtn.textContent = '->'
