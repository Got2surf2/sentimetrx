...
export default function ResponsesDashboard({ studyId, studyName, botEmoji, logoUrl='', isAdmin=false, userEmail='' }: Props) {
  ...
  }[s || ''] || 'bg-slate-700 text-gray-500')

  return (
    <div className="min-h-screen bg-gray-50">

      <TopNav logoUrl={logoUrl} isAdmin={isAdmin} userEmail={userEmail} currentPage='responses' />
      <SubHeader crumbs={[{label: 'Dashboard', href: '/dashboard'}, {label: studyName}, {label: 'Responses'}]} />
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5">Response dashboard — {total} total</p>
          </div>
          <button
            onClick={handleExport}
            disabled={exporting || total === 0}
            className="px-4 py-2.5 rounded-xl bg-gray-100 hover:bg-slate-700 disabled:opacity-40 text-white text-sm font-medium transition-all"
          >
          ...
          <div className="flex flex-wrap gap-3 items-end">
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="px-3 py-2.5 rounded-xl bg-gray-100 border border-gray-200 text-white text-sm outline-none focus:border-cyan-500 transition-colors cursor-pointer"
              >
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs text-gray-500 font-medium">NPS score</label>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex gap-2 items-center">
                <input
                  ...
                  className="px-3 py-2.5 rounded-xl bg-gray-100 border border-gray-200 text-white text-sm outline-none focus:border-cyan-500 transition-colors"
                />
                <span className="text-gray-400 text-sm">to</span>
                <input
                  type="date"
            ...
            {(sentiment || minNps || maxNps || from || to) && (
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-gray-100 hover:bg-slate-700 text-gray-500 text-sm transition-all"
              >
                Clear
              </button>
            )}
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {responses.map(r => (
                      <tr
                        key={r.id}
                        className="hover:bg-gray-100/40 transition-colors cursor-pointer"
                ...
                <div className="px-5 py-4 border-t border-gray-200 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={offset === 0}
                      className="px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-slate-700 disabled:opacity-30 text-white text-sm transition-all"
                    >
                      Prev
                    </button>
                    <button
      ...
      {selected && (
        ...
        >
          <div
            className="bg-white border border-gray-200 rounded-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto"
            onClick={e => e.stopPropagation()}
          >
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h2 className="font-semibold text-gray-800">Response detail</h2>
              <button onClick={() => setSelected(null)} className="text-gray-400 hover:text-white text-xl transition-colors">x</button>
            </div>
            <div className="px-6 py-5 flex flex-col gap-4">
              ...
              {Object.keys(selected.payload?.psychographics || {}).length > 0 && (
                <div>
                  <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Psychographics</div>
                  <div className="flex flex-col gap-1.5">
                    {Object.entries(selected.payload.psychographics).map(([k, v]) => (
                      <div key={k} className="flex gap-3 text-sm">
                        <span className="text-gray-400 capitalize flex-shrink-0">{k.replace(/_/g,' ')}</span>
              ...
              {Object.keys(selected.payload?.demographics || {}).filter(k => selected.payload.demographics[k]).length > 0 && (
                <div>
                  <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Demographics</div>
...
function AnswerBlock({ label, value }: { label: string; value?: string }) {
  ...
  return (
    <div>
      ...
      <div className="text-slate-200 text-sm leading-relaxed bg-gray-100/50 rounded-xl px-4 py-3">{value}</div>
    </div>
  )
}
