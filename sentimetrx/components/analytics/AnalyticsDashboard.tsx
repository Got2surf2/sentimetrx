...
export default function AnalyticsDashboard({ studyId, studyName, botEmoji, botName, logoUrl='', isAdmin=false, userEmail='' }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <TopNav logoUrl={logoUrl} isAdmin={isAdmin} userEmail={userEmail} currentPage='analytics' />
      <SubHeader crumbs={[{label: 'Dashboard', href: '/dashboard'}, {label: studyName, href: '/studies/' + studyId + '/responses'}, {label: 'Analytics'}]} />
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={setLast30}  className="px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-slate-700 text-slate-300 text-xs transition-colors">Last 30 days</button>
            <button onClick={setAllTime} className="px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-slate-700 text-slate-300 text-xs transition-colors">All time</button>
            <div className="flex items-center gap-2">
              <input type="date" value={from} onChange={e => setFrom(e.target.value)}
                className="px-3 py-1.5 rounded-lg bg-gray-100 border border-gray-200 text-white text-xs outline-none focus:border-cyan-500 transition-colors" />
