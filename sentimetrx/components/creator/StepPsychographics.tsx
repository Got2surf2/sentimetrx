...
export default function StepPsychographics({ draft, updateConfig, onNext, onBack }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Section title="Demographics" description="Choose which demographic questions to show respondents at the end of the survey. All fields are optional for respondents.">
        <div className="flex flex-col gap-2">
          {(function() {
            ...
            var currentFields = draft.config.demoFields
            // Merge bank with current config — preserve enabled state from config
            ...
            // Add any custom fields not in the bank
            if (currentFields) {
            ...
            return (
              <>
                <div className="text-xs text-gray-400 mb-1">{enabledCount} of {merged.length} fields enabled</div>
                {merged.map(function(f) {
                  return (
                    <div key={f.key} className={'flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ' + (f.enabled ? 'border-orange-200 bg-orange-50' : 'border-gray-200 bg-white')}>
                      <button
                        type="button"
                        onClick={function() {
                          var updated = merged.map(function(m) { return m.key === f.key ? { ...m, enabled: !m.enabled } : m })
                          updateConfig({ demoFields: updated })
                        }}
