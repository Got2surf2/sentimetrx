        ...
        {studies.length > 0 && (
          <div className="flex flex-col gap-3">
            {studies.map(study => {
              ...
              const avgNpsDisplay = st.total > 0 ? st.avgNps : '---'
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-5 mt-3 flex-wrap">
                        <div>
                          <div className="font-semibold text-sm text-white">{st.total}</div>
                          <div className="text-slate-500 text-xs">Responses</div>
                        </div>
                        <div>
                          <div className="font-semibold text-sm text-white">{avgNpsDisplay}</div>
                          <div className="text-slate-500 text-xs">Avg NPS</div>
                        </div>
                        <div>
                          <div className="font-semibold text-sm text-green-400">{promoterPct}</div>
                          <div className="text-slate-500 text-xs">Promoters</div>
                        </div>
                        <div>
                          <div className="font-semibold text-sm text-red-400">{detractorPct}</div>
                          <div className="text-slate-500 text-xs">Detractors</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 flex-shrink-0 items-end">
                      ...
                      {study.status === 'active' && (
                        <Link href={'/studies/' + study.id + '/deploy'} className="text-xs px-3 py-1.5 rounded-lg bg-cyan-500/15 hover:bg-cyan-500/25 text-cyan-400 transition-colors">
                          Deploy
                        </Link>
                      )}
                      <button onClick={() => handleDuplicate(study)} disabled={isDuplicating} className="text-xs px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors disabled:opacity-50">
