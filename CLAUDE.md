...
- **American English everywhere (owner standing rule, 2026-09-03).** All LLM-generated text AND all shipped UI copy uses American spellings (color, analyze, center, gray, meters — never colour/centre/grey/metres). Enforced structurally for AI output: `AMERICAN_ENGLISH_RULE` in `lib/ai.ts` is appended to every system prompt centrally (ask-ana's own client includes it too) — do not remove it, and add it to any NEW direct-to-provider client that bypasses `lib/ai`. Output-side backstop: `lib/americanize.ts` converts AI-origin display prose (applied in the Data Story renderer) — extend it to new AI-prose surfaces, but NEVER run it on verbatim user data. Quotes stay as written.
