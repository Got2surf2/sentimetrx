...
// same chrome and same horizontal-bar layout (By Group / By Sub view modes,
// over/under-index ★ markers, sort-by-impact, show-all), differing only in the
// unit being compared (a taxonomy axis's sub-buckets instead of themes/entities)
// and the data source (server `tax_crosstab` rather than a client row scan).
//
// The breakdown-field selector ("Break down by:") sits top-left to match the
// other Compare views; the dimension/axis selector is the one data-type-specific
// control. Clicking any bar drills into the Comments filter for that sub.
...
export default function DimensionCompareTab({ datasetId, catFields, fieldLabel, onDrillDimension }: {
  ...
  const model = useMemo(function() {
    ...
    const allSubs = subs.slice().sort(function(a, b) {
      return smartAxes ? (subTotals[b] - subTotals[a]) : dimSubLabel(a).localeCompare(dimSubLabel(b))
    })
  ...
  function barRow(key: string, label: string, cell: number, groupTotal: number, rowTotal: number, maxRate: number, onClick: () => void) {
    ...
    return (
      <div key={key} onClick={cell ? onClick : undefined}
        ...
        <div style={{ flex: 1, height: 10, background: T.bg, borderRadius: 5, overflow: 'hidden' }}>
          <div style={{ height: '100%', borderRadius: 5, transition: 'width .5s', width: Math.max(maxRate > 0 ? rate / maxRate * 100 : 0, rate > 0 ? 2 : 0) + '%', background: color }} />
        </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, padding: '16px 24px', borderBottom: '1px solid ' + T.border, background: T.bgCard }}>
        <div style={{ marginBottom: 12 }}>
          ...
          <p style={{ fontSize: 12, color: T.textMid, margin: '3px 0 0' }}>How a dimension&apos;s sub-buckets split across a categorical field. Bars show the mention rate within each group; <strong style={{ color: '#16a34a' }}>★</strong> flags groups that over/under-index vs the baseline. Click a bar to read its comments.</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
            ...
            {/* Dimension (axis) — the one data-type-specific selector */}
              ...
              {DIM_AXES.map(function(a) {
                ...
                return (
                  <button key={a} onClick={function() { setAxis(a) }}
                    style={{ padding: '4px 11px', fontSize: 11, fontWeight: sel ? 700 : 500, borderRadius: 16, border: '1px solid ' + (sel ? AXIS_COLOR[a] : T.border), background: sel ? AXIS_COLOR[a] + '14' : T.bg, color: sel ? AXIS_COLOR[a] : T.textMute, cursor: 'pointer', fontFamily: 'inherit' }}>
                    {DIM_AXIS_LABEL[a]}
                  </button>
                )
              })}
              ...
              {(['group', 'sub'] as const).map(function(m) {
                return (
                  <button key={m} onClick={function() { setViewMode(m) }}
                    style={{ padding: '4px 12px', fontSize: 11, fontWeight: 600, borderRadius: 6, background: viewMode === m ? T.bgCard : 'transparent', color: viewMode === m ? T.accent : T.textMute, border: 'none', cursor: 'pointer', boxShadow: viewMode === m ? '0 1px 4px rgba(0,0,0,.08)' : 'none', fontFamily: 'inherit' }}>
                    By {m === 'group' ? 'Group' : DIM_AXIS_LABEL[axis]}
                  </button>
                )
              })}
            </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }} className="fadein">
        {!catFields.length ? (
          ...
          <div style={{ background: T.bgCard, border: '1px solid ' + T.border, borderRadius: 12, padding: '40px 24px', textAlign: 'center', color: T.textFaint }}>
            <p style={{ fontSize: 13, margin: 0 }}>No <strong>{DIM_AXIS_LABEL[axis]}</strong> mentions tagged for this dataset.</p>
          </div>
        ) : viewMode === 'group' ? (
          /* ── By Group: one card per field-value, sub bars within ── */
          <div>
              ...
              return (
                <div key={c} style={{ background: T.bgCard, border: '1px solid ' + T.border, borderRadius: 10, padding: '16px 18px', marginBottom: 12 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: T.text, marginBottom: 14 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 400, color: T.textMute, marginLeft: 6 }}>({pct}% of {DIM_AXIS_LABEL[axis]} mentions · {groupTotal.toLocaleString()})</span>
                  </div>
                  {ordered.length === 0 && (
                    <div style={{ fontSize: 11, color: T.textFaint, fontStyle: 'italic' }}>No {DIM_AXIS_LABEL[axis]} mentions in this group.</div>
                  )}
                  {ordered.map(function(x) {
                    return barRow(x.s, dimSubLabel(x.s), x.cell, groupTotal, model.subTotals[x.s] || 0, maxRate, function() { onDrillDimension(axis, x.s) })
                  })}
                </div>
              )
            })}
          ...
          /* ── By Sub: one card per sub-bucket, group bars within ── */
          <div>
            {visibleSubs.map(function(s) {
              ...
              return (
                <div key={s} style={{ background: T.bgCard, border: '1px solid ' + T.border, borderRadius: 10, padding: '16px 18px', marginBottom: 12, borderLeft: '4px solid ' + color }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: T.text, flex: 1 }}>
                      {dimSubLabel(s)}
                      <span style={{ fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 10, background: color + '18', color: color, border: '1px solid ' + color + '40', marginLeft: 8 }}>{DIM_AXIS_LABEL[axis]}</span>
                      <span style={{ fontSize: 12, fontWeight: 400, color: T.textMute, marginLeft: 6 }}>({pct}% of {DIM_AXIS_LABEL[axis]} mentions · {rowTotal.toLocaleString()})</span>
                    </span>
                    <button onClick={function() { onDrillDimension(axis, s) }}
                      style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 10, background: color + '18', color: color, border: '1px solid ' + color + '50', cursor: 'pointer', flexShrink: 0 }}>
                      View comments →
                    </button>
                  </div>
                  {ordered.map(function(x) {
                    return barRow(x.c, x.c, x.cell, x.groupTotal, rowTotal, maxRate, function() { onDrillDimension(axis, s) })
                  })}
                </div>
              )
            })}
            {hiddenSubs > 0 && (
              <p style={{ fontSize: 11, color: T.textFaint, marginTop: 4 }}>{hiddenSubs} more {DIM_AXIS_LABEL[axis]} bucket{hiddenSubs === 1 ? '' : 's'} hidden — turn on “Show all”.</p>
            )}
          </div>
        )}
      </div>
