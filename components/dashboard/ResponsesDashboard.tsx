...
import type { StudyConfig, SurveyPayload, SurveyQuestion, RatingOption, StudyTheme } from '@/lib/types'
...
export default function ResponsesDashboard({ studyId, studyName, botName='', botEmoji, studyConfig, logoUrl='', orgName='', isAdmin=false, analyzeEnabled=false, userEmail='', fullName='' }: Props) {
    ...
    fetch(`/api/studies/${studyId}/responses?limit=1000&fields=status,payload`)
      ...
      .then(json => {
        ...
        const hasConvo = all.filter((r: Response) => {
          ...
          return Object.values(oe).some((v: unknown) => v && String(v).length > 0)
        }).length
  ...
  const maxScore = ratingScale.length > 0 ? Math.max(...ratingScale.map((s: RatingOption) => s.score)) : (isNps ? 10 : 5)
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {responses.map(r => (
                      <tr key={r.id} className={"hover:bg-orange-50/40 transition-colors " + ((r.payload?.conversationLog?.length as number) > 0 ? 'cursor-pointer ' : '') + (checkedIds.has(r.id) ? "bg-orange-50/60" : "")} onClick={() => { if ((r.payload?.conversationLog?.length as number) > 0) setSelected(r) }}>
                        ...
                        <td className="px-2 py-3.5">
                          {(r.payload?.conversationLog?.length as number) > 0
...
function generateConversationHtml(msgs: ConvMsg[], botName: string, botEmoji: string, theme: Partial<StudyTheme>, response: Response) {
  return `<!DOCTYPE html>
...
function ShareConversationButton({ msgs, botName, botEmoji, theme, response, studyConfig, isLight, subtleText }: {
  msgs: ConvMsg[]; botName: string; botEmoji: string; theme: Partial<StudyTheme>; response: Response; studyConfig?: StudyConfig; isLight: boolean; subtleText: string
}) {
...
function buildConversation(response: Response, config: StudyConfig | undefined, botName: string): ConvMsg[] {
  const msgs: ConvMsg[] = []
  const p: Partial<SurveyPayload> = response.payload || {}
  const oe: Partial<SurveyPayload['openEnded']> = p.openEnded || {}
  ...
  if (p.customAnswers && config?.questions) {
    const questions = (config.questions as SurveyQuestion[]).filter((q) => q.type !== 'hidden' && q.enabled !== false)
  ...
  if (psychoEntries.length > 0) {
    ...
    for (const [key, val] of psychoEntries) {
      const bankQ = bank.find((b) => b.key === key)
  ...
  if (demoEntries.length > 0) {
    ...
    for (const [key, val] of demoEntries) {
      const field = demoFields.find((f) => f.key === key)
...
function ConversationModal({ response, studyId, studyConfig, botName, botEmoji, onClose }: {
  response: Response
  studyId: string
  studyConfig?: StudyConfig & { q2ExportLabel?: string }  // q2ExportLabel is a legacy config field not in StudyConfig
  botName: string
  botEmoji: string
  onClose: () => void
}) {
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-2.5" style={{ background: chatBg }}>
          ...
          {(() => {
            ...
            Object.entries(ca).forEach(([qId, val]) => {
              const q = configQs.find((cq) => cq.id === qId)
            ...
            const psEntries: [string, string][] = Object.entries(ps).map(([k, v]) => {
              const pq = (studyConfig?.psychographicBank || []).find((q) => q.key === k)
