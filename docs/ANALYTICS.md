...
**Action Plan — the "recover your 1–3★ guests" predictor (added 2026-06-22).** The report's **default tab**, powered by `lib/outletPredictor.ts` (`buildPredictor`, pure/unit-tested). **Frame** (chosen after an earlier severity/★-per-mention model was rejected as overstated + tautological): a brand's average is dragged down by its **low-rated (1–3★)** reviews, and the goal is moving those guests up. Per outlet the headline is its **1–3★ rate vs the brand's best-run quarter** (`targetLowRate`) — "X% of this location's reviews are 1–3★ vs ~Y% at your best outlets". The levers are the **driver themes its own unhappy guests cite most**: a theme is a *driver* only if it's **over-represented** in 1–3★ vs 4–5★ reviews brand-wide (`lift = P(theme | 1–3★) / P(theme | 4–5★) ≥ 1.2`, `nBad ≥ 20`) — this controls for base rate, so a loud-but-neutral topic (food, staff — discussed equally by happy and unhappy guests, ≈1×) is **not** surfaced as a problem. Each lever shows the share of the outlet's 1–3★ reviews citing it, the brand-wide over-representation multiple, a verbatim 1–3★ quote, and the **exemplar** to learn from (the best operator whose bad reviews least feature that theme). Per-review **theme membership** (bare keyword match, same matchers as the leaderboard) is captured in the `scanDataset` flat-row loop as `reviewMatrix: {placeId, rating, themes[]}[]`; the 1–3★ vs 4–5★ split supplies the sentiment, so the predictor measures over-representation, never bare presence. Quotes come from `lowExamples` (one per outlet×theme, from a 1–3★ review). Client render: `ActionPlan`/`LeverCard` in `outlet-report/OutletReportTabs.tsx`. **Honesty note**: keyword theme-matching ties a quote to a theme loosely, so the deck attributes quotes to the location only (not a specific theme); the model output is associational — a prioritization signal that benchmarks outlets against their peers, not a causal star change.

**Brand improvement plan — the executive summary (added 2026-06-22).** The brand-level "why you need us" view, fed by the same predictor. Route `app/analyze/[datasetId]/improvement-plan/` — **NOT a new tab**; reached via an **"Improvement plan →" button on the Leaderboard** header (the leaderboard is already the brand-level home). Sections: **the opportunity** (brand 1–3★ rate + its *spread* across outlets — e.g. 1.4% at the best vs 40% at the worst — the headline, plus the projected brand rate if every outlet hit the best-quartile target); **what drives unhappy guests** (`brandLevers` = themes by over-representation lift, with the non-drivers explicitly listed as "loud topics, not differentiators"); a **"where to start" hot-list** (`outletSummaries`, *highest 1–3★ rate first* — not "below median", which is tautological — each linked to its outlet report); and **who already does it well** (`exemplars` = best operators by lowest 1–3★ rate at volume). An **"Export deck"** button (`GET /api/datasets/[datasetId]/improvement-plan-deck`, org-scoped) renders the same plan as a **Datanautix-branded PPTX** via `lib/pptx/improvementPlanDeck.ts` + `renderDeck` (7 slides: opportunity KPIs → driver table → hot-list → verbatim quotes → best operators → method). `buildPredictor` returns `{model:{lowRate,bestLowRate,worstLowRate,targetLowRate,projectedLowRate,…}, drivers, brandLevers, exemplars, outletSummaries, outletLevers}`.
