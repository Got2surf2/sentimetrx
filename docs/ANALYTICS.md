...
> - **PDF template standard** (`lib/htmlToPdf.ts` `brandedPdfChrome`): every PDF gets a per-page running header (brand/project name, top-right) + footer (confidentiality · datanautix wordmark + datanautix.com · "Page X of Y"), and the renderer uses a `.keep` wrapper so a section title never orphans at a page bottom. **Apply to every new PDF template** (see `docs/ENGINEERING.md` + the `feedback_pdf_template_rules` memory).
