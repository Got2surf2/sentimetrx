...
- **Related concepts (2026-09-03, `lib/contextConcepts`; dimensions fixed 2026-09-05).**
  The tab LEADS with the structures the platform already computed for the target's
  comment set — **themes** (whole-comment membership via `commentMatchesTheme`),
  **dimensions**, and **entities** (catalog terms matched over the subset text) — with
  the raw word cloud demoted below. **Dimensions are fetched SERVER-side:** the rows
  route strips `data._tx` from every client row, so the original client-side read was
  dead code that could never populate on any dataset (caught by the parallel session's
  never-assume sweep; its "verified" note had observed absent data masked by the volume
  floor). `relatedConcepts` now reports the subset's `_rowId`s and ContextCloud runs one
  `tax_counts` aggregate per axis scoped to those ids with the active-question
  `fieldKey` and the same `dimensionsEnabled` gate as the Dimensions tab — the engine
  every other dimension surface uses, so chip counts are platform-recreatable.
  Everything else runs over the target's subset only (the never-re-tokenize-the-corpus
  perf rule); counts are whole-comment with a floor of 3 (the G² floor's spirit — 1-2
  ...
  `conceptDatasetId`/`conceptDimFieldKey`/`conceptDimensionsEnabled` through both
  popovers from TextMineModule.
...
- **Where it lives.** The **"Dimensions"** peer section **inside TextMine** (`TextMineModule` renders
  ...
  **re-rolls the view for that field-set** (a new combination is **auto-classified on selection**
  ...
  theme-coupled comments view; it's driven by the tag-filtered endpoint.) The evidence
...
| `components/analyze/textmine/FilteredCommentsPanel.tsx` | THE comments panel (all modes; server rows for entity/dim facets, client rows via `lib/commentRows` for themes-only/show-all) |
