...
**Collection management (2026-06-28):** a collection can now **grow after creation** — the collection card's ⋯ menu has **"+ Add datasets"** (`NewCollectionModal` reused in add-mode: hides the name field, requires ≥1 pick, excludes current members) → `POST /api/collections/[id]/members` (`[id]` = the collection's dataset_id) inserts the new `collection_members`, then **recomputes** the merged schema (`buildMergedCollectionSchema` over the full member set) and `datasets.row_count`. Mirrors the create route's validation (same-org, no nested collections, dedupe). Also fixed: **deleting a collection** silently failed when it was created by another session/teammate — the generic `DELETE /api/datasets/[id]` gated on `created_by === user.id`; collections are shared groupings (deleting one keeps the member datasets), so that gate is now skipped for `source='collection'` (org tenancy still enforced by the `org_id` match), and `AnalyzeClient.handleDelete` now surfaces the server error instead of doing nothing on a non-OK response.
