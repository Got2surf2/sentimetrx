...
**Tabbed report** (`OutletReportTabs.tsx`, client; tab order **Summary · Themes · Dimensions · Leaderboard**, default **Summary**):
...
- **Leaderboard** *(added 2026-06-22)* — the **inverse of the per-outlet tabs**: dataset-wide (ignores `?outlet=`), it fixes each theme/dimension and ranks the outlets. One card per theme and per dimension item (items sorted most-discussed first by `chainN`); each card shows **Top K / Bottom K** outlets by net-positive rate with the chain baseline, `n`, and the outlet's ★. **K is a slider** (`min 1 … max = min(10, ⌊outlets/2⌋)`), defaulting to **3 for ≤15 outlets, else round(20% × outlets) capped at 7** (`leaderboard.defaultK`). When an item's qualifying-outlet count `≤ 2K` the two columns would overlap, so it renders a single best→worst list instead; when `>20` outlets qualify, only the top-10 + bottom-10 are carried to the client (`truncated`) with a "+N in between" note. An outlet ranks for an item only with **≥`MIN_N_OUTLET`** mentions, and an item appears only if it clears `MIN_N_CHAIN`/`MIN_POLAR_SHARE` chain-wide. Computed by `buildLeaderboard` in `lib/outletReport.ts`.
...
**One scan, two views.** `lib/outletReport.ts` was refactored (2026-06-22) so a single `scanDataset()` pass over flat rows + taxonomy assertions builds the per-outlet/chain `{pos,neg,total}` accumulators that **both** the per-outlet report (`buildReport`) and the leaderboard (`buildLeaderboard`) read from. The page calls **`computeOutletBundle(datasetId, outlet)`** → `{ report, leaderboard }` (one scan, not two). `computeOutletReport` is retained as a thin `scanDataset → buildReport` wrapper.
