...
**Action Plan — the predictor (added 2026-06-22).** The report's **default tab**: the highest-impact, peer-relative things this outlet should work on, with what to fix (a guest quote) and **who to learn from** (a peer exemplar). Powered by `lib/outletPredictor.ts` (`buildPredictor`, pure/unit-tested). Model (**v2, de-biased**): a multivariate **ridge regression** of star rating on per-review **theme-negative** dummies — `rating ≈ β0 + Σ_T β_T·1[review negatively mentions T]`, `drag(T) = −β_T`. Ridge (L2, intercept unpenalized, **λ chosen by 5-fold CV**) splits the credit when complaints **co-occur** in one review, which the naive univariate severity (mean-rating gap) double-counts and overstates. Linear-not-logistic on purpose: the outcome is a continuous 1–5★ rating and the whole what-if is ★-denominated, so `β_T` **is** the ★-drag (no log-odds conversion). Per-review theme membership + **whole-review polarity** are captured in the same `scanDataset` flat-row loop as a `reviewMatrix: {placeId, rating, neg[]}[]`, computed **identically to the leaderboard** (keyword match + `lexiconScore`), so the engine and leaderboard agree on what a "negative <theme> mention" is. Whole-review polarity means co-firing themes within one bad review; ridge mitigates but can't fully separate, so output is a **prioritization signal, not causal**. Per outlet, each theme's "potential" = `max(0, outletNegRate − peerMedianNegRate) × drag(T)`, surfaced only for themes with ≥`minDriverN` (40) chain-wide negatives. The **exemplar** ("learn from X") is the peer with the lowest negative-rate among outlets with **≥`exemplarMinReviews` (60)** reviews, tie-broken by higher ★ then volume — the floor stops a tiny outlet at a fluke 0% from winning. Client render: `ActionPlan`/`LeverCard` in `outlet-report/OutletReportTabs.tsx`.

**Brand improvement plan — the executive summary (added 2026-06-22).** The brand-level "why you need us" view, fed by the same predictor's `brandLevers` + `outletSummaries`. Route `app/analyze/[datasetId]/improvement-plan/` — **NOT a new tab**; reached via an **"Improvement plan →" button on the Leaderboard** header (the leaderboard is already the brand-level home). Sections: an executive "opportunity" paragraph (the top lever, its ★-drag, how many outlets sit below the peer median); **levers ranked by modeled impact** (`brandLevers`, each with `chainLift` = vol-weighted brand ★ if every outlet hits the peer median, `outletsAboveMedian`, and `topOutlets` — the biggest offenders to "start with", linked to their outlet report); and a **"where to start" hot-list** (`outletSummaries`, worst rating first — location, rating vs brand, its biggest lever, projected ★). Numbers stay honest (a 4.44★ brand has thin aggregate headroom; the value is the *diagnostic precision* — which outlets, which themes, who to learn from — not an inflated total). A **deck export** (Datanautix-branded) renders the same plan as a hand-off PDF. `buildPredictor` gained `brandLevers.{peerMedian,outletsAboveMedian,topOutlets}` + `outletSummaries[]` for this page.
