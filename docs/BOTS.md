...
A **content-first** companion to the Agent Study. Where the Study leads with health/ops metrics (volume, depth, response rate), the Readout answers the two questions a stakeholder asks after a town hall or campaign: **what did people ASK**, and **what did they RAISE on their own** beyond their questions. NOWOCATS/Sarina is the first instance; the engine is generic (no per-agent hardcoding — a per-agent title override reads `bot.config.readoutTitle`, else defaults to `"{Name} — Conversation Readout"`).
