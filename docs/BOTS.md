...
- **Q&A-pairs export (2026-06-03).** `GET /api/bots/[id]/conversations/export?shape=pairs&format=csv|xlsx` emits one row per question→answer (`#, Session ID, Timestamp, Question, Answer, Language`), pairing each user turn with the agent's next reply (greeting preamble + prompt-leak skipped). `shape=turns` (default) keeps the one-row-per-turn export. Surfaced as a **Q&A pairs** download button on the Transcripts page.
