...
- **Seed migrations** (apply in order, each idempotent on slug):
  ...
  - `sql/one-off/2026-05-25-decision-study-second-pass-fixes.sql` — **current protocol** (six new rules from Sanjay's second test transcript: expanded banned-acknowledgement list adds "Fair," "Fair enough," "Got it," "Okay," "Alright"; Phase 4 verbatim modified to substitute respondent's noun instead of generic "this"; explicit "never ask a question they've already answered" rule with examples; new Phase 3 drill route for counterfactual answers ("I wish I had X"); strengthened non-answer rule #7 with explicit phrases ("how do I know," "I dunno," etc.) and explicit instruction to skip to next phase rather than push drill; Phase 4 pushback-recovery rule keeps the 5-band scale mappable when respondent calls it stilted)
