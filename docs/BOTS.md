...
- **PDF export (2026-06-03).** A **PDF** button on the report renders the clean baked HTML (`renderAgentStudyHtml`) into a hidden iframe and calls `print()` (client-side, no headless-chromium dependency). The baked stylesheet's `@media print` rule force-hides every `<details>` body, so the PDF is a flat summary with **no conversation drill-downs** (safe to hand out) — verified leak-free even with all `<details>` force-opened.
...
- **Transcripts: ignored conversations sink to the bottom (2026-06-03).** On `/bots/[id]/conversations`, auto-flagged (troll/bot/off-topic) and reviewer-excluded sessions are stable-sorted below clean/approved ones and dimmed (opacity 0.55) — kept reachable for the review workflow but out of the way. (`ConversationsClient.tsx`, `isIgnored` predicate.)
...
- **Open-question validation (2026-06-03).** `logged_questions` captures liberally (any uncertain reply), so the raw log is full of false positives — acks, statements, shared context, fragments. The study now (a) pulls each open question's preceding agent line for **context**, (b) **AI-validates** each as a genuine unanswered question and **restates** it, dropping false positives (`openQuestions.autoFiltered`), and (c) the live capture in `lib/logQuestion.ts` is hardened with a `looksLikeQuestionOrRequest` gate (kb_miss/ai_uncertain must read like a question; `deflect` exempt). One-time cleanup of existing rows: `scripts/agent-question-revalidate.ts [--bot <id> | --all] [--apply]` re-validates open rows and marks false positives `status='n_a'` + a `notes` trail (no migration — existing columns) so the Questions admin page self-cleans.
