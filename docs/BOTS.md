...
**Schema (`sql/078_phase3_new_schema.sql`)** — five new tables: `conversations`, `conversation_turns`, `pulseiq_events`, `pulseiq_event_conversations`, `pulseiq_topics`. All have `org_id NOT NULL`, RLS enabled, org-scoped SELECT policies, and no INSERT/UPDATE/DELETE policies (writes are service-role only). `conversations.bot_id` originally FK'd to `bots(id)`; after `sql/079` the table renamed to `agents`, so the FK now references `agents(id)` via Postgres's OID-based rebind. The column name `bot_id` is preserved for code-migration scope reasons — column rename is a separate optional future commit.
...
**Rollback** — flip the env flag off; the dual-write becomes a no-op immediately. The new tables can be dropped via `DROP TABLE IF EXISTS conversation_turns, conversations, pulseiq_topics, pulseiq_event_conversations, pulseiq_events CASCADE` as long as no read path has been cut over.
