...
> **Update (2026-06-01) — recordings accounting.** `usage_logs` gained a nullable `cost_cents` column (migration `092`) for non-token costs. The recordings ASR/transcription vendor charge now logs a flat-cost `usage_logs` row (`resource_type: 'recording'`, `event_type: 'recording_transcribe'`) via `logFlatCost`. The Opus/Sonnet recording passes are tagged `resource_type: 'recording'` (were `'dataset'`, hiding under "TextMine"). `/admin/usage` adds a "Town Hall" type label (the `recording` resource type; relabeled from "Recordings" when the product was promoted 2026-06-04) and folds `cost_cents/100` into token-derived cost.
>
> **Update (2026-06-17) — span re-transcribe.** The quiet-stretch targeted re-transcription (`lib/recordings/transcribeSpan.ts`) logs its ASR vendor charge the same way the full transcribe does — `logFlatCost` with `resource_type:'recording'`, `event_type:'recording_transcribe'`, `model:'asr:<vendor>:span'` — and also accumulates it onto `recordings.cost_cents`. So a span re-transcribe does not escape `/admin/usage` accounting.
>
> **Update (2026-07-14) — Super Agents (AGENT_TIERS Phase 1).** A chat turn from a `capability='super'` agent logs `resource_type:'bot'`, `event_type:'chat_super'` (vs `chat`) with the super model (`claude-opus-4-8` default, or the per-agent `capability_config.model`), so its higher per-turn cost is separable in `/admin/usage` and countable. Pricing is a flat per-agent monthly fee, so super turns are UNLIMITED by default; the abuse backstop `assertSuperTurnAllowed` (`lib/featureFlags`) enforces a monthly ceiling ONLY when an `org_features` row `feature='super_agent_turn'` with a non-null `quota_per_month` exists (counts this month's `chat_super` rows; fails open on error). No new table — reuses `usage_logs` + `org_features`.

> **Update (2026-07-14) — Phase 3 streaming + tool loop.** `lib/ai.ts` gained `callAIStream` — the streaming twin of `callAI` (Anthropic raw-SSE, optional `tools`; BYO OpenAI/Azure orgs degrade to one non-streaming `callAI`). It runs the SAME per-org gate (off/byo/platform via the shared `resolveEffectiveProvider`) and the SAME auto-`logUsage` (usage parsed from the stream's `message_start`/`message_delta` events), so streamed turns don't escape accounting. A Super Agent tool loop logs each NON-final round as `resource_type:'bot'`, `event_type:'chat_tool'`; the final round stays `chat`/`chat_super`, so the quota backstop's per-turn count is unchanged while `/admin/usage` sees the loop's full cost.
>
> **Update (2026-07-14) — Super Agent document ingestion (AGENT_TIERS Phase 2 P2c).** Uploading a PDF/DOCX/text file to an agent's knowledge base extracts text-first (local, no AI cost). A **scanned/image-only PDF** falls back to the vision pipeline (`renderDoc` + `visionReadPages`), which logs `resource_type:'bot'`, `event_type:'knowledge_doc_vision'` (Sonnet vision, ~1.6K image tokens/page via the standard Anthropic input-token path). Text/DOCX extraction and text PDFs cost nothing to ingest; the subsequent chunk classify still logs `knowledge_classify` as before.

> **Update (2026-06-03) — meeting-tool + vision.** `lib/ai.ts` now accepts image content blocks (Claude vision, Anthropic). New `resource_type:'recording'` event types: `recording_synthesize` (Q&A synthesis pass), `recording_slide_vision` (slide-deck vision read), `recording_phase_detect` (presentation→Q&A segmentation), `recording_presentation_summary`. Vision image tokens (~1.6K/page) are counted in the normal Anthropic `input_tokens` from the API response, so cost flows through the standard token-derived path with no special handling (~$0.01/slide on Sonnet).
...
Cost figures everywhere are computed in JavaScript from a single rate table in `lib/usageRates.ts` (`RATES`, `TIER_DEFAULT_MODEL`, `estimateCost`). **Cache accounting (fixed 2026-09-04):** Anthropic's `usage.input_tokens` EXCLUDES cache reads (they arrive in the separate `cache_read_input_tokens` counter), so `estimateCost` ADDS input + cache-read + output components. The original implementation subtracted cache reads from input — every heavily-cached call (Ask Ana especially: ~144K cache-read vs ~28K input per question) computed NEGATIVE, understating all historical cached spend in `/admin/usage`. Stored `usage_logs` token counts were always correct; only the derived dollar figure was wrong, so historical rows display correctly under the fixed formula. Measured real Ana cost after the fix: avg ~$0.11/question, median ~$0.04, max observed $2.21 (a long multi-round subgroup comparison). Both `lib/usageLog.ts` (server-side logger) and `app/admin/estimator/EstimatorClient.tsx` (forward-looking calculator) import from that one file, so the rate table lives in exactly one place.
...
| `event_type` | free TEXT | What the call did. Examples: `chat`, `chat_super` (Super Agent turn — AGENT_TIERS Phase 1), `chat_tool` (non-final round of a Super Agent tool loop — Phase 3), `query_rewrite` (Super Agent multi-query retrieval rewrite — Phase 2, Haiku), `summary`, `deflect`, `intent`, `mine_themes`, `expand_keywords`, `merge_themes`, `search`, `search_rerank`, `pptx`, `html_export`, `signals_pptx`, `insights`, `insights_deck`, `report`, `review`, `auto_reply`, `ai_reply`, `translate`, `clarify`, `study_suggest`, `persona`, `demographics`, `theme_detect`, `knowledge_classify`, `knowledge_doc_vision` (Super Agent scanned-PDF vision extraction — Phase 2 P2c), `simulate`, `expand_terms`, `grade_description`, `suggest_guide`, `suggest_topic`, `suggest_sensitive`, `ana`, `demo`, `ghost_suggest`, `entity_discovery`, `entity_extract`, `score_comments`, `outlet_action_plan` (Outlet Report's cached LLM "3 things to work on next" plan — one `advanced`-tier call per outlet, regenerated only when its review count / theme verdicts change). |
...
| `lib/cohortThemeAggregator.ts` (Phase 5 commit 1 — invoked from `/api/cron/townhall-theme-detection` for `pulseiq_sessions` rows on the new substrate) | townhall | `theme_detect` | standard |
...
**Previously-unlogged helper calls — now logged + gated (2026-07-02).** An earlier audit found ~7 `lib/` helpers calling `callAI()` with no `usage:`, so their spend was invisible AND (because gating keys off `usage.org_id`) ungated. Each now passes a `usage:` context so `callAI` auto-logs them, and `org_id` is threaded from the calling route so they also attribute per-org + honor off-mode/BYOK gating: `projectReport` (`project_report_themes`/`project_report_exec`) + `projectCompare` (`compare_report_themes`/`compare_report_exec`) — org from the collection's owner (`projectReportLoad`); `entityAnalysis` (`entity_canonicalize`/`entity_categorize`) — org from the export route (the admin `entity-analysis-deck` deck logs without org, by design); `nameExtractor` (`name_extract`) — org from `chatCore` (`bot.org_id`); `recordings/setupExtract` (`recording_setup_extract`) — org from the extract-setup route. **Still without org:** `reoExtractor` (`reo_classify`) is an admin-only internal tool (platform-scoped, logged without org intentionally). **`lib/embeddings.ts` — now logged (2026-07-02):** it makes its own OpenAI call (bypasses `callAI`), so it explicitly `logUsage`s the response's `prompt_tokens` as `{resource_type:'system', event_type:'embedding', model:'text-embedding-3-small', provider:'openai'}` with the caller's `org_id` — the last AI-cost blind spot. Rate added to `RATES` ($0.02/1M input, no output/cache).

> **RATES refreshed 2026-07-02 (`lib/usageRates.ts`).** The prior "May 2025" snapshot had Haiku under-priced ($0.80 vs the real $1.00) and Opus at $15/$75 (old Opus-3-era pricing) — current Opus 4.x is **$5/$25**, so recording-analysis cost (hardcoded to Opus) had been logged **~3× too high**. Cost is derived from stored tokens at display time, so the correction fixes historical rows too. Added Opus 4.8 + Fable 5; the `estimateCost` unknown-model fallback now uses the **standard** tier (Sonnet), not the cheapest (Haiku), so a new model isn't silently under-priced. OpenAI/Deepgram audio/embeddings are billed as flat cost, not token rates.
