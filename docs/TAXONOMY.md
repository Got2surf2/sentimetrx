...
- **In-app Dimensions view** (user-facing label **"Dimensions"**; internal code/routes/keys stay `taxonomy` — friendlier than "taxonomy", reads as analytical structure you can pivot/trend on, aligns with the future chart-integration plan; owner considered "Categories" (the competitor's word) but chose "Dimensions"). As of 2026-06-04 it lives as a **sub-tab inside TextMine** (`TextMineModule` renders `<TaxonomyModule>` when `subTab==='dimensions'`, shown for `datasetSource==='google_reviews'` **OR when the org has the `taxonomy` (Dimensions) capability** — a per-org flag in `ModuleFeatures`/team-settings that lets restaurant clients apply Dimensions to ANY analyze dataset, not just Google Reviews; threaded as `taxonomyEnabled` to TextMine/Charts/Stats. Exempt from the theme-model lock); the standalone top-level tab was retired (the `/analyze/[datasetId]/taxonomy` route still resolves but is unlinked). `TaxonomyModule.tsx` is self-contained and fed by
