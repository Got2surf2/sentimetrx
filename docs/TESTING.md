...
_Mocked-boundary example: `tests/unit/chatCoreTurn.test.ts` (added 2026-09-01) drives `handleChatTurn` — the unified chat engine — end to end with `callAI`/embeddings/usage mocked and a permissive Proxy-based fake service client (any query chain resolves canned per-table data; inserts recorded). Covers the silence-probe fast path (all skip reasons + fired probe), the org AI-off gate, byo-key propagation to the main call, the 1200-char input cap, and turn persistence (T0/T1, and askName+greeting first-turn numbering T0–T4). Took `lib/chatCore.ts` from 4% → ~27% statement coverage; extend this harness rather than building a new mock stack._
