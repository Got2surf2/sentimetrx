...
│   ├── wildfireLiveContext.test.ts # wildfire agent live injection (lib/wildfireLiveContext, BOTS.md §6 config.liveContext) — location extraction (ZIP vs City,ST vs "near X"; acreage/go-bag/"near me" false-positive guards), haversine + compass, mocked-fetch block build (nearest-first ordering, ZIP carry-forward, RX labeling, NWS alerts, empty-radius honesty, geocode/WFIGS fail-soft)
│   └── usageLog.test.ts
