...
│   ├── sampledSignalCounts.test.ts # sql/162 keyset pager (lib/sampledSignalCounts) — page accumulation + cursor advance, short-page termination, throw-on-error (callers fall back to exact), scaleSampledCount rounding/full-coverage/zero-scan guards. Live accuracy vs exact counts verified via untracked scripts/_verify_signalstats_e2e.ts against the TEST project's 128K dataset
