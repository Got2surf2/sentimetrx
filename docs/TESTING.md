...
CI runs `audit:gate` (dependency audit — high/critical root advisories vs `audit-allowlist.json`, since 2026-09-14), `typecheck`, `check:sql-tx`, and `test:coverage` on every push and PR. The test step runs with v8 coverage so each CI run publishes the coverage table and enforces a **ratcheting floor** (`coverage.thresholds` in `vitest.config.ts`): a regression below the floor fails CI. The floor is set just below the current baseline — **raised 2026-08-16 from 20/15/20/20 to 30/23/33/30** (statements/branches/functions/lines) over `lib/**` + `app/api/**`, because the old floor had drifted ~10pp *under* the real numbers and would have passed a 30% regression without complaint; a floor that far below actual is decoration, not a gate. Measured at the raise: statements 30.7 · branches 24.07 · functions 33.79 · lines 31.26. It is bumped up as new tests land — per the governance Tests-score progression plan. **Raised 2026-09-14 to 40/30/42/41**, then the same-day coverage week (12 new suites, +9pp) **re-ratcheted to 49/38/53/50** (measured 50.04 · 39.98 · 54.18 · 51.37) — lines + functions now enforce a floor ≥50%, the governance Tests-score 8-9 bar. New harness for loader/route suites: `tests/helpers/fakeSupabase.ts` (a filter-honoring in-memory service client — seed tables, assert on the query shapes). The thresholds are deliberately a no-regression gate, not a coverage *target*; the target is critical-path coverage, not a headline %.

**Coverage surface (2026-09-01).** The `include` is `lib/**` + `app/api/**`, minus the **internal deck generators** (owner decision, 2026-09-01): every standalone deck is internal-consumption only — none is a customer deliverable — so its generation code is not held to the audit's testing bar. The exclusion rule is structural, not name-by-name judgment: every **top-level `app/api/*-deck/**` route** (pitch decks, architecture/engineering decks, the bespoke NEPA/MCO/EA/NOWOCATS decks, etc.) plus the `lib/pptx` builders **only those routes import** (advancedResearch, blueMountainsNepa, eaMembership, eaNpsPitch, mcoListening+mcoLogo, nepaCaraReimagined, projectInsight, reviewIntelligence, salesPitch; diligenceDeck was deleted 2026-09-01 as dead code). Customer-facing PPTX stays **in** the surface: `lib/pptx/{shared,slideRenderer,styles}.ts` infrastructure, the dataset-scoped deck routes (`outlet-plan-deck`, `operational-review-deck`, `improvement-plan-deck` under `app/api/datasets/**`), and the dataset/agent/recording/collection export builders. A new internal deck must live under a top-level `*-deck` route (and keep its builder deck-only) to inherit the exclusion; anything a customer can download is product code and gets tested.

**Throwaway users get their password from `tests/helpers/testPassword.ts` (2026-09-20).** All five env-gated suites minted `randomBytes(16).toString('hex')` — lowercase + digits only. Run locally against the Sentimetrx-Test project, `admin.createUser` rejected that under the project's password policy (upper + lower + digit + symbol) and every suite died in setup with its tests reported as *skipped*. The helper appends a policy-satisfying suffix. CI's `isolation` job was green on the same code the same night, so its secrets evidently point at a project without that policy — not confirmed, worth checking when the secrets are next repointed.
...
│   ├── signalStats.test.ts    # signal-stats cache freshness — recompute when row_count changes under a stable theme-model hash (stale-toolbar bug); persist path goes through the atomic merge RPC (sql/145); sampled path above the 50K cap (sql/162: scale + sampled flag, exact fallback when the RPC is unavailable, exact at/under cap); per-field cache slots (analytics.signal_stats_by_field hit/miss/prune, active-key delegation). **Signals + ratio (2026-09-03)**: a signal is one theme carried by one comment, so the per-theme numerator must carry `p_substantive_only` (pinned by asserting the flag on the one-keyword calls) — the theme cards show those counts, and a total on another base is a second denominator; the any-theme union stays ungated because it divides the all-based records. Ratio = signals / substantive comments (multi-theme datasets push it above 1), `null` never `0.0` with no base, totalRows carries trio tier 1, and a DB predating `theme_counts_substantive` falls back to the ungated count instead of publishing an unmeasured zero. Live: untracked `scripts/_verify_signal_ratio.ts` (32 TEST datasets — monotonic trio ladder + exact ratio) and `scripts/_verify_signal_reconcile.ts` (strip total == sum of the per-theme counts the cards render, 12 datasets incl. 4 brand collections — a mock cannot show the collection member fan-out)
...
│   ├── consensusMining.test.ts # stratified + consensus mining (lib/consensusMining, 2026-09-03) — rating buckets (raw values ≤6 distinct, else thirds), stratifiedDisjointSamples proportional/disjoint/deterministic with small-stratum handling, compositionNote content (incl. the theme-for-EACH-side rule), matcher-equivalent keyword Jaccard, description-vocabulary rescue at the 0.25 threshold, consensusThemes support gating/majority sentiment/keyword-vote merge/stability metrics, and the zero-consensus fallback signal
│   ├── chartsMissingAnalytics.test.tsx # jsdom render regression (prod 2026-07-12): ChartsModule/StatsModule must render their degraded states — not throw — when the analytics blob lacks fieldSummaries/totalRows (script-seeded or compute-failed datasets; unguarded fieldSummaries derefs crashed Charts on the 785K scale test). Since 2026-09-03 also pins the recovery CTA: that shape must render the "Compute analytics" ComputePrompt, not a dead-end "No data loaded."
│   ├── chartCategoryTruncation.test.ts # topCategoryKeys (ChartsModule, 2026-09-03 audit) — every capped categorical chart's subset is top-N BY COUNT (ties alphabetical), never a display-order slice that drops the largest categories
│   ├── chartDisplayHelpers.test.ts # pure display helpers behind the 2026-09-03 chart-options pass — percentShareSeries (per-bucket % shares sum to 100, empty bucket = null not 0%), detJitter (deterministic, ±0.35 bound), compareCells (numeric-aware table sort, blanks last)
│   ├── chartsOptionsUI.test.tsx # jsdom + Plotly mock through the REAL ChartsModule — Time Series breakdown zero-fills calendar buckets (11 daily buckets, real 0s), the % share toggle converts lines to shares (75/25, null empties), and dragging a field lights viable slots ("drop here") while dimming/inerting incompatible ones incl. dragend cleanup
│   ├── chartsDistFallback.test.tsx # jsdom + Plotly mock: the no-histogram Distribution fallback hands Plotly RAW row values for a real box — never [min,avg,median,max] as fake data points (2026-09-03 audit)
...
│   ├── sampledAggregate.test.ts # sql/169 /aggregate sampled twins (lib/sampledAggregate) — per-op keyset-page accumulation (crosstab cells, field-value counts, date buckets, group values, numeric values), count scaling vs UNSCALED means/medians/stddev, even-n median interpolation + sample-stddev n<2 null, real p25_val/p75_val quartiles (percentile_cont parity, feed the >50K compute snapshot), ranked-grid/limit, p_row_ids passthrough, throw-on-error fallback. Live accuracy vs exact within ±2% on 56K/128K + no-57014 on 1M PERF TEST (untracked scripts/_verify_aggregate_sampled.mts)
...
│   ├── usageRates.test.ts     # estimateCost cache accounting — input + cache-read + output ADD (Anthropic reports them separately; the pre-9/04 subtraction drove cached calls negative), never-negative guard, unknown-model→standard-tier fallback
│   ├── emotionFlags.test.ts   # emotion-language flags (lib/emotionFlags) — disappointment/blame/churn-intent detection, negation guard ("won't be disappointed"), "should have" subject attribution (third-party→blame, passive/impersonal→disappointment, first-person dropped = regret dark), suppressChurn, evidence spans, embed grouping under a.emotion; anger + threat-ascribed (2026-09-03): tactic routing ("plays on people's fears"→anger never threat), first-person fear/worry stay dark, "not dangerous"/"never angry" negation guards, independent co-firing
...
│   ├── metricStripUnstamped.test.tsx # jsdom — the strip must not assert a substantive count it never measured. An unstamped `substantive` flag counts as 0; the strip falls back to the answered count, **suppresses the substantive share** (claiming 100% off the fallback would imply a measurement), and uses the all-based theme fit instead of "Diffuse 0%". A further case pins that stamped datasets still prefer the substantive numbers. **Signals ride the same base (2026-09-03)**, so an unstamped dataset reports zero of them for the same reason — the whole signals span is suppressed rather than rendering "0 signals · 0.00 per comment". Second describe block covers the coverage trio: all three tiers when `totalRows` is present (percentages deliberately distinct so a tier swap can't pass by accident), tier 1 dropped when absent (pre-trio caches) or when a stale `datasets.row_count` sits below the measured comment count (which would render "110% wrote"), and the ratio shown to two decimals or omitted when unmeasured
...
│   ├── filteredSummaries.test.ts # recomputeFilteredSummaries (ChartsModule) — filter-aware recount of summary-driven charts: categorical/numeric recount from filtered rows, COUNT surfaces scaled (means/extents unscaled), whole-dataset histogram bin edges reused, virtual __ fields skipped, real interpolated p25/p75 (gauge bands + box under filters)
...
│   ├── simulatorExport.test.ts   # lib/simulatorExport Driver Simulator payloads (spec v1.0) — identity: sigma = sqrt(SSE/(n−k−1)) below outcome SD + reconciles R², intercept+Σ(coef·mean) = outcome mean (acceptance §16.6), se k+1 / corr (k+1)² diag-1 symmetric positive-definite; logit: log-odds coefs (never OR), NO sigma, dummy sd = √(p(1−p)), separation blocked; constant predictor + NaN refusal; filename slugging
...
| Verbatim premise guard | `verbatimSupports` / `pickSupportingSentence` (`lib/verbatimGuard`) — the two quotes that actually shipped off-premise are the regression; no-polarity text rejected; negation respected; strongest-carrying sentence chosen over the first; **verification happens AFTER truncation**, so a signal cut off by `maxLen` does not count | A quote is evidence. A rating selects the REVIEW; the premise is a property of the TEXT DISPLAYED, and a complaint often opens pleasantly — the deck shipped "I got seated fast!" under "Verbatim 1–3★ reviews from the lowest-rated outlets" |
| Outlet PDF payload | `parseOutletPdfPayload` (`lib/outletPdfPayload`) — rejects a non-object body and a `selected.placeId` that isn't the requested outlet; coerces non-finite/wrong-typed numbers to 0; DROPS malformed array elements instead of throwing; narrows an unknown READ verdict; caps oversized arrays/strings; pads what-if rate vectors to the theme count and drops out-of-range `reviews13` indices | The Outlet Deep-Dive PDF is typeset from a payload the BROWSER posts back, and the builder interpolates its numbers into **unquoted CSS** (`style="width:${w}%"`). Server-computed values were safe there; POSTed ones are not. A short `currentRate` vector or a stray theme index would also `NaN` the whole `projectRecovery` projection |
| Outlet lever ordering | `soloRecovery` on every `ThemeStanding`, and `outletLevers` sorted by it descending — asserted per outlet across a whole real dataset by the untracked `scripts/_verify_outlet_pdf_sweep.mts`, which also fails if the systemic callout says "one systemic issue" while `brandLevers.length > 1` | The cards used to be ordered by `peerPercentile` ("how unusual is this?") while the copy claimed "highest-leverage fix" ("what do I do first?"). Those routinely disagree: on Ruth's Chris the bottom-10% theme by percentile recovered FEWER guests than the #2 |
| Action-plan priority order | `pickPriorityThemes` — impact order applied when `leverOrder` is given, READ-severity fallback without it, SOLID/STRENGTH themes never promoted, unranked (outcome) themes sink rather than float, capped at 3 | The plan and the lever cards share a page. Ranking the plan by severity while the cards ranked by impact made "Priority 1 · BIGGEST LEVER" contradict the card under it |
| Export completion is observable | `scripts/_verify_ops_export_states.mts` (untracked) drives the real Operational Review modal: the button flips to a working state, the elapsed clock ADVANCES (a frozen label is worse than none), the download event fires, the file is a real PPTX, the modal closes and the pill resets | `window.location.href` cannot report completion, so a 20-60s build looked like a dead button. Leave Competitors BLANK — that field is what triggers the paid DataForSEO pull |
| Transport-vs-application error boundary | `isTransientTransportError` / `retryTransient` / `retryTransientResult` (`lib/retryTransient`) — retries a dropped socket, and does NOT retry a constraint violation, a `57014` statement timeout or a 4xx; walks the `cause` chain; terminates on a cyclic chain; returns the last `{data,error}` when retries are spent | Retrying an application error is wasted load; not retrying a transport failure cost a production dataset. undici reports these as a bare `TypeError: fetch failed` whose **cause** carries `UND_ERR_SOCKET`, so top-level message matching misses every one |
| Upload ETA | `formatEta` + the estimator (`app/analyze/new/UploadClient`) — coarse wording ("about 2 minutes", 5s buckets under a minute), withheld until 3 batches complete, never negative on the final batch | The estimate is shown to a user waiting on a long upload. Per-second precision on a figure that keeps moving reads as broken, and the first one or two batches carry connection setup so an early projection is wildly wrong |
| Upload chunk vs server stride | `CHUNK_SIZE <= ROWS_PER_BATCH` (and `=`, since anything less just multiplies round trips), plus byte headroom for a 52-column chunk | The route writes `row_index = batch_index * ROWS_PER_BATCH + offset` and the rollback DELETE spans exactly that range — an oversized client chunk overlaps the next batch, so a rollback deletes another batch's rows. A silent data-integrity fault, not a perf regression |
| Outlet Dimensions block | `OutletDimensionsView` (jsdom) — hidden when unavailable, "tracks the network" empty state, strengths+weaknesses merged into one best-first list, a NEGATIVE net rate signed and never called "positive", singular "1 mention", both arms on one shared scale, a quote framed as a mention rather than proof of sentiment, and the denominator named | It renders a signed distance from a network average — the ways it can quietly mislead are an unshared bar scale, an unsigned net rate, and a quote presented as evidence of a polarity it does not carry |
| Outlet-reporting gate | `availableSections` Advanced arm (enable + data both required, neither sufficient) and `locationField` (deepest rung wins, single column counts, order-independent, `ignore` excluded) | Outlet reporting became an entitlement on 2026-08-18. The nav pill and the `/outlet-*` routes must agree — a hidden pill over a reachable URL is not a gate — and the location marker is derived from `hierarchyLevel` rather than stored, so the derivation is the thing that can break |
...
_Route-behavior example: `tests/integration/story-share-routes.test.ts` (added 2026-09-02) pins the Data Story short-link lifecycle (sql/198): the public `/story/[slug]` viewer serving a live story as no-store/noindex text/html, 410 with distinct copy for revoked vs expired, 404 for unknown/malformed slugs and for a deleted storage object (the hard kill), plus generation minting `/story/<slug>` with the row stamped from the org-gated dataset — and falling back to the signed-token link when the table is missing (deploy-order safety). Extended 2026-09-03 with the Share-modal management handlers: GET listing, PATCH revoke stamping `revoked_at` through a fully org-scoped update (id + dataset_id + org_id filters asserted), extend adding 7 days to a live expiry vs reviving an expired link from NOW, and 404s for unknown stories or cross-org callers. Extended 2026-09-04 with the verbatim-focus contract: `body.fields` resolves that selection's own theme set for the story, and a never-mined selection 400s ("mine themes first") rather than narrating a different question's themes._

_Route-behavior example: `tests/integration/aggregate-route-ops.test.ts` (added 2026-09-01) pins the chart-aggregation dispatcher's op semantics beyond its (already gate-tested) auth: grid/series/stats reshaping with `(blank)` buckets, the sampled path gating on `row_count > AGG_SAMPLE_CAP` with throw→exact fallback, the sql/169 `p_row_ids` PGRST202 deploy-order retry, the sql/164 `p_field_key` retry, and the collection fan-out (per-member RPCs, counts summed, synthetic `_collection_label` stamped from member labels)._
...
_Pure-logic example: `tests/unit/commentRows.test.ts` (2026-09-04, CommentsPanel retirement) pins `lib/commentRows.buildClientCommentRows` — the client row source behind the unified Comments panel's themes-only/show-all modes: membership agrees with `commentMatchesTheme` row-for-row (the count-reconciliation contract; the FTS route was rejected precisely because its predicate differs), show-all = ANY theme, empty-text rows excluded from the non-empty denominator, `_rowId` passthrough with index fallback, and the server panel's row shape._

_Pure-logic example: `tests/unit/anaSegment.test.ts` (added 2026-09-04) pins Ask Ana's self-composed subgroup scoping (`lib/anaSegment` behind the query tools' `where`): whereToFilters ≡ applyFilters semantics with blanks excluded from subgroups, condition ANDing, input validation, and — against a PostgREST-faithful fake — compound cat∩cat∩range resolution, both string and numeric storage shapes for a text value, range-only dataset walks, and (2026-09-04) the fuzzy layer: matchStoredValues precedence (exact → case-insensitive → normalized substring either way), partial-label resolution recorded as `mappings` with the label reporting the stored values that ran, and the no-lexical-match error listing the field's actual values for Ana's synonym retry. Collection scope (2026-09-04, dataset-aware fake): a `scope` of member ids makes value discovery, containment scans, and range walks union across members — the collection's own id holds zero rows — with single-dataset behavior unchanged when scope is absent. The fakes emulate the aliased jsonb arrow-select (`"f0":data->"Age"`) the resolver uses for fields-only range checks, and — since sql/200/201 — the `segment_match_ids` RPC itself (text-equality + drf-style numeric guard, keyset paging), so every scenario is proven identical through the SQL fast path; a dedicated test pins PGRST202-fallback parity (JS path returns the same ids)._

_Pure-logic example: `tests/unit/contextConcepts.test.ts` (added 2026-09-03) pins the Context tab's Related-concepts layer (`lib/contextConcepts`): concepts computed over the target's comment subset only, dimensions NEVER computed client-side (rows never carry `_tx` — the 2026-09-05 fix; the original test fed rows WITH `_tx`, an impossible input, which is how the dead code passed) with `subsetRowIds` reported for ContextCloud's server tax_counts fetch and `dimensionChipsFromCounts` pinning floor/cap/axis labels, the 3-comment floor, entity matching scoped to the subset, and the theme modal excluding its own theme._
...
_Pure-logic example: `tests/unit/dataStory.test.ts` (added 2026-09-02) covers the Data Story generator (`lib/dataStory.ts`): theme figures asserted EQUAL to the engine's own `recountThemes` output, quotes gated by the verbatim-premise rule (keyword-anchored sentence must carry the theme's sentiment), the rating/segment field heuristics and their thin-data drop-outs (2026-09-04: `segmentFieldLabel` is the schema LABEL, never a raw key — the collection story's injected `__member__` column must not surface as a heading), narrative parse-with-fallback, the `storyTitle` de-stutter/suffix rules, and renderer escaping + section gating (no `undefined`/`NaN`, hostile names escaped, Datanautix-only branding)._

_Pure-logic example: `tests/unit/analyticsCompute.test.ts` (added 2026-09-01) covers the per-field analytics engine behind dataset sync/compute/trim and collection recompute: every summary type on the pure accumulator path, the discrete (≤20 distinct) vs continuous numeric profiles, a streaming-equivalence invariant (chunked `pushRows` ≡ one in-memory pass — the contract collection recompute depends on), and the SQL path including the sql/161 comma-safe non-empty count for open-ended fields with its sampled fallback, plus the sql/199 real-quartiles read (`p25_val`/`p75_val` from the RPC, median fallback on a pre-199 DB — the pre-199 bug stored p25 = median = p75). Since 2026-09-03 it also pins the >50K sampling doctrine on the compute itself: above `AGG_SAMPLE_CAP` the exact per-field RPCs must NOT run (they time out — the 126K ANES compute stored nonNull=0 for all 25 numeric fields) and the sampled block twins carry scaled counts + real quartiles; and an exact-path RPC error must throw, never persist a fake all-zero summary._

_Real-artifact example: `tests/unit/pptxSlideRenderer.test.ts` (added 2026-09-01) renders a deck holding every `SlideSpec` type through the REAL pptxgenjs (no mocks) and validates the produced .pptx as an archive with jszip: slide count, spec titles/content present in the slide XML, Datanautix (never Sentimetrx) in the file metadata, the unknown-type bullets fallback, and — on every slide of every render — the ENGINEERING §6 corruption tripwire (no 9+-digit OOXML attribute values, `idx=` placeholder indices excluded as legitimate). This is the automated half of the deck-validity bar; pixel-QC via LibreOffice remains the visual half._

_Mocked-boundary example: `tests/unit/analystMemory.test.ts` (added 2026-09-02) pins the "Ana remembers" contracts: `memoryPromptBlock` injects only ACTIVE memories applicable to the open dataset and always carries the framing-only invariant; `loadAnalystMemories` pairs org_id AND user_id explicitly; and the `/api/analyst-memory` route stamps the caller's identity (body-supplied org/user ids ignored), refuses cross-org dataset scoping, 404s on another user's rows for PATCH/DELETE, upserts the `ana_interviewed` flag, and is idempotent on the statement text (a case-insensitive re-proposal returns the existing row — no duplicates). The interview → chip → save → framing loop is browser-verified — see ANALYTICS.md "Ana remembers"._

_Mocked-boundary example: `tests/unit/anaDigest.test.ts` (added 2026-09-04) pins the dataset digest diff (`lib/anaDigest`): snapshots keep only themes with a real cached count (no fake zeros); first visit returns null; row growth/shrink/unchanged lines with the ago label ("earlier today" / "1 day ago" / "N days ago"); theme match-count moves |Δ|-sorted and capped at 4 with an overflow line; new/removed theme flags; and the whole theme comparison skipped when the framework's field changed between visits. The injected-block → briefing-leads-with-deltas path is browser-verified — see ANALYTICS.md "The dataset digest"._

_Mocked-boundary example: `tests/unit/aggregateOps.test.ts` + `tests/unit/anaQueryTools.test.ts` (added 2026-09-01) pin the Ask Ana query engine. aggregateOps covers the shared op dispatcher extracted from the charts aggregate route (now also behind Ana's `query_data` tool): op dispatch + grid reshaping, exact-path RPC args, the sampled-twin gating above `AGG_SAMPLE_CAP`, the filter row-id `p_row_ids` passthrough with its PGRST202 deploy-order retry, rowIds sanitization, and the errAt DB-error contract; plus (2026-09-04) the collection fan-out: per-member RPC calls with summed counts, weighted-mean/pooled-stddev merges pinned against a hand-computed union (median null across members, kept for a lone member, `medianNote` present), `_collection_label` group_stats answering one group per member, bucket merges for date_series, and above-cap collections fanning out exact instead of the single-id sampled twin; plus the tax exact-path threshold (tax_* ops exact at 126K while scalar ops still sample — the block sample's small-strata bias). anaQueryTools also pins (2026-09-04) the two-subgroup `vs` compare: the A/B contract error (vs without where, no queries run), per-group agg calls with each side's resolved ids, hand-computed pp share deltas and avg/median deltas in `comparison`, filter intersection errors naming WHICH side emptied, the overlap note, the Dimensions gate refusing before any resolution, the compare-never-charts rule, and the "Comparing two subgroups…" status label. anaQueryTools additionally pins the Dimensions gate (tax_* refused with a redirect when ctx.hasDimensions is false) and the feature-naming status labels ("Counting Dimensions (emotion) tags…"). anaQueryTools covers `query_data`'s input mapping (ctx rowIds/fieldKey ride along, limits clamped, non-200 → model-readable error+hint, oversized results compacted with truncation notes) and `find_quotes` (exact whole-dataset total labeled as ignoring filters, internal `_` fields stripped from quotes, filtered-view preference via `inFilteredView`), plus `chartConfigForQuery` — the canvas-handoff mapping onto the Charts tab's `{chartType, config}` shape, including the field-type-aware numeric→distribution rule (a type-blind mapping rendered "No data for this field", caught live 9/02) and null for unmappable ops — and `read_comments`: targeted reads report the exact total match count with in-view rows first, untargeted-with-filters reads an evenly-spaced slice of the filtered view's ids, untargeted-without falls back to the representative sample, all with internal `_` fields stripped, and the `field` param scopes quotes to the analyst's active column — empty-column rows excluded and counted (never padded from other fields), labels resolving to data keys, the active-view default, the substantive-text-first no-field fallback, fill-to-limit paging (a sparse column still fills the requested verbatim count), and the representativeness-drift flag (a 100%-Male pull vs a 50/50 dataset is reported). `tests/unit/anaChartSpec.test.ts` pins the inline-chart block parser: valid bar/line specs with row/point caps, malformed JSON degrading to visible text (never a broken bubble), and the unterminated-fence pending state while streaming. `tests/unit/anaPdf.test.ts` pins the findings-PDF composer: pipe tables → real tables with numeric right-alignment, escaping, chart blocks → print bar rows/SVG lines, the logic appendix, and the composed-PDF doctrine (break-inside: avoid present, no forced break-before). `tests/unit/anaViewSpec.test.ts` pins the set_view filter conversion: cat/range specs into the live Filters shape, malformed entries dropped without failing the handoff. The end-to-end reconciliation bar (Ana's answers ≡ the tabs) is verified in-browser against TEST — see ANALYTICS.md "Ask Ana query engine"._

_Mocked-boundary example: `tests/unit/outletReport.test.ts` (added 2026-09-01) drives `computeOutletReport`/`computeOutletLeaderboard` end-to-end over a synthetic 2-outlet chain with only the service client faked — the lexicon, verbatim guard, and delta/verdict math run real. Pins the peer-relative strengths/weaknesses with premise-supporting quotes (a ▼ weakness must quote text that carries the negative premise), the MIN_N chain floor keeping thin themes off every surface, the absolute theme verdicts (STRENGTH/FIX bands), the sub-200-review fleet null, and the "dirty soda/cherry cola" clean:neg noise filter — with the fixture sized so the filter (not the floors) is what's discriminating. 0% → 73% statements on `lib/outletReport.ts`._

_Same pattern: `tests/unit/outletScanCache.test.ts` (added 2026-09-01) pins the persisted-scan cache contract (sql/195, PERF §8): cold call = full row page-through + one cache write; warm call = ZERO `dataset_rows_flat` reads and byte-identical output across `computeOutletReportWithPredictor`/`computeOutletLeaderboard`/`computeHierarchyReport`; each fingerprint component (row_count, last_synced_at, theme model, taxonomy updatedAt, hierarchy designations) forces a re-scan when it moves; an unknown cache version degrades to a re-scan, never an error. The stateful fake service client also honors `.update()`, so the write-through is exercised, not skipped._

_`tests/unit/rowsCache.test.ts` (2026-09-02) pins the IndexedDB rows-cache invalidation contract: the freshness key is deterministic and changes on any of row_count / last_synced_at / schema (aliases are baked into cached rows); the IDB helpers no-op safely outside a browser. The cache hit/miss behavior itself is browser-verified (no /rows fetch on repeat open)._

_`tests/unit/datasetIngest.test.ts` (2026-09-02) drives the server-side upload worker with a stateful fake service + Storage while the REAL lib/csv parser, autoDetectSchema, and stampRowSubstantive run — pinning parity with the retired client-side flow (RFC4180 quotes, column filtering, alias labels, contiguous row_index) plus the safety contracts: pause at the deadline, resume without duplicates (max(row_index) beats a stale checkpoint), and error marking with the file kept for retry. `uploadChunking.test.ts` retired with the client batching it tested; the E2E happy path is browser-verified._

_`tests/unit/filteredCommentsHighlight.test.ts` (2026-09-02) pins the shared Comments highlight pass: theme/entity TERMS match on word boundaries; dimension-EVIDENCE phrases (sql/196) match without them — the classifier's evidence is a fixed-width window that can cut mid-word, and anchoring would silently drop exactly the highlight the dimensions-consistency fix exists to show. Longest-phrase-wins prevents double-marking a keyword inside an evidence span. Extended 2026-09-04 for the per-theme palette port: a keyword with a `kwPalettes` entry marks in its theme's colors (case-insensitive, singular/plural fallback for expanded variants) while unmapped terms keep the amber default._

_`tests/unit/taxonomyKeywordMatcher.test.ts` (2026-09-02) pins the clause-scoped sentiment rule for neutral mentions against the REAL dictionaries + lexicon: "the manager was nice" → pos; contrast conjunctions bound the clause (praise can't leak across a "but"); "not helpful" flips negative; sentiment-free mentions stay neu (the by-rating fallback's contract); polarized phrases keep their tuned polarity._

_Mocked-boundary example: `tests/unit/chatCoreTurn.test.ts` (added 2026-09-01) drives `handleChatTurn` — the unified chat engine — end to end with `callAI`/embeddings/usage mocked and a permissive Proxy-based fake service client (any query chain resolves canned per-table data; inserts recorded). Covers the silence-probe fast path (all skip reasons + fired probe), the org AI-off gate, byo-key propagation to the main call, the 1200-char input cap, turn persistence (T0/T1, and askName+greeting first-turn numbering T0–T4), RAG injection (semantic RPC → high/mid/skip confidence bands, negative-only deflect, keyword fallback with rank→confidence normalization, opponent contrast), the super tool-loop stream path (multi-query rewrite, super knobs, chat_super usage), a town-hall turn (awaited mirror with the townHallId), and the PulseIQ facilitation policy (clarifier on a curt answer stored as source=clarifier with the sql/154 counter bump, stay-on-topic after a substantive answer, move-on rotation with the no-dismissal tone rule, global-checkout chill standby, all-covered close-out, opening-response topic classification, and the theme-detection response-count trigger incl. the manual-mode gate). Took `lib/chatCore.ts` from 4% → ~34% statement / 38% line coverage; extend this harness rather than building a new mock stack._
