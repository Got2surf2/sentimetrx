...
- **Known follow-up #5 (RESOLVED, separate commit)**: `/api/townhall/sessions/[id]/analyze` was reading `townhall_sessions/_themes/_turns` only — phase-3 town halls would 404 on dataset sync. Now substrate-aware: detects whether the id/slug resolves to `town_halls`, branches to a phase-3 path that pulls from `town_hall_conversations` → `conversations` → `conversation_turns` (role='user') and pairs each user turn with its preceding assistant turn within the same conversation (mirrors bot-level analyze pairing). Topics resolved from `town_hall_topics`. Same `DatasetRow` shape so Ana receives identical schema regardless of substrate — datasets can be combined for multi-event rollups (e.g. all Vindman events combined in Analytics).
- **Known follow-up #5b (RESOLVED, separate commit)**: `/api/townhall/responses` POST (post-session psycho/demo upsert) was validating against `townhall_turns` only — phase-3 participants would 404. `sql/083_townhall_participant_responses_phase3.sql` adds nullable `town_hall_id` column + FK to `town_halls(id) ON DELETE CASCADE`, relaxes `session_id NOT NULL`, adds a CHECK constraint (`session_id IS NOT NULL OR town_hall_id IS NOT NULL`), and replaces the old `(session_id, participant_id)` unique key with two partial unique indexes — one per substrate. Route now resolves substrate from the `session_id` payload (legacy `townhall_sessions` vs phase-3 `town_halls`), validates participant via `townhall_turns` (legacy) or `town_hall_conversations → conversations.participant_id` (phase-3), and upserts with the appropriate column populated. NOWOCATS doesn't have post-session questions configured today (`cohort_config.psychographicBank=[]`, `demoFields=[]`) so this is future-proofing — but any new phase-3 town hall that turns them on works correctly now.
- **Follow-up #6 RESOLVED (separate commit)**: bot-level analyze (`/api/bots/[id]/analyze`) row shape now carries `town_hall_slug` + `town_hall_name`. Populated via a single join through `conversations` → `town_hall_conversations` → `town_halls` for each session_id in the sync batch (small fan-out — affected sessions only, not corpus). Empty string for 1:1 widget conversations. `buildBotSchema()` declares both new fields as `categorical`. Means Ana can filter a single bot's dataset by town hall slug/name — alternative to the per-event-dataset workflow from Gap #5 for users who want everything in one place. Both workflows coexist.
- **Follow-up #5c (ad hoc, RESOLVED 2026-05-22)**: `/api/townhall/sessions/[id]/export?format=json` (the JSON the PulseIQ facilitator modal consumes via `setConvModal`) was reading `townhall_turns` only — phase-3 NOWOCATS participants returned empty. Augmented with a phase-3 block that resolves the slug-or-id to `town_halls`, pulls `town_hall_conversations → conversations → conversation_turns`, pairs user+assistant turns per conversation, and emits each pair with `bot_flags` + `user_flags` + `user_sentiment` + per-participant `name` + `persona` from `agent_session_personas`. Backward compatible — legacy turns get null for the new fields. Required because the facilitator modal at `SessionDetailClient.tsx` now renders pills + persona bar.
- **NOWOCATS state — 2026-05-22**: `town_halls.nowocats` row provisioned in prod (`status='draft'`, 9 seed topics, bot_id=Sarina). Sarina updated with 7 focuses + `probe_focus_enabled=true`. Demo 14-turn conversation "Maria" seeded into the substrate (`bs_demo_nowocats_001`) with full content_flags + persona + 1 logged question. Vercel prod env flipped: `TOWNHALL_VIA_AGENT_HANDLER=true` + `DUAL_WRITE_PHASE3=true`. The remaining manual step is clicking **Start** in the facilitator dashboard to flip status `draft → live`.
