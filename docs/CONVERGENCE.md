...
| 3 | **REVISED 2026-07-03 (owner decision: legacy PulseIQ data is discardable; zero active sessions).** Client protocol **modernization**, not adapter: update `TownHallChat.tsx` + the resume endpoint to speak the unified protocol directly (`skipped`/`[done]` semantics, localized labels, `round_hold` already flows). **No `townhall_turns` write-back layer gets built** — with no live sessions and discardable history there is nothing to keep alive; the 37 legacy readers get ported to the new substrate in tranche 2 instead of kept fed. Agent data (`agents`, `bot_conversation_turns` — Sarina/Mason/mvp-plus) is NOT part of this and never migrates; the discard applies ONLY to `townhall_sessions`/`townhall_turns`/`townhall_themes`, executed as the LAST step of tranche 2 after the new path is proven. | M | Was the design-heaviest item; the discard decision collapses it |
