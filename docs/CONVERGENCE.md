...
**Decided:** 2026-05-20
**Revised:** 2026-05-20 — model sharpened from `cohort: true|false` flag on agent to a separate `pulseiq_events` concept that wraps a collection of conversations.
**Owner:** Sanjay
...
pulseiq_event_conversations (
  town_hall_id uuid not null references pulseiq_events(id) on delete cascade,
...
| Cohort theme aggregation | `lib/townhallThemeDetection.ts` + cron | Renamed `lib/cohortThemeAggregator.ts`; reads conversations via the `pulseiq_events` join; writes to `pulseiq_topics` |
...
| Dashboard view | `/admin/townhall/...` pages | Rebuilt to read `pulseiq_events + pulseiq_topics + conversations + conversation_turns` |
...
dual-write + handler ON (the 3 `pulseiq_events` / 426 `pulseiq_topics` prod rows
...
**Changes:**
- `bots` → `agents` (rename; remove fields that move into `pulseiq_events.cohort_config`).
- `bot_conversation_turns` + `townhall_turns` → `conversation_turns` + a new `conversations` parent table.
- `townhall_sessions` → `pulseiq_events` (cleaner name, narrower scope: it's now a config + cohort layer, not a conversation container).
...
**User-facing naming convention** (per CLAUDE.md) does NOT change. Agents stay "agents" in the UI; town halls stay "PulseIQ" in the UI. Internal code uses `agents` + `pulseiq_events`. The product still has two surface labels for users; the *implementation* under both is one substrate.
...
- **2026-05-20 (revision)** — Model sharpened. Dropped the `cohort: true|false` flag on the agent row; introduced `pulseiq_events` as a separate concept that wraps a collection of conversations with cohort config + analysis layer. Same agent can run 1:1 chats and town halls simultaneously. Topic injection is write-back (cohort writes discovered themes into a per-town-hall topic pool) rather than mid-conversation interrupt. Acceptance test restated: June town hall = a `pulseiq_events` row pointing at Sarina, not a flag on Sarina herself.
...
- **2026-05-21** — Phase 4 commit 2 landed. `app/api/townhall/chat/route.ts` gained an opt-in delegation branch gated by `TOWNHALL_VIA_AGENT_HANDLER` (env, default OFF). When the flag is ON AND `session_id` resolves to a `pulseiq_events` row (uuid or slug), the route bypasses the legacy 995-line PulseIQ orchestrator and delegates to `handleChatTurn` with `townHallContext = { townHallId, slug }` populated. PulseIQ-specific features (theme assignment, response counter, language switch, auto-end, standby) are NOT carried into the new path — they get rebuilt on the unified substrate in Phase 5. With zero `pulseiq_events` rows in the system today, the new path is dark on the way in; it activates only after Phase 6 creates the first row. New helper `lib/phase4Flags.ts` matches the `phase3Read.ts` env-gating idiom.
- **2026-05-21** — Phase 5 commit 1 landed. Cohort theme aggregator ported from legacy schema to unified substrate. New `lib/cohortThemeAggregator.ts` mirrors `lib/townhallThemeDetection.ts` but reads `pulseiq_event_conversations` → `conversations` → `conversation_turns` (`role='user'`, `content_en` || `content`), dedups against `pulseiq_topics`, writes new topics with `source='auto_detected'`, `state='pending'`. Per-topic sentiment is dropped (lives per-turn on `conversation_turns.sentiment` now). The cron at `/api/cron/townhall-theme-detection` now scans BOTH legacy `townhall_sessions` (status='active') AND new `pulseiq_events` (status='live'); both paths coexist until the legacy schema is dropped. Legacy lib + cron block stay intact for transition. Specs updated: TOWNHALL.md § Theme Detection split into legacy + new-substrate subsections; USAGE_ACCOUNTING.md emitter table + cron table reflect the new lib.
...
- **2026-05-21** — Phase 5 commit 3 landed. `handleChatTurn` now gains a townHallContext-aware branch after RAG: fetches `pulseiq_topics` (state in active/pending), tallies cohort-wide `response_count` via `pulseiq_event_conversations` → `conversations` → `conversation_turns.topic_id`, builds participant-specific `discussedTopicIds` from this session's prior turns, calls `pickNextTopic`, and pushes a "TOWN HALL TOPIC FOCUS" instruction into systemParts. Chosen `topic_id` carries through to the turn-storage path: `mirrorTurns` extended with optional `topic_id` per turn, plus optional `townHallId` (idempotent upsert into `pulseiq_event_conversations`) and `participantId` (populates `conversations.participant_id`). PulseIQ delegation now passes `participantId` through `TownHallContext`. Sarina regression: **18 PASS / 3 PARTIAL / 1 FAIL / 0 ERROR** — one better than baseline (17/4/1/0); bot path unaffected since all new logic is gated behind `if (ctx.townHallContext)`.
...
- **2026-05-22** — **NOWOCATS is the first town hall on the new substrate** (not Vindman as originally written in § 4 Phase 6). Sarina is the underlying agent. Launch window: early June 2026. To make that launch real, four participant-facing + facilitator-mutating routes had to accept phase-3 town halls (gaps surfaced in the 2026-05-22 PulseIQ feature audit). Shipped:
  - `app/api/townhall/join/[sessionId]` GET + POST — resolves `pulseiq_events` by slug/id (falls back from `townhall_sessions`). POST skips the legacy `townhall_turns` opener insert for phase-3 (chatCore creates the conversation + first turn pair on the participant's first message).
  - `app/api/townhall/live/[sessionId]` GET — phase-3 branch pulls stats from `pulseiq_event_conversations` → `conversations` → `conversation_turns` (role='user'), sentiment from `conversation_turns.sentiment` (precomputed), per-topic counts from `topic_id`. Trending/keyword analytics minimal — full rebuild deferred.
  ...
  - `app/api/townhall/themes/custom` POST — if `session_id` resolves to `pulseiq_events`, inserts into `pulseiq_topics` with `source='manual'` instead of `townhall_themes` `source='custom'`.
  ...
  - `app/api/townhall/sessions/[id]` DELETE — phase-3 branch cascades `conversations` (deletes turns via FK), then drops `pulseiq_events` row.
...
- **Known follow-up #5 (RESOLVED, separate commit)**: `/api/townhall/sessions/[id]/analyze` was reading `townhall_sessions/_themes/_turns` only — phase-3 town halls would 404 on dataset sync. Now substrate-aware: detects whether the id/slug resolves to `pulseiq_events`, branches to a phase-3 path that pulls from `pulseiq_event_conversations` → `conversations` → `conversation_turns` (role='user') and pairs each user turn with its preceding assistant turn within the same conversation (mirrors bot-level analyze pairing). Topics resolved from `pulseiq_topics`. Same `DatasetRow` shape so Ana receives identical schema regardless of substrate — datasets can be combined for multi-event rollups (e.g. all Vindman events combined in Analytics).
- **Known follow-up #5b (RESOLVED, separate commit)**: `/api/townhall/responses` POST (post-session psycho/demo upsert) was validating against `townhall_turns` only — phase-3 participants would 404. `sql/083_townhall_participant_responses_phase3.sql` adds nullable `town_hall_id` column + FK to `pulseiq_events(id) ON DELETE CASCADE`, relaxes `session_id NOT NULL`, adds a CHECK constraint (`session_id IS NOT NULL OR town_hall_id IS NOT NULL`), and replaces the old `(session_id, participant_id)` unique key with two partial unique indexes — one per substrate. Route now resolves substrate from the `session_id` payload (legacy `townhall_sessions` vs phase-3 `pulseiq_events`), validates participant via `townhall_turns` (legacy) or `pulseiq_event_conversations → conversations.participant_id` (phase-3), and upserts with the appropriate column populated. NOWOCATS doesn't have post-session questions configured today (`cohort_config.psychographicBank=[]`, `demoFields=[]`) so this is future-proofing — but any new phase-3 town hall that turns them on works correctly now.
- **Follow-up #6 RESOLVED (separate commit)**: bot-level analyze (`/api/bots/[id]/analyze`) row shape now carries `town_hall_slug` + `town_hall_name`. Populated via a single join through `conversations` → `pulseiq_event_conversations` → `pulseiq_events` for each session_id in the sync batch (small fan-out — affected sessions only, not corpus). Empty string for 1:1 widget conversations. `buildBotSchema()` declares both new fields as `categorical`. Means Ana can filter a single bot's dataset by town hall slug/name — alternative to the per-event-dataset workflow from Gap #5 for users who want everything in one place. Both workflows coexist.
- **Follow-up #5c (ad hoc, RESOLVED 2026-05-22)**: `/api/townhall/sessions/[id]/export?format=json` (the JSON the PulseIQ facilitator modal consumes via `setConvModal`) was reading `townhall_turns` only — phase-3 NOWOCATS participants returned empty. Augmented with a phase-3 block that resolves the slug-or-id to `pulseiq_events`, pulls `pulseiq_event_conversations → conversations → conversation_turns`, pairs user+assistant turns per conversation, and emits each pair with `bot_flags` + `user_flags` + `user_sentiment` + per-participant `name` + `persona` from `agent_session_personas`. Backward compatible — legacy turns get null for the new fields. Required because the facilitator modal at `SessionDetailClient.tsx` now renders pills + persona bar.
- **NOWOCATS state — 2026-05-22**: `pulseiq_events.nowocats` row provisioned in prod (`status='draft'`, 9 seed topics, bot_id=Sarina). Sarina updated with 7 focuses + `probe_focus_enabled=true`. Demo 14-turn conversation "Maria" seeded into the substrate (`bs_demo_nowocats_001`) with full content_flags + persona + 1 logged question. Vercel prod env flipped: `TOWNHALL_VIA_AGENT_HANDLER=true` + `DUAL_WRITE_PHASE3=true`. The remaining manual step is clicking **Start** in the facilitator dashboard to flip status `draft → live`.
