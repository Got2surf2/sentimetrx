...
- **Known follow-up #5**: `/api/townhall/responses` POST still validates participant existence against `townhall_turns` only — a phase-3 participant submitting post-session psycho/demo answers will 404. Fix is a one-line OR-branch into `town_hall_conversations`+`conversations`. Not blocking NOWOCATS launch unless they use post-session psycho/demo questions.
