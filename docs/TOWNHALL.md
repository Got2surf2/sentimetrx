...
>   The computed payload is cached in-process for 8s (2026-09-02, PERF §8): the route is polled every 10s by every live-screen viewer and each poll re-read up to 20K turns, so N viewers now cost one compute per cycle per instance; the screen still refreshes every poll. The `/pulseiq` list similarly no longer runs the full per-hall stats fetch — `listTownHallsAsLegacy` reads distinct participants from the link rows and total user turns from an indexed head COUNT, concurrently across halls.
...
9. **Response counter + auto theme detection** — Stored counters, maintained atomically via `increment_pulseiq_response_counter` (sql/154 — one RPC per stored topic-tagged reply bumps `pulseiq_sessions.response_counter` + the topic's `pulseiq_topics.response_count`; supersedes sql/144, which targeted the dropped `townhall_sessions` and is now removed). The engine READS these stored counters for topic balancing and the every-N trigger — **no per-message COUNT queries** (2026-07-04: the convergence had regressed to recomputing tallies from `conversation_turns` on every turn; restored to the legacy O(1) design after owner flag). If `theme_detection_mode='auto'`, detection fires every `theme_detection_every_n_responses` (facilitator-settable, default 20), throttled to 1/2min per hall
...
> **Runaway-topic controls (2026-07-04, load-test finding):** a k6 run exposed a defect chain — concurrent inline detections raced past the label/keyword dedup (each read existing topics before the others inserted; the AI's "find NEW themes" instruction produces synonym variants that slip the 0.5 keyword-overlap gate) and minted **405 near-duplicate pending topics in one session**, and since per-turn cohort work (topic tallies, balancing, semantic opening match, facilitation) scaled with the pool, chat p95 hit 55 s. Three bounded controls now apply: (1) **pending-pool cap 30** — `detectThemesForTownHall` returns early (before the AI call) when ≥30 pending topics await review; (2) the **inline count-trigger is throttled** to one detection per 2 min per hall via `checkRateLimit('theme-detect:<hallId>')` (the 15-min cron backstops); (3) the **per-turn topic pool in chatCore is capped at 25** — `active` (seed) topics always survive the cap, pending slots go to the most-mentioned. Post-fix re-run: 0 failures, chat p95 20 s. The facilitator console still shows ALL pending topics — only the per-turn engine pool is bounded.

The cron (`app/api/cron/townhall-theme-detection/route.ts`) scans only `pulseiq_sessions` (status='live', `cohort_config.theme_detection_mode` defaulting to 'auto') — the legacy `townhall_sessions` scan was retired in tranche 2 (2026-07-03); it also skips sessions with no new user turns since `last_theme_detection_at` (2026-07-04 — idle live sessions no longer pay for detection every 15 min). `lib/townhallThemeDetection.ts` survives until the frozen legacy orchestrator is deleted (its count-based trigger still calls it).
