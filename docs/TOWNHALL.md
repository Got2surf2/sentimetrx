...
>
> **Export route json branch substrate-aware (2026-05-22, ad hoc Gap #5c)**: `/api/townhall/sessions/[id]/export?format=json` was reading `townhall_turns` only — clicking a participant in the PulseIQ facilitator dashboard's Responses tab would return empty data for any phase-3 town hall (NOWOCATS), so the conversation modal couldn't render. Now augments the legacy turns with a phase-3 block that pulls `pulseiq_session_conversations → conversations → conversation_turns`, pairs user+assistant turns by turn_number per conversation, and emits each pair with `bot_flags` + `user_flags` + `user_sentiment` + per-participant `name` + `persona` from `agent_session_personas`. Legacy turns get `null` for the new fields. Backward compatible — existing callers ignore the new fields. Required because the facilitator modal at `SessionDetailClient.tsx` now renders pills + persona bar (see Facilitator Console block below).
>
...
Both lib paths coexist; the cron (`app/api/cron/townhall-theme-detection/route.ts`) scans BOTH `townhall_sessions` (status='active', `config.engine.theme_detection_mode='auto'`) AND `pulseiq_sessions` (status='live', `cohort_config.theme_detection_mode` defaulting to 'auto'). Legacy lib + cron block are dropped once no town hall is on the old schema.
