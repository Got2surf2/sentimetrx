...
- **Update** — body may carry any of `{ name?, brand_tag?, underlying_agent_id?, analysis_org?, analysts?, objectives?, confidentiality_class? }`. Owner / org-admin / platform-admin. `name` trim ≤200; `analysts` sanitized to `[{name, member_id?}]`; `objectives` to `{summary, questions[]}` or null; `confidentiality_class` validated against the enum. This is the **backfill** path for the §2.8 attribution/objectives on an existing town hall (UI: ProjectDetailsPanel, §5.4).
