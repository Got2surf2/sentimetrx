...
**Visualization.** A split-mic transcript is color-coded by source mic in the report (`TranscriptTab`): Mic 1·L plain dark, Mic 2·R indigo + italic, with a legend — layered with the Q/A bold/italic audit overlay (color is the channel signal so it never clashes with role styling). The mic test mirrors this: a stereo device shows two colored caption lanes (L/R) from a **single Deepgram live socket** opened with `multichannel=true&channels=2` (interleaved 2-ch PCM via `pcm16-stereo-worklet.js`), routed to lanes by `channel_index`. (Earlier a socket-per-channel was used; the second silently failed — one multichannel socket is reliable and half the cost.) The live captions are convenience only; the **authoritative speaker separation is the batch pass** (`multichannel` → channel = speaker, below). A merged-vs-split *live window* was considered and dropped — per-mic font styling already signals "separate streams," and the deliverable comes from batch.
