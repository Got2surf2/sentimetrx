...
- **Tabbed surface (widened 2026-07-30):** the page was a single-scroll summary (meeting notes + exec summary + Q&A). It now mirrors the internal report's tabs — **Presentation · Q&A · Action items · Participation · Transcript** — via `app/th/[token]/PublicTabs.tsx`, a client shell that only picks which server-rendered section is visible (no report data or fetching crosses into the client bundle). Every section is conditional, and PublicTabs drops the tab bar entirely when only one survives.
  - **Two internal tabs are deliberately never exposed: Coverage** (analyst QC — flagged-for-review counts, coverage gaps) **and Live vs Final** (raw-vs-corrected ASR accuracy check). Both describe *how the report was produced* rather than what happened at the meeting. Add public sections in `page.tsx`; never by widening `PublicTabs`.
  - **The transcript is the entity-corrected view**, via the same `normalizeSegments` the internal tab uses — never the raw ASR. Participation reuses the pure `computeParticipation`; the public view renders stats + speaking-share bars only, with no audio playback or editing controls.
  - Still never rendered: review flags, confidence, cost, org/user IDs. (Asker/panelist and speaker names ARE shown — they're part of the shareable meeting record.)
  - **Weight:** the page now inlines the full transcript in the RSC payload (~330 KB on a NOWOCATS meeting vs ~80 KB before). Fine at meeting scale; revisit with lazy per-tab fetching if a multi-hour transcript makes it unwieldy.
