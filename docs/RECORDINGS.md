...
> ⚠️ **Blocker (verified 2026-06-07): the grant endpoint needs a Member+ key.** The current `DEEPGRAM_API_KEY` is valid for streaming (raw-key Token auth opens the live WS) but returns `403 Insufficient permissions` on `/v1/auth/grant` — it lacks token-grant scope. Provision a **Member-or-higher** Deepgram key and set it as **`DEEPGRAM_GRANT_KEY`** (in `.env.local` + Vercel). `grantDeepgramToken` prefers it and falls back to `DEEPGRAM_API_KEY`. Until then live captions stay in the best-effort "unavailable" state and recording still works. The `['bearer', token]` browser subprotocol is still UNVERIFIED end-to-end (couldn't grant a token to test it) — re-run the grant+Bearer WS check once the key is in.
...
> **Deferred hardening:** (1) continuous resumable upload *during* the meeting (crash durability + zero upload-wait at Stop). The MVP uploads the assembled Blob on Stop — fine for a ~1hr meeting (~tens of MB), but a tab crash loses the audio. (2) **Persist** the live transcript + latest rolling summary on the recording (a `live_summary`/`live_transcript` column) so a Stop shows an instant provisional recap while the batch pipeline runs, and so the transcript survives a crash. The MVP keeps both client-side only (computed from caption text passed in the request); no segments table / migration was added. Revisit when reliability or the instant-recap UX is wanted.
