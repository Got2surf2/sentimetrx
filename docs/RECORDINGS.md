...
- **Backstop (lane staggering, display):** `packLanes()` (`lib/recordings/timeline.ts`) still stacks any residual genuine overlap (simultaneous speech) into separate lanes so a block never hides another's number, across the Coverage tab, share page, and PDF (`renderTimelineHtml`). Tests in `tests/unit/recordings/{timeline,transcriptRoles}.test.ts`. *PPTX deck timeline still TODO* — reuses the same model.
