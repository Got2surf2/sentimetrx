...
4. **Polish fourth pass — Claude Sonnet 4.6** (added 2026-06, `buildQaPolishPrompt` / `polishQaPairs` in `analyze.ts`). Runs over **all Q&A pairs**. A faithful editorial cleanup of each verbatim quote into readable, **public-shareable Question/Response text** (the vendor-sample format) → written **additively** onto the payload as `polished_question` / `polished_answer`. The verbatim `question`/`answer` stay the record of truth. Rules: preserve meaning exactly, remove only filler/false-starts/crosstalk, fix grammar, keep every figure/name/date/commitment, stay strictly neutral, never drop substance. Accepts an **optional `glossary`** of canonical entity spellings to normalize against (LLM handles phonetic ASR mis-hearings that edit-distance fuzzy matching misses). The glossary is built by `glossaryFromEntities` = the confirmed entity map's canonicals (§3.5b) ∪ any manually-typed **New Town Hall wizard** "Names & terms" entries (`setup_inputs.glossary: string[]`), threaded into `analyzeRecording`/`reanalyze`/`regenerate`. Non-fatal: on failure the fields are left unset and every surface falls back to the verbatim text. **Also re-run on single-pair regenerate** (`regenerate.ts`) so a regenerated pair keeps a polished version. Cost ≈ $0.25.
...
4. **Transcript** — full transcript with speaker labels + timestamps. Toggle "Group by speaker turn" vs "Segment-level". Search-in-transcript box. Each segment has a "▶ Play from here" button. **The raw ASR transcript is the record of truth — preserved verbatim, never edited by AI.** When a reviewed entity map exists, a **Corrected / Raw toggle** appears (defaults to Corrected): the Corrected view applies the deterministic variant→canonical spelling fix (`normalizeSegments`, client-side from the raw segments + `entity_map`) so names read correctly; Raw shows the untouched ASR. Two transcripts, one record — the raw is never mutated.
