...
**Per-page footer + pagination (added 2026-06-17):** the footer is no longer a single element at the document tail — `reportPdf.ts` renders it on **every page** via `page.pdf({ displayHeaderFooter, footerTemplate })`: the Datanautix wordmark (data=teal, nautix=orange) · the **meeting name** (HTML-escaped, centered, ellipsized) · "Page N of M". `headerTemplate` is emptied to kill Chromium's default date/title header, and page margins moved from CSS `@page` to the `margin` option so the footer band is reserved and margins don't stack (CSS `@page{margin:0}`). Paper is **US Letter (8.5×11)** with **1″ left/right** margins (`14mm` top / `16mm` bottom); the footer template's side padding tracks the 1″ inset so it aligns with the body. Widow-title control: `h1,.topic,.ov-h{break-after:avoid}` keeps a section title with the content that follows (a topic heading never sits alone at a page foot), plus `orphans:3;widows:3` on flowing body copy. Q&A cards / presentation items / the overview box already carry `break-inside:avoid`.
