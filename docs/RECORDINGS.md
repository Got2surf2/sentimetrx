...
- **On-screen report** (`ReportClient`): an opening **"⚠ DRAFT — pending human review"** banner with a **Mark as reviewed** button (PATCH `draft:false` → clears it live) + a large faint diagonal **DRAFT** watermark over the content.
...
The PATCH route (`app/api/recordings/[id]`) accepts `draft` (boolean) for the finalize action. Orthogonal to `signoff` (a draft can later be formally approved).
