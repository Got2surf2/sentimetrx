...
**Entity-spelling corrections apply on read across the whole PDF (fixed 2026-06-17).** Previously the reviewed `entity_map` (§3.5b) was applied only to the transcript appendix (`normalizeSegments`); the Q&A cards / action items / exec summary rendered the polished/verbatim text **un-normalized**, so a name fixed *after* analysis (e.g. ASR "Big Road" → canonical "Vick Road") showed corrected in the appendix but stale in the Q&A — reading as interchanged. `renderTownHallReportHtml` now builds the variant→canonical replacements once (`nz`) and applies them to **all AI text** (Q&A via `qaCard`, action item description/owner, exec summary), not just the transcript. So a post-analysis spelling fix flows everywhere without a re-analyze. **Direction matters:** the correction only yields the right word when the entity-map entry has the **correct spelling as `canonical`** and the mis-hearing as a `variant` — a reversed entry replaces the correct word with the wrong one. (On-screen report Q&A + the PPTX deck still normalize only via re-analyze — a follow-up if needed.)
