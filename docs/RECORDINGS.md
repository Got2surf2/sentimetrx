...
**Rename propagation (2026-06-28).** Renaming a recording (PATCH `/api/recordings/[id]` with `name`) now also renames its **derived dataset** (`recordings.dataset_id` → `datasets.name`), so the dataset name shown in `/analyze` + collection membership doesn't drift from the recording. Collection membership itself is by `dataset_id` (UUID), never by name, so a rename never drops a dataset from a collection. (Older recordings renamed before this need a one-time `datasets.name` backfill from their recording.)

**Community voices only — panel/organizer exclusion (2026-06-28).** Organizer speech is excluded from community-voice analysis, the same way agent analysis only counts the human's turns and ignores the bot's own. `lib/recordings/panel.ts` `isPanelMember(name, panel)` matches a speaker against the meeting's `setup_inputs.panel` roster (normalized; tolerant of middle initials; conservative — first+last token must match, so a partial/first-name match never false-excludes a community member). The report bake filters on the **asker**: any Q&A/commentary pair whose asker is a panel member is dropped from the community Q&A **and** Commentary, while panel **answers** stay attached to genuine community-asked pairs (`panelist_name`). Empty roster → nothing excluded (so a meeting with no panel filled in isn't filtered — fill the roster to enable it). *Follow-up:* stamp `speaker_role` on the mirrored `dataset_rows_flat` so Ana / TextMine theme-mining / the collection merge exclude panel content too (needs a re-mirror of existing recordings).
...
- **Speakers** — the **single** speaker-management surface (the "🎙 Speakers" panel inside `TranscriptReview`). It (a) names each diarized label (seeded by the self-intro "my name is…" detector) → `recordings.speaker_names`, and (b) holds an editable **extra-speakers** roster (moderator/audience/anyone the recording didn't split out) → `recordings.setup_inputs.speakers`. **One "Save speakers"** button persists both via `POST /speaker-names` (`{ names, extra_speakers }`). `applySpeakerNames` rewrites diarized labels before the Opus prompt, so attribution uses confirmed names. (There is no longer a separate Speakers field on the setup page — Setup keeps **Panel members**, which flow in read-only as available names.)
- **Verbatim text** (fix ASR word errors) and **speaker reassignment** (assign a segment to a different speaker) → `recording_transcripts.segments` via `PATCH /api/recordings/[id]/transcript` — either `{ edits: [{ index, text?, speaker? }] }` (per-index text/speaker) or `{ segments: [...] }` (a sanitized, re-sorted **full replacement** for structural edits); recomputes `word_count`, raw ASR preserved in `raw_response`. Owner/admin only. The reassignment dropdown offers diarized labels **+ panel members + the extra-speakers roster** (deduped) — so segments are assignable even when diarization produced **no labels at all**. An **"Auto-fill speakers"** button fills runs of unassigned segments bounded on **both** sides by the **same** speaker (no guessing across a change); fills stage as pending edits to review before Save. `GET /api/recordings/[id]/transcript` returns `roster_panel` + `roster_extra` for the dropdown.
- **Structural segment edits (added 2026-07-24, `lib/recordings/segmentEdits.ts` — pure + unit-tested).** For hand-correcting imperfect diarization, edit mode gives each segment three more controls beyond text/speaker: **‹ › start-time nudges** (±0.5s / ±2s, moving the shared boundary with the previous turn — so a speaker's turn can be made to start a beat earlier/later); **✂ split-at-cursor** (place the caret in the text where a second speaker begins and split into two segments — the split time is interpolated from the caret offset, and the second half is left speaker-less to assign from the dropdown — for the case where two speakers' short back-and-forth got merged into one segment); and **⤒ merge-up** (join a segment into the one before it — one continuous span + joined text, keeping the prior speaker; a *real* one-segment merge, distinct from merely reassigning the speaker, which leaves two rows). Because these shift array indices/timestamps, the whole working array persists via the `{ segments }` mode above. As with any transcript edit post-analysis, the report surfaces a "re-run Q&A to apply" hint.
- **Entity spellings** (`recordings.entity_map`) — still its own gate control; feeds the polish pass only (not extraction).

The same `TranscriptReview` (with `editable`) also runs in the report Transcript tab post-analysis; there a save surfaces a "re-run Q&A to apply" hint since extraction has already happened. Saved segment edits propagate to the parent (`onSegmentsSaved`) so the read view and tab-switch remounts stay in sync without a reload.

**Quiet-stretch targeted re-transcription (added 2026-06-17).** The coverage report's **Long quiet stretches** (≥5min with no extraction, `coverage.ts perMinuteGaps`) are actionable. Each lists a clickable **▶ time range** (plays that span via the report audio modal) and, for owner/admin, a **↻ Re-transcribe span** action with a vendor pick (**Whisper** default — not VAD-gated, so it surfaces faint speech; Deepgram alt). `POST /api/recordings/[id]/transcribe-span { start_sec, end_sec, vendor }` → `retranscribeSpanWorkflow`: slices `stitched.mp3` to the span in a Sandbox (ffmpeg `-ss/-t -c copy`, reuses the now-exported `extract.ts` helpers), runs ASR on just the clip (`lib/recordings/transcribeSpan.ts`), shifts the clip-relative segments to absolute time, and **merges** them into `recording_transcripts.segments` (drops existing segments overlapping the span, re-sorts, recomputes `word_count`). It does **NOT** clear extractions or re-analyze — the Q&A is preserved; re-analyze separately to reflect recovered speech. **Bookkeeping (sql/129):** the span is appended to `recordings.respan_log` (`[{start_sec,end_sec,vendor,at}]`) so the Coverage tab marks that stretch **"✓ re-extracted with <vendor>"** and hides its re-transcribe button (no repeating the paid pass), and `recordings.qa_stale` is set `true` so the report shows a **"Q&A is out of date — Re-run Q&A →"** banner across all tabs. `qa_stale` is cleared by the analyze pass (`runAnalyze`); `respan_log` is reset to `[]` by a full re-transcribe (a fresh transcript supersedes the spans). Flips status `transcribing → complete` (bounces to the status page like a full re-transcribe). The span ASR charge is logged to `usage_logs` (`logFlatCost`, `recording_transcribe`, `model:'asr:<vendor>:span'`) and added to `recordings.cost_cents` (see `docs/USAGE_ACCOUNTING.md`). Like the rest of the Sandbox/ASR pipeline, this leg only exercises in a deployed env (Sandbox + ASR keys), not locally.
...
**Visualization.** A split-mic transcript is color-coded by source mic in the report (`TranscriptTab`): Mic 1·L plain dark, Mic 2·R indigo + italic, with a legend — layered with the Q/A bold/italic audit overlay (color is the channel signal so it never clashes with role styling). The mic test mirrors this: a stereo device shows two colored caption lanes (L/R) from a **single Deepgram live socket** opened with `multichannel=true&channels=2` (interleaved 2-ch PCM via `pcm16-stereo-worklet.js`), routed to lanes by `channel_index`. (Earlier a socket-per-channel was used; the second silently failed — one multichannel socket is reliable and half the cost.) The live captions are convenience only; the **authoritative speaker separation is the batch pass** (`multichannel` → channel = speaker, below). A merged-vs-split *live window* was considered and dropped — per-mic font styling already signals "separate streams," and the deliverable comes from batch. **Naming the channels:** once a 2-channel device is confirmed, the Mic check offers two optional name fields (Left·Mic 1, Right·Mic 2 — e.g. "Facilitator"/"Audience"). While testing, **speaking into a mic lights up its name box** (the clearly-louder channel pulses an emerald ring + "🔊 speaking"), so the user can map each physical mic to L/R and name it correctly. They're sent with `POST …/process { channel_labels: [left, right] }` → `recordings.channel_labels` (sql/124), and the report transcript shows the name in place of `S1`/`Mic 1·L` on each line (with the L/R mic tag kept underneath; legend uses the names too). Unnamed → the generic `Mic 1·L / Mic 2·R` fallback.

**Crosstalk marking:** the report transcript detects where two mics had **overlapping speech** (a time-sweep over per-channel segments; both channels active at once) and draws a **wavy amber underline** on those lines with an explanatory tooltip + legend — so a reader knows garbled text there is simultaneous-speaker crosstalk, not an ASR error.
...
**Keeping the split consistent (fixed 2026-06-09).** Live stereo was intermittent — detected at capture but the two channels often arrived *identical* (dual-mono). Causes + fixes:
- **Browser audio processing forces mono (the main cause).** Chrome's APM (AGC / echo-cancellation / noise-suppression) runs in **mono** — with any of them on, getUserMedia returns a 2-channel track that's actually one channel duplicated. True stereo requires **all three off**. So **all processing now defaults OFF** (`LiveClient` initial `MicSettings`): the AGC-on-breaks-stereo failure is silent and catastrophic, whereas AGC-off on a quiet *mono* mic is mild and recoverable (meters + software gain + the recommendation cover it) — asymmetric risk → default to the stereo-safe side. The Mic check also **detects identical channels live** (L−R difference energy vs signal; ~0 = dual-mono) with a one-click **"Turn off processing for true stereo"**, and once a test confirms a 2-channel device it shows a **persistent prominent warning** on the processing controls (red when any is on, with a "Turn all off for stereo" button; green confirmation when off) so the footgun is never stumbled upon. If processing is already off and channels are still identical, it points at the device (RØDE not in Split). The post-test recommendation suggests AGC *on* only for a quiet **mono** mic.
- **MediaRecorder doesn't reliably preserve a 2-channel track.** When the track is stereo the recorder now records the output of a WebAudio graph with an **explicit 2-channel `MediaStreamAudioDestinationNode`** (`channelCount=2, channelCountMode='explicit'`), not the raw track — guaranteeing both channels survive into the blob (unity-gain mono still records raw).
- **The dual-mono guard in `extract.ts` ALWAYS runs** (step 2) — it correctly keeps genuinely-distinct channels and collapses identical ones, so an AGC-induced dual-mono recording becomes mono (no phantom duplicate S1/S2) rather than fake stereo. `recordings.capture_stereo` (sql/123) is written as a **diagnostic only** (client intent): `capture_stereo=true` + `audio_channels=1` pinpoints a lost split.
...
**Provenance (2026-06-27):** `fetchBrandEntities` requests `authoritativeOnly: true` — Town Hall seeds/corrects ONLY toward canonicals from an OFFICIAL record (a crawled URL / uploaded document) or a human, never a UGC-invented name (entity_catalog source authority, ENGINEERING.md → "Shared correction layer"). So a brand's review-discovered entities don't seed the meeting glossary; its agent's KB-derived (`crawl`/`document`) + manual names do.
...
**Agent-picker scope (fixed 2026-06-17):** the "link an agent" picker (New wizard, Setup editor, report Export tab) lists agents from the **caller's own org** for non-admins, but **admin orgs see agents across ALL orgs** — so a platform admin can link a pilot agent that lives in a client org (e.g. "Sarina" in *Arjun Pilots*) to a Town Hall created in the admin (*Datanautix*) org. Previously every picker was hard-scoped to the recording's org with no admin override, so cross-org agents were invisible *and* the PATCH save rejected them. Both the pickers (`app/recordings/new|[id]/setup|[id]/report`) and the PATCH agent-link validation (`app/api/recordings/[id]`) now key off `isAdminOrg`, mirroring `/api/bots`.
...
**Bot→brand rollup (Phase 3, 2026-06-27).** The linked agent's own curated entities now also flow INTO the brand collection's `entity_catalog`: tagging an agent with a `brand_tag` (`sql/136`) rolls its visible entities up into the brand's collection-scope catalog (`rollupAgentEntitiesToBrand`, `lib/correction/rollup.ts`; details in BOTS.md §9.y.2b). So a Town Hall tagged to a brand draws on Sarina's reviewed spellings via the **collection** scope even when it doesn't name her as the underlying agent — making the brand glossary authoritative rather than re-inferred per meeting.
...
**Action items are editable too (added 2026-06-17).** The Action Items tab (`ActionItemRow`) has the same ✎ Edit affordance: `{ edited_description?, edited_owner?, edited_due_date? }` is a revertible payload overlay on `action_item` rows (display + the PPTX deck prefer the edit; AI values intact; "Revert to AI" clears). Same `PATCH …/extractions/[extractionId]` route, gated to the matching unit type (Q&A text/scope/span edits reject on action items and vice-versa).

**Resolving a "needs review" flag (added 2026-06-17).** The curator's per-pair `flagged_for_review` (§3.5b/curator pass) had no resolve action — it only cleared on a full re-analyze. The same PATCH route now accepts `{ flagged_for_review: false }` ("mark reviewed" — only ever clears, never sets, and also nulls `flag_reason`), surfaced as a **✓ Mark reviewed** button on flagged Q&A cards. The **Coverage tab badge** ("N to review") counts `flagged_for_review` from the **live** extractions state (not the stored `coverage_report.flagged_count` snapshot), so it drops immediately on Mark-reviewed; the Coverage body's "Flagged for review · N" is likewise recomputed live via `computeCoverage`. This is distinct from the report-level **sign-off** (§5.4 below), which finalizes the whole draft.
...
### 4.1d `GET`/`POST /api/recordings/extract-setup` — propose setup fields from a document

Setup-before-media doc ingestion. The wizard (§5.2) uploads the deck that *will* be presented and gets back proposed setup fields to confirm. **Two-step, direct-to-storage** (decks routinely exceed Vercel's **4.5 MB function body limit** — a single multipart POST returns a plain-text `Request Entity Too Large` that the browser then chokes on parsing as JSON):
- `GET` → reserves a temp object path under `<org>/_setup-extract/<uuid>/deck.pdf` (fully server-controlled) and returns `{ upload:{endpoint,bucket}, storage_path }`. The browser TUS-uploads the PDF **straight to Supabase Storage** (same path Town Hall media uses — no function body, no 4.5 MB ceiling), authorized by the org-prefix storage RLS policy (sql/091).
- `POST` → `{ storage_path, source }` (JSON) of the just-uploaded PDF → `{ proposal: { objectives:{summary,questions[]}, agenda[], panel[], glossary[] }, source: <filename> }`. The path is re-validated to live inside the caller's `<org>/_setup-extract/` sandbox before use.

**Stateless:** the uploaded PDF is rendered to page PNGs + read with Claude vision in one pass (`lib/recordings/setupExtract.ts`, reusing the slide-vision rails), then **all temp objects are deleted**. (The `callAI` here now carries a `usage:` context — `resource_type:'recording'`, `event_type:'recording_setup_extract'` — so the setup-extraction spend is logged; 2026-07-02, see `docs/USAGE_ACCOUNTING.md`.) Nothing is persisted to a recording — the analyst confirms the proposal in the wizard (it pre-fills only empty fields, recording `setup_provenance`), and the deck itself is attached later with the recording as a `slides` file (or now via §4.1e in the Setup editor). v1 is PDF-only (PPTX/DOCX → convert-to-PDF first, future). The prompt is grounded — it proposes only what's on the slides, empty when unsure.

### 4.1e `POST /api/recordings/[id]/documents` + `DELETE …/documents/[fileId]` — persist project documents (documents-anytime, 2026-06-07)

Edit-anytime document persistence — attach the deck + briefs to a recording at **any lifecycle stage** (the unified Setup editor §5.7 uses these; create mode can't, since no recording exists yet). **Two upload paths** (a slide deck can exceed Vercel's 4.5 MB function body limit, same as §4.1d): `GET ?role&name` reserves the storage path + TUS endpoint so the browser uploads the deck **straight to Storage**, then `POST` (application/json, `{storage_path, original_filename, role, mime_type, size_bytes}`) registers the landed object. A legacy **direct multipart upload** (`file` + `role`, ≤50 MB through the function) remains for small docs / service-role callers.
...
**Draft reports (`draft`, sql/125).** A generated report is, by default, an **unreviewed AI draft**. The generate gate's **"Mark as draft (pending human review)"** checkbox (default ON) sends `draft:true`; `runAnalyze` writes `recordings.draft` (both the full and skip-Q&A paths). The column DEFAULTS to **false** so existing completed reports aren't retroactively watermarked. While `draft=true`:
- **On-screen report** (`ReportClient`): an opening **"⚠ DRAFT — pending human review"** banner with an inline **sign-off** (optional note + "Review complete — sign off & finalize") + a large faint diagonal **DRAFT** watermark over the content.
- **Exported PDF** (`reportHtml.ts`): a `position:fixed` DRAFT watermark (repeats every page) + the opening statement in the header.
- **Exported deck** (`recordingDeck.ts`): a faint diagonal DRAFT watermark + a "DRAFT — PENDING HUMAN REVIEW" pill on the title slide.
- **Public share** (`/th/[token]`): the same banner + watermark, so external viewers see it's a draft.
**Finalize = sign-off (the enforced process).** Clearing `draft` is NOT an anonymous toggle — it happens ONLY through the sign-off route, which captures **who** reviewed: `POST /api/recordings/[id]/signoff` (auth'd via `getUserContext`) stamps `recordings.signoff = { approved_by (the user's name/email), approved_by_member_id, approved_at, note }` **and** sets `draft=false`; **DELETE** revokes it (`signoff=null`, `draft=true` — back to draft). The generic PATCH route intentionally does NOT accept `draft`, so there's no backdoor finalize. The report's draft banner runs the sign-off inline; the Export tab's **Review sign-off** panel does the same + Revoke. So a finalized report always carries a named reviewer + timestamp, and the "✓ Approved by X on DATE" line shows in the header, the PDF, and the deck.
...
Server-rendered PDF of the meeting report (the forwarded deliverable). `lib/recordings/reportHtml.ts` (`renderTownHallReportHtml`, a pure function) bakes a self-contained inline-styled HTML doc — meeting meta + **Meeting Notes** (the `proceedings_summary` presentation half, when present) + exec summary + polished Q&A by topic (`polished_*`, fallback verbatim) + an **Action Items** section (`action_item` rows, edited overlay — added 2026-06-17; the deck shows these too, but the public `/th` link **never** does, so the PDF deliberately carries more than the public link, same as the optional transcript appendix) — then headless Chrome's `page.pdf()` renders it (`page.setContent`, **not** `goto`, so Tailwind isn't needed). **Meeting Notes section (added 2026-06-16):** `renderTownHallReportHtml` takes `proceedings` and renders a `proceedingsSection` (overview paragraph + per-`items[]` card: title, slide refs, presenter, what-was-presented, key-figure chips) above the Overview — the **same content the in-app Presentation tab (§7.1.0) and the deck's Meeting Overview slide show**, so the forwarded PDF is no longer Q&A-only. When notes are present the eyebrow reads "Meeting Summary" and the exec-summary heading becomes "Q&A Overview"; with no presentation phase `proceedings` is null and it falls back to the prior Q&A-only layout unchanged. `reportPdf.ts` threads `rec.proceedings_summary` through, and both consuming routes (`report/pdf`, `report/send`) now select `proceedings_summary`. Chrome resolution mirrors the Agent Study PDF route: `@sparticuz/chromium` on the Linux serverless runtime, an installed Chrome locally (key off `process.platform`, **not** `process.env.VERCEL`).

**Speaker reassignment from the audio player (2026-07-30).** Reassignment already existed in the Transcript tab (edit mode → per-segment `<select>`, plus split/merge/nudge and the fill-same-speaker-gaps helper), but the player is where a mis-attribution is actually *heard*. `AudioModal` now offers two controls, both gated on `canEdit` (so never in `publicMode`): a **"move block →"** select on each speaker-change header that reassigns the whole run in one call — the common failure — and a hover-revealed per-line select for a single stray segment. Both target an **existing diarization label** rather than free text, so the speaker stays joined up with participation, the roster and every other view. Persists through the existing `PATCH /transcript` `{ edits: [{ index, speaker }] }` path; indices are positions in the RAW segments array, which line up because `correctedSegments` is a 1:1 map of it.

**Why correction tooling and not model work.** Diarization is the ASR vendor's voice clustering — there is no knob on our side. NOWOCATS Community Meeting 2 is `audio_channels=1` with **0 of 760 segments carrying a channel**, so identity is 100% clustering, and it found **11 speakers where Participation shows ~9** — over-splitting. A public meeting on a single mic is the worst case: audience at varying distances, crosstalk, and two-second interjections ("Right.") that are exactly what clustering gets wrong. The structural fix is **capture, not code**: `computeParticipation` already prefers channels over clustering when `audio_channels >= 2`, so a panel mic on one channel and an audience mic on the other makes identity deterministic and removes this failure class for future meetings.

**Corrections now apply to every transcript-text surface in the app too (2026-07-30).** The same on-read normalization the PDF/deck/report already used was missing from the in-app surfaces that display raw segments: the **audio player's follow-along list**, **action-item source hovers**, **coverage drill-downs**, and the **Participation floor-ribbon tooltip** (which shows up to 400 chars of the turn). A correction the owner had already entered — canonical `Plymouth Sorrento`, variant `Plymouth-Cemento` — was showing corrected in the report and PDF but raw in the player, which is where they actually listen and spot remaining errors. `ReportClient` now derives `correctedSegments`/`correctedTranscript` once and passes those to every text-displaying consumer.

Two deliberate exceptions, both keeping RAW: the **Transcript tab** (`TranscriptReview` owns the corrected/raw toggle and needs the raw side to offer it — normalizing its input would make "Raw" show corrected text) and **Live vs Final** (it diffs live ASR against final ASR; entity corrections would read as ASR improvements and misrepresent the comparison).

**Entity-spelling corrections apply on read across the whole PDF (fixed 2026-06-17).** Previously the reviewed `entity_map` (§3.5b) was applied only to the transcript appendix (`normalizeSegments`); the Q&A cards / action items / exec summary rendered the polished/verbatim text **un-normalized**, so a name fixed *after* analysis (e.g. ASR "Big Road" → canonical "Vick Road") showed corrected in the appendix but stale in the Q&A — reading as interchanged. `renderTownHallReportHtml` now builds the variant→canonical replacements once (`nz`) and applies them to **all AI text** (Q&A via `qaCard`, action item description/owner, exec summary), not just the transcript. So a post-analysis spelling fix flows everywhere without a re-analyze. **Direction matters:** the correction only yields the right word when the entity-map entry has the **correct spelling as `canonical`** and the mis-hearing as a `variant` — a reversed entry replaces the correct word with the wrong one. **The same on-read normalization now also runs on the PPTX deck (`recordingDeck.ts` — Q&A, representative exchanges, exec summary, headline, topic summaries, decisions, action items) and the on-screen report (Q&A cards + action items), so all three deliverables agree without a re-analyze.**

**Per-page header + footer + pagination (added 2026-06-17):** running header/footer on **every page** via `page.pdf({ displayHeaderFooter, headerTemplate, footerTemplate })`. **Header** (top-right): `powered by` + the Datanautix wordmark — **data = Sarina teal `#0F7173`, nautix = Ana orange `#E85A1A`** (the `lib/pptx/shared.ts` palette primaries; one word, no separator) — with a hairline rule beneath separating the header band from the body. The footer carries a matching hairline rule above it. First-page section labels (Panel/Objectives) use the larger `.sec-h` style with top margin so blocks aren't cramped. **Footer**: generated-at timestamp (left, the viewer's **local time** — the browser's IANA `tz` is posted to the route and validated; `h:mm A` + zone abbrev, falls back to the server zone for the email path) · the **meeting name** (HTML-escaped, centered, ellipsized) · "Page N of M" (right). Page margins are owned **entirely** by the `page.pdf` `margin` option, which reserves the header/footer bands — **do NOT also set `@page{margin}`** in the CSS (a competing `margin:0` made content use the full page height and bleed under the header/footer on dense pages; fixed 2026-06-17). Paper is **US Letter (8.5×11)**, **1″ left/right**, `18mm` top/bottom; the header/footer side padding tracks the 1″ inset. **First-page context:** the header block prints date · location, the **Panel** roster (name + role chips, from `setup_inputs.panel`), objectives, and the executive-summary Overview before any Q&A, so a reader has context for the questions. Widow-title control: `h1,.topic,.ov-h{break-after:avoid}` keeps a section title with the content that follows, plus `orphans:3;widows:3` on flowing body copy; Q&A cards / presentation items / overview box carry `break-inside:avoid`.
...
4a. **Participation** (added 2026-07-06) — conversation-dynamics panel computed **purely and deterministically** (no AI) from the diarized `recording_transcripts.segments` by `lib/recordings/participation.ts` (`computeParticipation`), client-side from props already in `ReportData` (no new fetch, route, or migration). Sits between Action items and Transcript in the tab bar. Contents: a KPI strip (speakers detected, top-speaker dominance share, floor changes + crosstalk moments, quiet time of total), a **panel vs community share bar** (shown only when a `setup_inputs.panel` roster name matched via the conservative `isPanelMember` — otherwise the split is null, never a misleading 0%), a **floor ribbon** (one lane per speaker, bars = merged turns on the meeting timeline; each bar is a button — click to play that span through the shared audio modal, hover to read its transcript text via the turn's carried `text`), and a per-speaker table (talk time, share bar, turn count, avg turn, longest turn — clickable, plays that span through the shared audio modal — and words-per-minute pace). Semantics: speaker identity keys on `segment.channel`+`channel_labels` for true-stereo captures (`audio_channels ≥ 2`), else `segment.speaker`+`speaker_names` (same precedence as the Transcript tab); raw labels are grouped by their **normalized resolved display name**, so two labels that show the same person **merge into one speaker row** — a voice cluster split in two, two channels labeled alike, or a segment whose raw label was set directly to the name (`"J.C. Viert" → "J.C. Viert"`, which an earlier `name ≠ raw` guard missed; corrected 2026-07-24); consecutive same-speaker segments < 2s apart merge into one *turn*; inter-segment gaps ≥ 3s count as *quiet time*; different-speaker turn overlap ≥ 0.2s counts as *crosstalk*; share is of total speaking time, not wall clock. Degrades honestly: no segments → "appears after transcription"; a Whisper-only mono transcript (no diarization) → an explanatory empty state, never a fabricated split. Unit-tested in `tests/unit/recordingParticipation.test.ts`. The **shared audio modal** anchors its synced transcript to the playing segment **on open** (not only once playback reaches the next segment): the scroll effect keys on the active-segment index *and* the signed-URL load, since the transcript list only mounts after the URL resolves (fixed 2026-07-24).
5. **Reports** — PDF download, XLSX download, "Enable public link" toggle, "Send to principals" email-list field. **Config versions + sign-off panel (§2.8, 2026-06-07):** below the export controls, a panel to **Save version** (manual snapshot with an optional note) + a list of all versions (number, manual/analysis badge, the live one marked, who/when/note), and a **review sign-off** control ("Mark reviewed & approved" / "Revoke") that drives the approval stamp across the report/deck/PDF. **Report header attribution (§2.8):** the report header shows **Prepared by {analysts} · {analysis_org} · Config v{n} · analyzed {date}**, a **confidentiality** pill, the **✓ Approved by** stamp when signed off, and an **Objectives** box (summary + questions) — all from the new `recordings` columns. The deck (§4.13) and PDF (§4.5) carry the same attribution + confidentiality + sign-off. PDF export options: (a) Q&A summary only, (b) Full transcript only, (c) **Both — Q&A summary + transcript appendix** (default for "send to principals at end of meeting"). **Brand & linked agent editor (added 2026-06-05, owner-only):** a brand-tag input + agent picker that `PATCH`es `recordings.brand_tag` / `underlying_agent_id` — lets an *already-uploaded* recording be tagged to a brand/agent after the fact (previously creation-only), so a subsequent re-extract seeds spelling correction from that brand's curated `entity_catalog` (§3.5c). The PATCH route accepts `{name?, brand_tag?, underlying_agent_id?}` (owner/org-admin/platform-admin; agent verified to exist — non-admins restricted to the recording's org, **admin orgs may link cross-org**, see §3.5c) alongside the existing rename + org-transfer branches. **Project setup link (edit-anytime, 2026-06-07):** this tab carries a **"Project setup → "** card linking to the unified Setup editor (§5.7) — the full meeting/panel/agenda/names/objectives/attribution form. This **replaced** the old in-report `ProjectDetailsPanel` (which edited only a subset) so there is **one** setup editor, not two. The report header also gets an **"Edit setup ⚙"** link to the same page. Re-running the analysis afterward feeds updated objectives into the synthesis (§3.5).
...
**Phase 2 shipped state (2026-05-31):** route at `app/recordings/[id]/report/{page,ReportClient}.tsx`. Server resolves dataset → recording (reverse lookup via `recordings.dataset_id`), redirects non-recording datasets back to `/analyze/[id]`. Tabs 1–4 are live; tab 5 (Reports) is a stub listing what's coming. Flagged cards render with a yellow background + their `flag_reason`. Topic ordering follows the agenda from `setup_inputs.agenda`; non-agenda topics ("Other", anything the model invented despite the prompt) trail at the end of the tab.
...
- **Reports tab:** **PowerPoint export wired (2026-06).** An "Export to PowerPoint" button POSTs to `/api/recordings/[id]/export/pptx` (§ 4.13), downloads the returned `.pptx` blob, and shows the `LottieLoader` while generating; disabled with a hint until `status='complete'`. PDF (§ 4.5) + XLSX + public share (§ 4.7) remain listed as coming-soon below the button.
...
- **Full parity with the internal report (2026-07-30):** `/th/[token]` renders the SAME `ReportClient` as the internal report, in `publicMode`, rather than a parallel simplified page. A hand-built public twin drifts from the internal report the moment either changes, so the shared link now IS the report: real tabs, expandable Q&A cards, the clickable/hover floor ribbon, transcript search.
  - **Public tabs: Presentation · Q&A · Action items · Participation · Transcript.** Dropped in `publicMode`: **Reports** (exports, share panel, sign-off), **Coverage** and **Live vs Final**. Coverage flags every agenda item with no Q&A pairs as a gap — but presentation sections ("Welcome and Opening Remarks", "Study Methodology") never draw audience questions, so publicly it reads as *"this report missed 4 of 7 agenda items"* when nothing is wrong. Live vs Final exposes raw-vs-corrected ASR.
  - **Dropping a tab from the nav is not enough** — `tab` state drives the body, and the internal default is `coverage`, so `publicMode` also overrides the default tab and rejects internal keys arriving via `?tab=`.
  - **`canEdit` is computed ONCE** in `ReportClient` as `!publicMode && (isOwner || isAdmin)` and passed down; never re-derived downstream. Any new mutation affordance must gate on the `canEdit` it receives. Hardening this surfaced three controls that had **no edit gate at all** and rendered for anyone: QATab's "Re-extract all", its per-topic `⋯` re-extract menu, and QACard's scope / hand-edit / regenerate / mark-reviewed cluster. Harmless while the report was login-only; a hole the moment the component served a public route. `QATab` and `QACard` now take `canEdit`.
  - **The RSC-payload rule.** `ReportClient` is a client component, so anything on `data.recording` / `data.transcript` is serialized into page source and readable in view-source **whether or not a tab renders it**. Passing the `select('*')` row published the sign-off reviewer's note, `confidentiality_class`, internal objectives/analyst attribution, and the share token; the transcript row carried `raw_response` (the whole ASR vendor payload) and `cost_cents`. `page.tsx` now builds an **allowlisted** recording + transcript — never a denylist, so a column added later cannot silently become public. `agents: []`, `isOwner/isAdmin: false`, `analyticsDatasetId: null`. Page weight dropped ~2.3 MB → ~207 KB as a side effect.
  - **Audio is a separate opt-in** (`recordings.share_audio`, sql/185, default false). Publishing the written report must never imply publishing residents' voices, so an enabled share link with `share_audio=false` still 404s at the audio endpoint. Toggle sits in the Reports tab beside Polished/Verbatim; `POST /share` accepts `include_audio` under the same write-only-when-present rule as `show_verbatim`.
...
23. Share panel on report page (Reports tab).
...
| 7 — Mic check pre-flight panel (2026-06-09) | **Code complete (local)** | `MicCheck.tsx` replaces the bare dropdown+AGC on the idle screen: pick the input device, then **Test microphone** opens a preview stream with live **per-channel level meters** (L/R for stereo — speak into one mic, only its bar moves → confirms the split in-tool; single bar for mono), with **too-quiet / clipping** guidance. Adjustments the browser actually exposes: **AGC**, **echo-cancellation**, **noise-suppression** toggles, and a **software gain boost** (1×–4×, +0…+12 dB) applied via a WebAudio gain graph — the recorder records the graph output when gain≠1 (unity keeps the raw stream, the zero-risk default; the graph preserves stereo). Constraints centralized in `buildAudioConstraints(MicSettings)`, shared by the test and the real capture. **Browser can't set hardware gain** (device/RØDE Central only) — the panel says so. **Full dress-rehearsal additions:** (a) **live captions during the test** (same Deepgram token→WS→`pcm16-worklet` path as the real recorder, best-effort); (b) **record a test clip & play it back** (local Blob/object-URL off the gain-applied stream, never uploaded, 30s cap); (c) **live monitor** — a "🎧 Listen" toggle routes the processed mic to the speakers (gain node, 0=muted) so toggling AGC/echo/noise/gain is **audible in real time** for A/B; off by default with a headphones/feedback warning. All preview nodes (meters, captions worklet, clip dest, monitor) hang off the single gain stage; constraint changes re-open the preview preserving monitor state. (d) **Recommends settings after a test** — tracks peak level + clipping during the test, and on Stop produces plain-English guidance + a suggested `MicSettings` (clipping→lower gain; low→raise gain to target; echo/noise→off for room; stereo+AGC→AGC off) with a one-click **Apply recommended**. (e) **Stereo/mono clip playback** — a stereo test clip plays as recorded (Mic 1 = left ear, Mic 2 = right) with a Stereo/Mono toggle; the mono mix is derived on demand (decode → OfflineAudioContext 1-ch downmix → WAV). (f) **Auto-tune sweep** — opt-in, with on-screen instructions: records ~3s under each AGC×noise-suppress combo (echo-cancel held off), measures peak/clipping + an SNR estimate (10th vs 90th-pct frame RMS), scores (SNR − clip/level penalties), and recommends the winner via the same Apply card. To keep the user talking for the full sweep, it shows a fresh **interesting-facts paragraph** to read aloud (`lib/recordings/readingPassages.ts`, ~80-word true-fact passages, random no-immediate-repeat). The sweep is **paced to the passage** (`sweepTiming`: ~150 wpm, 24–48s total) so it doesn't end before you finish the factoid, and a **single best-effort Deepgram socket runs across the combos** to drive **live reading progress** — words light up karaoke-style + a progress bar, driven by spoken-word count vs the passage length (longer captures also = better SNR estimates). Honest scope: objective signal only, needs consistent speech, can't judge a silent 2nd speaker — the live monitor + clip playback stay the human check. |
