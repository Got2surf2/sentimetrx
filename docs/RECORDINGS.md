...
**Live vs final transcript.** `recordings.live_transcript` = the raw real-time Deepgram ASR (saved on Stop, untruncated). The authoritative post-processed transcript is in `recording_transcripts.segments`. Keeping both enables evaluating how much the batch pass (Whisper/hybrid + entity correction) improved on the live captions. **A side-by-side comparison view is not built yet** — the data is persisted, the UI is the next step.
