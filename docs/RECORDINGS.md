...
- **Browser audio processing forces mono (the main cause).** Chrome's APM (AGC / echo-cancellation / noise-suppression) runs in **mono** — with any of them on, getUserMedia returns a 2-channel track that's actually one channel duplicated. True stereo requires **all three off**. The Mic check now **detects identical channels live** (L−R difference energy vs signal; ~0 = dual-mono) and surfaces a warning with a one-click **"Turn off processing for true stereo"** (sets AGC/echo/noise off → re-opens the preview). If processing is already off and channels are still identical, it points at the device (RØDE not in Split).
