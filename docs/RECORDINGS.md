...
**Per-page header + footer + pagination (added 2026-06-17):** running header/footer on **every page** via `page.pdf({ displayHeaderFooter, headerTemplate, footerTemplate })`. **Header** (top-right): `powered by` + the Datanautix wordmark (data=teal, nautix=orange, one word). **Footer**: generated-at timestamp (left, `MMM D, YYYY, h:mm A UTC`) · the **meeting name** (HTML-escaped, centered, ellipsized) · "Page N of M" (right). Page margins moved from CSS `@page` to the `margin` option so the header/footer bands are reserved and margins don't stack (CSS `@page{margin:0}`). Paper is **US Letter (8.5×11)** with **1″ left/right** margins (`16mm` top / `16mm` bottom); the header/footer side padding tracks the 1″ inset so they align with the body. Widow-title control: `h1,.topic,.ov-h{break-after:avoid}` keeps a section title with the content that follows (a topic heading never sits alone at a page foot), plus `orphans:3;widows:3` on flowing body copy. Q&A cards / presentation items / the overview box already carry `break-inside:avoid`.
