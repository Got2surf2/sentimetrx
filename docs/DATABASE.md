...
| `help_feedback` | Thumbs up/down on the in-product Help assistant (Guide, 🛟) answers (HELP_AGENT §9, sql/184): one row per rating (`rating` ∈ {-1,+1}) with the `(question, answer)` pair + `page_route`. The KB-gap detector — repeated thumbs-down/unhelpful answers drive the next help-article backlog. Written by the service-role `/api/help/feedback` route (stamps org_id from the authed caller); RLS org-scoped SELECT. |
