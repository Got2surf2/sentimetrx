...
| `data_stories` | Data Story short links (sql/198): one row per generated shareable story — crypto-random `slug` (the capability, ~71 bits, same trust model as `question_batches.share_token`), `storage_path` into the private `report-exports` bucket, editable `expires_at` (7d default) and `revoked_at`. Read publicly by slug via `/story/[slug]` (service role; checks both lifecycle columns per request); written solely by the story-generation route, org_id stamped from the org-gated dataset. RLS org-scoped SELECT. |
| `analyst_memories` | "Ana remembers" (sql/197): one row per standing instruction an analyst confirmed — provenance `interview`/`correction`/`observed`, status `active`/`pending`/`archived`, optional `dataset_id` scope (NULL = org-wide). Shapes Ana's framing/emphasis/ordering ONLY, never figures. Written solely by the service-role `/api/analyst-memory` route (org_id+user_id stamped from auth); RLS org-scoped SELECT. |
| `dataset_visit_snapshots` | Dataset digest state (ANALYTICS "since you last looked", sql/202): one row per (user, dataset) — the compact `{rowCount, themeCounts, themeFieldKey}` snapshot taken at that analyst's most recent Ana briefing. The next briefing diffs it in code (`lib/anaDigest`) and upserts fresh state. Written solely by the service-role ask-ana route (org_id paired with the dataset's own org); RLS org-scoped SELECT. |
...
| `dataset_state` | Per-dataset analysis state: `schema_config` (field types/stats, grown by `mergeSchemaStats`), `theme_model`, analytics cache, entity settings, `outlet_action_plans` (sql/183 — place_id-keyed cache of the Outlet Report's LLM action plans; atomic per-key merge via `merge_outlet_action_plan`), `outlet_scan_cache` (sql/195 — persisted Advanced-Analytics scan: per-outlet aggregates + row digest, fingerprint-keyed, written compute-on-miss by `lib/outletReport.loadScan`; safe to NULL). |
