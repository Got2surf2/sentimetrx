...
1. **Accessibility bar**: every prompt must be one an 80-year-old can understand and answer without thinking. Prior wording had drifted into therapy-speak ("in the room with you," "what bubbles up," "sit with this," "what's there for you," "pulls your attention back") and corporate/clinical language ("played out," "magnitude," "anticipated," "based," "setting aside"). Plain-language rewrite across every phase — table in BOTS.md § 9.z replaced wholesale. Examples: "How much does this take up space for you these days" → "How often does this come up for you these days"; "Setting aside whether it was right or wrong — when you sit with this, what's there for you?" → "Even if you think it was the right call, what comes up when you think about it now?"; "What part of your own piece in it weighs on you?" → "What about your part in it still sticks with you?". Added an explicit language-bar paragraph to the system_prompt so the AI itself maintains the standard.

2. **New Phase 7 — Other decisions since** (behavioral shadow): "Since this one, when you've had other big decisions to make — has this one been on your mind?" Captures whether the prior decision casts a shadow over current decision-making — theoretically distinguishes regret (typically reshapes subsequent decisions) from pure disappointment (typically doesn't). Per Connolly & Zeelenberg decision-justification theory, the behavioral spillover is one of the most diagnostic outcomes. Phrased to ask about *psychological presence* rather than *learnings / changes* (those frames prime the change-comparison and contaminate the very thing being measured). Three-route drill: yes → "how does it come up for you"; no → backup probe "is there anything you find yourself watching for now that you weren't before" (catches implicit behavior change); sometimes → "when it does come up, what part of it comes back". Renumbers prior Phases 7-10 → 8-11.

`sql/one-off/2026-05-24-decision-study-plain-language-rewrite.sql` applied to prod. System_prompt grew 7371 → 7517 chars. Live audit clean (zero banned-word matches on the live page). Opener also simplified: "wasn't quite what you'd been expecting" → "didn't turn out the way you thought they would". Phase 9 D1 example "25 to 34" dropped (age-inappropriate default — respondent picks their own decade). Phase 9 D3: "based" → "live".
