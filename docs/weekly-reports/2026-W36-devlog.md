...
## 2026-09-03 — Chart options pass: % modes everywhere honest, guided drag-and-drop (owner ask)
**Why:** Owner (looking at a Themes-over-time count chart): "should we support a % option?… take another full critical pass at every single chart… a more intelligent way to drag and drop… assume naive users." Yes to % — raw counts conflate a theme's drift with volume drift. Shipped: Time Series Count/% share toggle (shares over the FULL pre-cap set; empty buckets null; new top-12 series cap by volume, disclosed; calendar zero-fill via periodCompare.fillBuckets for breakdown AND single-line) · Crosstab Count/% of row/% of column + in-cell values ≤400 cells with count+both-shares hover · Scatter deterministic hash jitter (auto-ON on rating×rating lattices) + OLS trend line with R² + 5,000-point deterministic cap · Distribution % of responses toggle · Funnel % of top stage · Treemap % of total · Table click-to-sort (numeric-aware, blanks last). Drag-and-drop: dragging a field lights viable slots ("▼ drop here") and dims/inerts incompatible ones — wrong drops can't register. Also fixed: loaders showed literal `…` (JSX attributes don't process escapes). Verified: 3 jsdom tests drive the REAL ChartsModule (Plotly mock) — % shares sum to 100 with null empties, 11 zero-filled day buckets, highlight/dim/cleanup — plus pure-helper tests (percentShareSeries, detJitter, compareCells); 2,148 suite green. ⚠️ Pixel-level browser pass PENDING: the shared dev server was saturated by the parallel session's continuous compiles (76 queued requests) — eyeball TS %/crosstab/scatter on Ruths Chris (0aca432b) at the next quiet window before push.

## 2026-09-04 — Ask Ana panel coexists with the chrome: nav stays reachable, Sherpa stays visible
**Why:** owner: "sherpa disappears when Ask Ana is invoked… once it expands, we lose a lot of the menu nav options - the menu should be gracefully wrapping when appropriate." Three distinct defects, one root theme — the panel took space and nothing else was told.

**Header collapse never saw the panel.** The DatasetHeader collapse ladder (`.ana-c1..c9`) was viewport `@media` queries, but opening the panel shrinks the header via `marginRight` — the viewport doesn't change, so no query fired and the right-side actions were clipped by `overflow: hidden`. Converted to `@container` queries with `container-type: inline-size` on the header — same thresholds, now measured against the width the header actually has. Found while verifying: the whole-tab hide rungs (≤850/≤640) had NEVER worked in any era — the tabs' inline `display: flex` beats a stylesheet rule, so those rules now carry `!important` (probed in-page at 830px/600px: c7–c9 then c6 actually disappear).

**Sherpa was buried.** HelpWidget pins `bottom-5 right-5 z-[60]`; the panel is z-1500 and covered it entirely. The panel now dispatches an `ask-ana-panel` CustomEvent (`{open, width}`) on mount/unmount and expand toggle, plus stamps `data-ask-ana-width` on `<html>` for late-mounting listeners (HelpWidget remounts with TopNav on every page). HelpWidget shifts its `right` by the panel width — verified live: launcher at `right: 440px` with the docked panel open, back to 20px on close.
...
## 2026-09-03 — Anger + ascribed-threat emotion flags (the ANES-validated constructs, keyword tier)
**Why:** owner asked "what about anger, fear?" and whether to piggyback an industry-standard detector. Recommended against the lexicons (NRC/LIWC carry documented demographic biases — the same class our own ANES validation flagged — and classifier models return no evidence spans, breaking the every-claim-carries-evidence rule). Instead shipped the two constructs the August ANES study (150,377 open-ends vs actual votes) validated at ~90%+ marker precision, as $0 keyword-tier flags.

**What the validation dictated, now encoded in code and spec (TAXONOMY §2a):** `threat ascribed` is the third-party construct ("this is dangerous", "a threat to…") — 4× the volume of first-person fear; **first-person fear and worry stay dark** (worry = ambivalence-not-exit, softer than a plain complaint, plus unresolved demographic lexicon skew — never compare groups on these flags until settled); fear-mongering accusations ("plays on people's fears", "scare tactics") route to **anger**, not threat; the three negative bands never merge into a composite; emotion is explanation/routing, never prediction. TAXONOMY_VERSION v4→v5 so re-classified rows are distinguishable.

**Verified:** 7 new unit tests (18 total in emotionFlags) covering the tactic-routing rule, negation guards ("not dangerous", "never angry"), first-person-fear darkness, and independent co-firing; full suite green. Backfill re-run on prod (same owner-authorized script — idempotent overwrite, now stamping v5) lands the new flags on all previously processed datasets.

## 2026-09-03 — Emotion charts: live-axis filtering + the five-flag label
**Why:** owner: "like we have themes and dimensions we should have emotions available for charts and stuff." Emotion already IS a chart dimension (`__dim_emotion__`, in the Dimensions picker group wherever taxonomy is enabled — which this morning's backfill flipped on for ANES/Trump/EA/National Parks). The real gap: an emotion-TIER dataset would offer all 8 axes in the picker with 7 of them dead (no ABSA classification ever ran). The Charts picker now filters the Dimensions group to axes with data in `dimSubCounts` (all show until the fetch lands — no flicker regression on restaurant datasets where every axis fires). Also updated `DIM_AXIS_LABEL_LONG.emotion` for the five-flag era.

**Verified in-browser on TEST EA** (backfilled to v5 via the script pointed at TEST — env-override wins over .env.local): picker shows exactly Themes · title · **Emotion Language** (no dead axes), and the Emotion Language bar chart reconciles row-for-row with SQL: disappointment 201 · anger 57 · churn intent 49 · blame 18 · threat ascribed 5.

## 2026-09-04 — Honest percentages, faster Ana subgroups, and the Dimensions toggle actually holds
**Owner session, five hits in one afternoon.** (1) An ANES chart showed a year at "100% threat" — traced to the deterministic block sample catching ONE flagged 1998 row (truth: 19% of 26; year-ordered rows put 1998's strip in block tails). Fixes: tax_* ops go EXACT ≤200K rows (`TAX_EXACT_CAP`; the 57014 was measured at ~1M), Percentage mode drops groups under a 10-count floor with a "too few responses" note, and percent hovers always disclose the raw n. (2) The stacked-bar legend drowned under rotated theme labels — long-label charts now use the box plot's right-side vertical legend precedent. (3) Ana ran "Counting dimension mentions…" on a Dimensions-OFF dataset — the route now computes the same hasDimensions gate as Charts, warns the model, and hard-refuses tax_* ops; labels name the feature ("Counting Dimensions (emotion) tags…"); TextMine's mining-time auto-enable paths treat existing-classification+toggle-off as a standing human opt-out (the auto-tagger had PATCHed the owner's OFF back to ON); backfill script now flips only on first discovery. (4) A 3-condition ANES subgroup took 5+ minutes and timed out — parallel scan waves, fields-only jsonb selects, sampled value discovery, cap 60K→150K, and a cross-turn LRU: measured fail@minutes → 33s cold / instant warm on the exact owner scenario. (5) Panel polish: Sarina-teal header (reads as its own window), and the conversation now survives close/reopen via sessionStorage (streaming stripped; Clear resets).

**Verified:** in-browser on TEST (teal header rgb(14,116,118); thread restored after X→reopen including a typed question; long-label percentage chart with right legend and `%{y}% (n=%{customdata})` hovers carrying real counts) + prod read-only timing harness for the subgroup + 2,171 tests green (anaSegment fakes taught the aliased arrow-select).

## 2026-09-04 — Subgroup matching as one SQL (sql/200+201): minutes → 9 seconds
**Why:** owner ("it is insane and unusable"): a 3-condition ANES subgroup still cost 33s even after the JS parallel pass, because ids were enumerated CLIENT-side. Now `segment_match_ids` filters inside Postgres: conditions compile once per call into a native predicate (format-%L dynamic SQL — injection-safe, everything is a literal) evaluated during the keyset scan, paged 20K at a time. sql/200's first cut (per-row SQL function over a materialized blob CTE) measured 6s/page — dangerously near the statement timeout — so sql/201 rewrote it with the compiled predicate: 1-2s/page, **9.0s end-to-end on the exact owner scenario, identical 7,337 ids**, instant on repeat via the cross-turn LRU. `segment_row_matches` stays as the executable spec of the semantics; the JS scan path remains as the PGRST202 fallback so un-migrated DBs keep working; value resolution (fuzzy matching, mappings, stored-values errors) deliberately stays client-side.

**Ops:** sql/199 (pending since 9/03) + 200 + 201 applied to PROD via the management-API template (`scripts/_apply_199_200_prod.mjs` / `_apply_201_prod.mjs`), ledger recorded, REVOKE lockdown verified true, `docs/db/schema.sql` snapshot refreshed from prod. TEST applied via psql. 42 anaSegment/anaQueryTools tests green — the fakes emulate the RPC and every pre-existing scenario produces identical results through the SQL path; a dedicated test pins JS-fallback parity.

## 2026-09-04 — Provenance steps carry their duration
**Why:** owner, after a slow Ana turn: "maybe we should add a duration for each step of the provenance so we know where things are taking time?" Exactly right — slow turns were opaque. Every query step in the "Show my logic" trail now ends with its wall-clock cost (`· 4.2s`), and each interim reasoning line carries the model round it took (`· thought for 8.1s`), so a slow answer decomposes at a glance into query time vs thinking time. Also: ANES young-R-vs-D findings brief shipped as a branded PDF (pdftoppm-QC'd, ~/Downloads), and the young-men 2020→2024 shift computed through the platform path (segment SQL + exact matcher over all rows).

## 2026-09-04 — Cost accounting: cached calls computed NEGATIVE (estimateCost sign bug)
**Why:** owner asked what each Ana question costs. Pulling real `usage_logs` rows surfaced avg −$0.29/question — impossible. Root cause: `estimateCost` subtracted `cache_read_tokens` from `input_tokens`, assuming inclusion, but Anthropic reports them as SEPARATE counters that add. Every heavily-cached call (Ana: ~144K cache-read vs ~28K input) went negative, understating all cached spend in /admin/usage since caching landed. Stored token counts were always right — only the derived dollars — so history displays correctly under the fixed formula. 3 pin tests (components add, never negative, unknown-model→standard-tier).

**The actual answer (post-fix, prod usage_logs):** all 39 logged Ana questions avg **$0.11** / median $0.04; the complex query-engine era (since 9/01) avg **$0.145** / median $0.054; max observed $2.21 (a long multi-round subgroup comparison). The session's direct script analyses (young R/D, young men, turnout/breadth) used zero LLM — pure compute.

## 2026-09-05 — Context "Related concepts" dimensions were dead code — resurrected server-side
**Why:** the parallel session's never-assume sweep caught it and flagged my lane: `lib/contextConcepts` read `row._tx` from TextMine's client rows to build dimension chips, but the rows route strips `_tx` from every unprojected row — the section could never populate on any dataset. Its 9/03 verification note ("dimensions correctly hidden under the floor") had observed absent data masked by the volume floor. The test that "covered" it fed rows WITH `_tx` — an input that cannot occur in the app.

**Fix (the sweep's "best" route):** `relatedConcepts` now reports the subset's `_rowId`s; ContextCloud fetches one `tax_counts` aggregate per axis scoped to those ids with the active-question fieldKey, behind the same `dimensionsEnabled` gate as the Dimensions tab — platform-recreatable, zero row payload, the same engine every other dimension surface uses. Dead `_tx` read and its local types removed; props threaded through both popovers from TextMineModule's three call sites.

**Verified:** server contract exercised end-to-end on TEST EA with the popover's exact shape — 800 "crash"-subset rows → chips `Disappointment 22 · Anger 11 · Churn Intent 6` (the first output this feature has ever produced; the new anger flag already surfacing). The rewritten test pins the real contract (dimensions never computed client-side + subsetRowIds reported) plus `dimensionChipsFromCounts` floor/cap/labels. In-panel render check pending a logged-in browser session (auth expired; typecheck + the verified endpoint call mirror it).

## 2026-09-04 — Ask Ana compares two subgroups in ONE call (`vs` on query_data)
**Why:** every "X vs Y" question (young R vs young D on ANES; FL dine-in vs GA carry-out) cost Ana two full where-resolutions plus model-side arithmetic on the diff — the exact class of number the deck-credibility rule says code must compute. The open-queue item "compare-two-subgroups Ana tool" (owner idea, 9/04) closes with this.

**Build:** `query_data` takes `vs` (same condition shape as `where`; `where` = group A, `vs` = group B). Both sides resolve through one extracted `resolveScope` helper (per-turn cache + cross-turn memo + fuzzy matching unchanged), both intersect active filters (an emptied side errors naming WHICH group), and the op runs per group through a shared `buildAggParams` — a compared number and a single-group number cannot diverge. The result carries `groupA`/`groupB` (label + n + body) plus a code-computed `comparison`: pp share deltas sorted by |Δ| for counts ops, avg/median deltas for numeric_stats; overlapping groups get an `overlapNote`. Prompt bullet routes comparison questions here; logic trail logs "Compared A (n) vs B (m)"; compare calls never emit a canvas chart (single-scope chart would contradict the answer). 8 new unit tests (2,185 total green).

**Verified:** harness `scripts/_verify_compare_vs.ts` (untracked KEEP) against TEST Carrabba's — shares reconcile to returned counts by hand, group sizes 21,914/3,781 match Ana's earlier two-call run exactly. Live in-browser: "FL dine-in vs GA carry-out" → status "Comparing two subgroups…", one 14.7s Compared step in the trail, answer leads with pp deltas and reports both group sizes (555 vs 21,914) with a small-group honesty note. Notably the model still (correctly) prefers a shared-where crosstab when the groups differ on a single field — `vs` earns its keep on compound groups a crosstab can't express. Gotcha burned an hour: the long-running dev server had silently stopped recompiling — two "failed" live tests ran pre-change code; fresh server fixed it.

## 2026-09-04 — CommentsPanel retired: one comments panel, two row sources
**Why:** the Comments tab ran two visually different panels — the rich client CommentsPanel for themes-only and FilteredCommentsPanel once any entity/dimension facet joined — the deferred second half of the 9/04 merge. Owner gave the go; scoping surfaced that the queue's described mechanism (flip themes to the server /comments fetch) would have been a regression dressed as cleanup: that route's theme predicate is FTS — a DIFFERENT matcher than the canonical regex family the theme cards' counts compile — and it ignores active filters. Owner picked component-unification instead (server flip + matcher migration offered and declined).

**Build:** FilteredCommentsPanel is the ONE panel; themes-only/show-all feed it client rows via new `lib/commentRows.buildClientCommentRows` (canonical `commentMatchesTheme` membership — pinned by test to agree row-for-row — so drill counts don't move; per-theme regexes compiled ONCE after per-row compilation froze the tab at 50K rows × 7 themes). Panel gained `countLabel` ("matched of non-empty (X%)"), per-card theme `badgeThemes` in each theme's palette (precompiled), and the Back button rides the chips slot. CommentsPanel (945 lines) deleted; lint ratchet 172→167 (its 5 warnings died with it, none added). Deliberately dropped (small, owner-visible): color-by-field selector (score-field border tint remains), shuffle, hover-a-theme re-highlighting. 6 unit tests; suite 2,191 green.

**Verified in-browser (TEST):** Carrabba's theme drill 2,231 of 31,847 (7%) with palette highlights + multi-theme badges; show-all 6,746 (21%) fast after the precompile fix; EA Disappointment facet = server mode 201 of 201 (reconciles with the facet's own count) with amber evidence highlights; AI Summary generated from client rows; Back returns to Overview.

## 2026-09-04 — Ask Ana panel polish: factoid visibility + brand styling, status inside the Thinking bubble
**Why:** owner parked three panel nits mid-task (9/04): the "Did you know?" fact strip sometimes rendered clipped under the composer while Ana was thinking; it was styled too muted; and the tool-status step ("Counting values…") floated loose below the Thinking bubble.

**Build:** (1) Clipping root cause — the factoid mounts 3s in and rotates via its OWN state, so the messages-keyed auto-scroll never fired for it; it now `scrollIntoView({block:'nearest'})`s itself on mount and every rotation. (2) Restyled: "DID YOU KNOW?" label in Ana orange (800), fact in Ana teal (600). (3) `FormattedResponse` takes `statusText` and renders the status step inside the Thinking bubble while there's no answer text; once text streams the step reverts to the existing below-bubble line.

**Verified in-browser (TEST Carrabba's):** opening briefing showed "Thinking… / Counting values…" in ONE bubble with the orange/teal factoid below; after a long conversation + a compare question, the new Thinking bubble's factoid sat fully visible above the composer (the exact owner screenshot scenario); JS-measured — scrolled the panel to the top mid-think, next rotation pulled the strip back to fullyVisible:true. No console errors; typecheck + 2,191 tests green.

## 2026-09-04 — Dataset digest: "since you last looked" leads Ana's opening briefing (sql/202)
**Why:** the long-queued digest item, unblocked by the owner's surface decision today: start in Ana's briefing (converges later with shoulder tap as the scheduled vs threshold-triggered modes of one system). It's a heads-up DELTA report, not a summary — Data Story owns summaries.

**Build:** `dataset_visit_snapshots` (sql/202, RLS org-read, service-role writes org-paired) holds one `{rowCount, themeCounts, themeFieldKey}` snapshot per user×dataset, taken at each briefing. On a briefing turn the ask-ana route diffs stored vs current IN CODE (`lib/anaDigest`) and injects the block with orders to lead with it and never recompute; then upserts fresh state — "last visit" = "last briefing", deliberately. Lines: row delta with ago label; theme match-count moves (the stored model's own cached counts — deliberately NOT shares, whose displayed base is substantive comments and isn't snapshotted), |Δ|-sorted, capped at 4 + overflow line; new/removed themes; the whole theme section skipped if the framework's field changed. First visit = store-only; pre-202 DB = digest-less briefing (supabase-js `{error}` checked, not try/caught — the retryTransient lesson). 8 unit tests.

**Verified:** sql/202 applied to TEST (psql), `test:rls` green. In-browser on TEST Carrabba's: briefing #1 stored the snapshot (verified in SQL); mutated it via SQL (backdate 3d, rows −1,220, one theme 60→89, one theme swapped); briefing #2 opened "🔴 Since your last visit (3 days ago): +1,220 new responses" and flagged all three framework shifts verbatim from the injected block, then re-upserted current state. Typecheck + full suite green. ⏭ prod: sql/202 apply via mgmt-API template + schema-snapshot regen ride the next prod migration pass.

## 2026-09-05/06 — Conjoint engine + DDHQ demo (external repo: conjoint-engine)
**Why:** owner exploring a conjoint offering ("be sure we can do it before offering it"); a DDHQ (Decision Desk HQ) meeting inside ~2 weeks turned the prototype into a full demo. Built OUTSIDE this repo (private github.com/Got2surf2/conjoint-engine) to protect the differentiator and avoid platform build costs; Sentimetrx supplies vendor keys (Deepgram/Anthropic via .env.local) and will receive the probe-verbatims CSV as a dataset so the analysis half runs in the product.

**What:** validated HB choice engine (synthetic recovery gate; camera benchmark 86% vs 20% chance; AMCE replication of Hainmueller–Hopkins–Yamamoto to the third decimal) · voice interview (Sarina/Datanautix persona, Deepgram Aura TTS + STT, tap/type/speak parity) with model-triggered probes, one-step digs on vague answers, abstention + stated-tie escape hatches, dissonance prohibitions (credential + income plausibility classes), per-respondent attribute-order randomization · results dashboards with plain-English narrative, 95% CIs on the displayed scale, screens-vs-tiebreakers, preference segments (k-means on importance profiles) + per-segment simulator (one matchup spanned 4%–77%) · live re-estimation judged against a measured MCMC noise floor (±0.79, drawn as a band) · tabbed one-URL demo shell · RUNBOOK + 18-item METHODS_REVIEW. ~28 commits; repo private; demo runs locally.

**Sentimetrx tie-ins:** probe verbatims are the first choice+why dataset anywhere (none exists publicly — checked); upload path via out/probe_verbatims.csv. Owner process rules set this week now global: adversarial self-review before presenting (never make the owner QA), and hold ALL pushes (any repo) without explicit authorization.
