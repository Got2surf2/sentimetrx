...
### Dimensions: wider hair/food-safety rules + scoped Comments search (2026-06-07)

Two follow-ups from inspecting Carabbas GSS dimensions directly against prod.

**1. Food-safety under-firing on "hair".** The keyword dict flags hair only on specific in-food phrasings (to avoid false alarms on "blonde hair"/"angel hair"). Auditing all 24 rows with the word "hair" found 2 genuine misses: "hair in the soup" (dict had "hair in the food" but not "…soup") and "a hair found in my food" (word order vs "found a hair"/"a hair in"). Added generalized **in-food** phrasings (`hair in the`, `hair found in`, `hair on my plate/food`, `hairs in my/the`) to `lib/taxonomyKeywords.ts`; bumped `TAXONOMY_VERSION` v2→v3. Verified 9/9: catches both misses, fires on none of the hygiene ("hair dangling", "hairnet") or staff-appearance ("blonde hair") cases — those stay out by design. NOTE: existing tagged rows keep their v2 tags until re-classified (no auto-reclassify of already-tagged rows).

**2. Comments-tab search was dataset-wide.** The "Search comments" box in TextMine reused the dataset-wide `SearchPanel` → `/api/datasets/[id]/search`, which FTS-searches the `tsv` column built from EVERY field of EVERY row (`sql/031`) — so it ignored the active-field selection, the schema field on/off flags, and the current filters (results included rows/fields not in view). Replaced it (in the Comments tab only) with a new client-side `CommentSearchPanel` that searches **only the in-view filtered rows × the active open-ended field(s)** — so it inherently respects filters, field selection, and schema (off/non-open-ended fields are never in scope). The dataset-wide `SearchPanel` is unchanged and still powers the analyze-header search by design. tsc + 461 tests clean.

**Re-classify applied to prod (2026-06-07, user-authorized).** Re-ran the keyword classifier (brand `core`, dict v3) over the two live datasets so the new hair rules take effect on existing rows: **Carabbas GSS** (all three field-keys — MOST / LEAST / MOST+LEAST; combined run last so the legacy `dataset_row_taxonomy` table stays on the combined key) and **Ruths Chris Reviews** (review_text + owner_response; review_text last). A read-only probe first confirmed both were originally `core` (stored RC food-safety 43 ≈ core-44, not the learned-`rc` 293), so `core` preserves prior coverage and only adds the hair hits. Result — food-safety per key: Carabbas LEAST 24→26, combined 25→27, MOST 1 (unchanged); RC review_text 43→44, owner_response 0. All keys now `prompt_version=v3`. Idempotent upsert (no deletes).

### Town Hall: setup-before-media + attribution + config versioning — data model (2026-06-07)

**Why**: owner wants a Town Hall *project* set up before the audio/video exists (ingest the deck/briefs to pre-fill the setup, attach the recording later), proper attribution of the analysis org (default Datanautix) + analyst(s), and a version history of the project config so every deliverable traces to the exact setup that produced it. Design scoped + decisions locked with the owner this session; this is phase 1 of 5 (data model only — no behavior yet).

**Phase 1 (sql/118 + `lib/recordings/types.ts`)**: additive, older recordings unchanged.
- Lifecycle: new `draft` / `awaiting_media` states ahead of `uploading`; both excluded from the active-pipeline partial index.
- `recordings` columns: `analysis_org` (default 'Datanautix'), `analysts` jsonb, `objectives` jsonb `{summary,questions[]}` (will be AI-proposed from uploaded docs + steer the synthesis pass), `setup_provenance` jsonb, `confidentiality_class` (CHECK, default `client_confidential`), `signoff` jsonb, `analyzed_config_version` int.
- New `recording_config_versions` table (RLS + org-scoped SELECT from day one) — snapshot per analysis run (`source=analysis`) + manual "Save version" (`source=manual`); `UNIQUE(recording_id, version_number)`.
...
### Town Hall: AI auto-classification of presentation scope (§3.5e) (2026-06-07)

The follow-up to manual scope flagging. New analyze pass (`classifyPresentationScope` + `buildScopeClassifyPrompt`, Sonnet) that runs **only when the meeting had a presentation**: `runAnalyze` builds a `presentationContext` (slide `presentation_outline` titles/key-points/figures, else the presentation-phase transcript via `buildPresentationContext`) and passes it through `AnalyzeInput.presentationContext`. One batched call labels each Q&A pair `payload.presentation_scope` in_scope/out_of_scope, grounded strictly in the presentation summary; mutates payloads in place (same refs as extractions). Pre-fills the manual flag (§3.5d), which still overrides. Non-fatal; skipped for pure-Q&A meetings. Cost ≈ $0.05–0.15. Spec RECORDINGS.md §2.4/§3.5/§3.5e/§5.4. tsc clean, 361 unit tests. Takes effect on the next analyze/re-extract of a presentation meeting. NOT pushed.

### Town Hall: per-pair sentiment label on Q&A cards (2026-06-07)

Owner asked for a sentiment label on each question. The data already existed (`QaPairPayload.sentiment` — asker's-perspective exchange tone, set by extraction, powering sentiment_breakdown + Coverage topic dots); it just wasn't shown per-pair. Surfaced it: a colored sentiment chip on each Q&A card + a dot in the collapsed header + sentiment filter chips on the Q&A tab (`sent:<value>`, reusing the module `SENTIMENT_DOT` map). No AI, no migration. Pre-June recordings show none until re-analyzed. First slice of the folded edit-anytime plan → [[project_townhall_edit_anytime]]. Spec RECORDINGS.md §5.4. tsc clean, 361 unit tests.

### Town Hall: edit-anytime phase 1 — full-setup PATCH + document file role (2026-06-07)

Foundation for editing a town hall at any stage. Extended `PATCH /api/recordings/[id]` (§4.3b) to accept the remaining setup fields — `meeting_date`, `location`, `language`, `setup_inputs` (object), `meeting_profile` (object|null) — on top of the name/brand/agent/attribution/objectives it already took, so the (planned) unified Setup editor can change anything at any lifecycle stage. Metadata edits are instant; setup_inputs/meeting_profile/objectives are analysis-shaping (re-analyze to apply). New migration `sql/119` widens `recording_files.file_role` CHECK to include `document` (briefs, for phase 3's document-anytime upload — stored, not pipeline-processed); type `RecordingFileRow.file_role` updated. **sql/119 NOT yet applied to prod** (not needed until phase 3). Spec RECORDINGS.md §4.3b. tsc clean, 361 unit tests. → [[project_townhall_edit_anytime]]

### Town Hall: edit-anytime phase 2 — unified Setup editor (2026-06-07)

The create wizard and editing-after-the-fact are now **one component**. Extracted `components/recordings/RecordingSetupForm.tsx` (a mode-switched generalization of the old create wizard); `RecordingWizardClient` is now a thin `mode="create"` wrapper. New page `app/recordings/[id]/setup/page.tsx` renders it in `mode="edit"`, reachable at **any** lifecycle stage via an "Edit setup" link added to both the status header and the report header/Export tab. Edit mode PATCHes §4.3b and stays put (Saved ✓ + `router.refresh()`); it omits brand/agent (owned by the report Export tab) and ASR strategy (fixed once processing starts) so each field has exactly one editor. **Removed the duplicate `ProjectDetailsPanel`** from ReportClient — the Export tab now links to the unified editor instead of carrying a parallel subset form (Enforce-Shared-Components). Applied `sql/119` to prod (file_role widened to media|slides|document) ahead of phase 3. Spec RECORDINGS.md §5.2/§5.4/§5.7 + §4.3b. tsc clean; 459 unit/integration tests pass (the 2 pre-existing `recordings-routes` 401-vs-403 failures are unrelated to this change). NOT pushed. → [[project_townhall_edit_anytime]]

### Town Hall: fix 2 stale/inconsistent tests in recordings-routes (2026-06-07)

Cleared the two pre-existing failures in `tests/integration/recordings-routes.test.ts`:
- **Audio route auth (real fix):** `GET /api/recordings/[id]/audio` used `getUserContext` → returned **401** for an authed user with no org, where its sibling action sub-routes (process/analyze/reanalyze) return **403**. Reworked to the canonical pattern — `getAuthUser` (401) → `users.org_id` lookup (403 when missing) → recording load — while **preserving the admin-org bypass** (now read from `getUserContext` only for the `isAdminOrg` flag) and the build-path-from-recording-org behavior (the `9d37139b` play-segment fix). Spec RECORDINGS.md §4.12 now states the 401/403/404 contract.
- **Stale create test:** the "400 on … empty files" assertion predated setup-before-media (`d6fe82c8`), which makes an empty `files[]` a valid `awaiting_media` create (201). Removed the stale assertion and added positive coverage that empty files → 201 `awaiting_media`.

tsc clean; full suite 462 pass (was 460 + 2 failing). NOT pushed. → [[project_townhall_edit_anytime]]

### Town Hall: edit-anytime phase 5 — Presentation report tab (2026-06-07)

The last gap in the edit-anytime arc: `proceedings_summary` (the presentation half of a community meeting) was rendered in the deck/PDF but had **no on-screen home** — the report tabs were qa/actions/coverage/transcript/export only. Added a **Presentation** tab to `ReportClient` that mirrors the deck's "Meeting Overview" (`lib/pptx/recordingDeck.ts`): the neutral overview paragraph + one card per agenda item (title, presenter, what_was_presented, key_figures chips, slide_refs), plus the source deck filename/slide-count from `presentation_outline`. **Conditional** — only shown when the meeting had a presentation phase (or a proceedings_summary exists); it leads the tab bar when present (default tab stays Coverage), and `?tab=presentation` falls back to Coverage otherwise. Empty state for a presentation meeting whose summary wasn't generated. No AI, no migration, no new data — pure surfacing of existing fields. Spec RECORDINGS.md §5.4 (new tab 0). tsc clean. **Populated tab NOT pixel-verified** — prod currently has 0 presentation recordings and I won't fabricate prod data; the markup mirrors the already-QC'd deck slide and the report's visual language, so owner will eyeball on the next community-meeting analysis. → [[project_townhall_edit_anytime]]

### Agents: explain the Agent Study health dot on hover (2026-06-07)

Owner asked why the dot next to the agent name was red and wanted the reason on hover. The dot encodes `computeHealth`'s status (green/amber/red/idle); red = ≥10 open questions AND >25% of the 30-day conversation count (an unanswered-backlog alert). Added `healthTitle()` in `app/bots/[id]/report/ReportClient.tsx` (mirrored in `lib/agentStudyHtml.ts` for the shared snapshot) that turns the health counts the report already has into a plain-English `title` tooltip per dot colour, with `cursor:help`. Pure client-side from existing `AgentHealth` fields (openQuestions / conversations30d / conversations7d) — no lib/agentStudy change, no STUDY_SCHEMA_VERSION bump, no cache concern. Spec BOTS.md §health (dot thresholds + tooltip). tsc clean. NOT pushed.

### Town Hall: edit-anytime phase 3 — documents-anytime (2026-06-07)

Persist project documents (the deck + briefs) to a recording at any lifecycle stage, not just pre-fill. New §4.1e endpoints: `POST /api/recordings/[id]/documents` (direct multipart — docs are small, no TUS) attaches a `role='slides'` deck (PDF → vision-read by ingestSlides, sets has_slides, at-most-one/replace) or a `role='document'` brief (stored, not processed); `DELETE …/documents/[fileId]` detaches one (org-paired, **refuses media files**). Both org-gate via getUserContext + `.eq('org_id')` with admin-org bypass. The unified Setup editor (§5.7) gains an edit-mode "Project documents" pane: lists attached docs (server-fetched), attach-deck (also pre-fills) / attach-brief / remove. Create mode stays pre-fill-only (no recording to persist to yet). sql/119's `document` file_role (already applied) is now used. Added 3 gating tests (401/403/404 + id/org_id pairing + media-refusal) to recordings-routes.test.ts. Spec RECORDINGS.md §4.1e + §5.7 + §4.1d. tsc clean; 465 tests pass. NOT pushed. → [[project_townhall_edit_anytime]]

### Town Hall: edit-anytime phase 4 — re-analyze drift banner (2026-06-07)

Closes the edit-anytime arc. New `isAnalysisConfigDrifted` (lib/recordings/configVersion.ts) compares ONLY the analysis-shaping fields (setup_inputs/meeting_profile/objectives) of the current config against the snapshot at `analyzed_config_version` (deterministic `canonical` compare) — metadata edits (name/location/analysts/attribution) are deliberately NOT drift. Surfaced as an amber banner: on the **report** (above the tabs) with a one-click **Re-analyze (~$1)** confirm → `POST …/reanalyze {scope:'all'}` → router.refresh(); on the **Setup editor** (edit mode) as an informational banner with an "Open report →" link (keeps the re-analyze action in one place). **Key fix:** a `scope='all'` reanalyze now re-snapshots + re-stamps `analyzed_config_version` (it previously didn't — only the first-analysis `/analyze` route did), so re-analyzing actually clears the banner; `scope='topic'` leaves it as-is (partial). 7 new unit tests for the drift helper (never-analyzed → false, metadata-only → false, each shaping field → true, missing snapshot → false). Spec RECORDINGS.md §4.11/§5.4/§5.7. tsc clean; 472 tests pass. NOT pushed. **Edit-anytime phases 1–5 all done.** → [[project_townhall_edit_anytime]]

### Docs sync: FEATURES.md — Town Hall edit-anytime capabilities (2026-06-07)

Inventory sync after the edit-anytime arc (phases 1–5). Added a "Project setup & editing (edit-anytime)" block to FEATURES.md (setup-before-media, unified Setup editor editable at any stage, documents-anytime, drift banner, config versioning + sign-off) and a "Presentation report tab" bullet under the meeting tool. RECORDINGS.md (canonical spec) + the per-phase devlog entries already carry the detail; this keeps the top-level feature inventory current. SPEC.md doesn't enumerate recordings, so no change there.
