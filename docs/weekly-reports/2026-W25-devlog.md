...
## 2026-06-22 — Outlets: Lottie loader on outlet switch

**Why**: Switching outlets in the Outlets report is a server round-trip (the report recomputes server-side from `?outlet=`) — there was no loading feedback, so it looked frozen between outlets.
