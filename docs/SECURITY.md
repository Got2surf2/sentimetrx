...
> **Update (2026-06-01).** `getUserContext` previously selected a non-existent `users.is_admin` column (admin status lives in `users.role` / `organizations.is_admin_org`), which made the query error and bounce authenticated pages to `/login`; it now derives `isAdmin` from `org.is_admin_org`. Feature visibility remains the org∩user intersection in `effectiveFeatures` (`lib/resolveOrg.ts`), now with Analytics as the parent gate for its sub-features (googleReviews / reddit / substack / **taxonomy** — the 2026-06-05 Dimensions capability, an analyze-child forced off when Analytics is off). (Recordings was promoted out to the top-level Town Hall product 2026-06-04 — gated on `features.recordings` alone.)
...
| **Meeting recordings + slides** | audio/video files, presentation decks | PII (assume) | Stored in the `recordings` Storage bucket, org-scoped (`<org_id>/<recording_id>/…`). Audio → ASR vendors (Whisper/Deepgram); transcripts → Claude. **Slides (sql/097, 2026-06): rendered to page PNGs and sent to Claude vision** as image content blocks (`lib/ai.ts`) — same Anthropic egress posture as transcript prompts; signed-URL render in a Vercel Sandbox, no file passes through worker memory. Same retention as other recording artifacts. |
