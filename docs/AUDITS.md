...
| **Filters-compliance sweep** | SCHEDULED (Brief F) | Every consumer of `SerializedFilters`/`applyFilters` + surfaces that silently ignore filters | `docs/PERFORMANCE_REVIEW.md` §7 Brief F (results table appended on completion) | A new filter consumer ships, or filter semantics change |
