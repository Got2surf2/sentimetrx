...
| **npm audit** | **CI gate on every push since 2026-09-14** (`scripts/audit-gate.ts`) + Dependabot alerts and weekly grouped PRs (`.github/dependabot.yml`, since 2026-09-14); last manual pass 2026-09-14 (8 → 2, both allowlisted) | High/critical ROOT advisories vs `audit-allowlist.json` (entries expire via `reviewBy`) | `audit-allowlist.json` + ENGINEERING.md npm-audit sections; weekly governance report | Automatic — a new root turns CI red; re-verify an allowlist entry when its `reviewBy` passes |
| **CodeQL** | Every push/PR to `main` + weekly Monday 02:00 ET since 2026-09-14 (`.github/workflows/codeql.yml`) | `javascript-typescript` + `actions`, default query suite; `tests/` and `scripts/oneoff` excluded | Security → Code scanning; weekly governance report (Security band reads the open-alert count) | Automatic. NOT a deploy gate yet — triage the first-run backlog, then make it a required check |
...
| **Charts-tab design/correctness audit** | 2026-09-03 (fix set + options pass shipped same day) | All 13 chart types in `ChartsModule.tsx`: input validity, truncation honesty, statistical integrity, matcher consistency; second pass added per-chart characterization options (TS % share, crosstab %, scatter jitter/trend/cap, dist %, funnel/treemap %, table sort) + drag-target guidance | `docs/ANALYTICS.md` "Charts audit fix set" + "Chart options pass" (both 2026-09-03); still deferred (saved-chart display options, `_aggCache` invalidation, module split) in memory `project_charts_audit` | A new chart type ships, or a renderer adds its own truncation/ordering instead of `topCategoryKeys` |
