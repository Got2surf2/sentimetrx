...
async function main() {
  ...
  slides.push({
    ...
    // 3 items per column — renderer caps at 3 anyway. Pick the most impactful.
    inputs: [
      { value: totalChars.toLocaleString(),          label: 'characters of text analysed', sub: `${fieldValues.length} responses · ${totalSentences.toLocaleString()} sentences · ${uniqueWords.toLocaleString()} unique words` },
      { value: allRaw.length.toLocaleString(),       label: 'entity mentions extracted', sub: 'comma · semicolon · "and" · slash · ellipsis splits' },
      { value: `${members.length} datasets`,          label: 'source datasets in the collection', sub: '1 open-ended field · all rows examined' },
    ],
    processing: [
      { value: `${tracker.aiCalls} AI passes`,        label: 'Claude Haiku (fast tier)', sub: `~${Math.round((tracker.inputTokens + tracker.outputTokens) / 1000)}K tokens · 0 retries · 100% schema-valid` },
      { value: `${uniqueRaw.length} → ${canonicalCount}`, label: 'mentions canonicalised', sub: `${uniqueRaw.length - canonicalCount} variants merged · ${Math.round((uniqueRaw.length - canonicalCount) / uniqueRaw.length * 100)}% deduplication` },
      { value: totalComparisons.toLocaleString(),     label: 'entity comparisons performed', sub: `${Object.keys(catCounts).length} categories auto-assigned + cross-validated` },
    ],
    outputs: [
      { value: String(top24.length),                    label: 'organisations ranked', sub: `top ${top24.length} = ${top24Share}% of mentions · ${catBars.length} categories charted` },
      { value: String(quotes.length),                   label: 'verbatim quotes selected', sub: 'one representative per category' },
      { value: String(tracker.dataPointsSurfaced),      label: 'data points surfaced', sub: `${tracker.insightsStated} insight statements · ${slides.length + 1} slides total` },
    ],
    pipelineStages: [
