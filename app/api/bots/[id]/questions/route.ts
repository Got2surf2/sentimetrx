...
export async function GET(_req: NextRequest, props: Params) {
  ...
  const { data: rows, error } = await service
    ...
    .select('id, session_id, conversation_id, turn_id, user_message, language, classification, status, resolved_by, resolved_at, notes, suggested_kb_addition, answer_text, draft_response, original_comment, source, external_contact, batch_label, created_at')
    .eq('bot_id', params.id)
