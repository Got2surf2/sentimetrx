...
export async function POST(_req: NextRequest, props: Params) {
  ...
  const { data: col } = await service.from('collections').select('id, org_id, slug, kind').eq('id', params.id).single()
  if (!col) return NextResponse.json({ error: 'Collection not found' }, { status: 404 })
  if (!isAdmin && (col as any).org_id !== orgId) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  const colOrg = (col as any).org_id as string
  const colSlug = (col as any).slug as string | null
  ...
  const linked = (agents ?? []).filter(a => slugify(String((a as any).brand_tag ?? '')) === colSlug)

  let agentsPulled = 0
  let pushed = 0
  for (const a of linked) {
    const r = await rollupAgentEntitiesToBrand(service, { agentId: (a as any).id, orgId: colOrg, brandTag: (a as any).brand_tag })
