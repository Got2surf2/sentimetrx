...
const DECKS: Deck[] = [
  {
    ...
    subtitle: 'Reimagining the federal comment-analysis stack (CARA for the Forest Service, PEPC for the Park Service / DOI), worked live on the Blue Mountains comment record. Opens on the TWO SIDES of a comment period — Ask (a real-time agent answering questions, capturing + summarizing every conversation) vs Record (capturing + processing formal comments) — grounded in the real intent split (81% pure positions, 19% ask-and-position, ~1% pure question), showing CARA only does the Record side, by hand. Then walks the CARA-equivalent view (funnel, dedup, issues, stance), then the NEXT funnel layer — the open questions buried in the comments — and a MEASURED answerability test: the deployed Blue Mountains assistant was pointed at the most-asked real questions and graded by a strict independent judge — measured with the full 371-page Draft EIS ingested: 28% high-confidence, 64% at least partial (71% by question volume), up from 20%/52% on the 28 curated chunks; retrieval tuning targets 80%+. Includes a PII-triage capability (auto-flag PII to clear the held-pending-review backlog: 88% of the released set auto-cleared, 12% to a reviewer) and closes on labor-reduction + human-in-the-loop. Every number from the real corpus + live agent; no invented stats.',
