...
import {
  REO_DOMAINS, REO_ASPECTS, REO_SENTIMENTS, REO_SEVERITIES, REO_DOMAIN_COLOR,
...
const inputStyle: React.CSSProperties = { fontSize: 16, padding: '6px 8px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', color: NAVY }

export default function GoldSetClient() {
  const [reviews, setReviews] = useState<Review[]>([])
  const [counts, setCounts] = useState<Record<string, number>>({})
  const [idx, setIdx] = useState(0)
  const [working, setWorking] = useState<ReoObservation[]>([])
  ...
  async function load() {
    ...
    setCounts(j.counts || {})
    // jump to the first pending review
  ...
  useEffect(() => {
    if (!cur) return
    setWorking(JSON.parse(JSON.stringify(cur.gold ?? cur.proposed)) as ReoObservation[])
  ...
  function setObs(i: number, patch: Partial<ReoObservation>) {
    setWorking(w => w.map((o, j) => j === i ? { ...o, ...patch } : o))
  }
  function changeDomain(i: number, domain: ReoDomain) {
    // reset aspect to the new domain's first valid aspect
    setObs(i, { domain, aspect: REO_ASPECTS[domain][0] })
  }
  function addObs() {
    setWorking(w => [...w, { domain: 'FoodBeverage', aspect: REO_ASPECTS.FoodBeverage[0], sentiment: 'Negative', evidence: '', severity: 'none' }])
  }
  ...
  async function save(status: 'approved' | 'edited' | 'skipped') {
    ...
    if (status !== 'skipped') body.gold = status === 'approved' ? (cur.gold ?? cur.proposed) : working
    const res = await fetch('/api/admin/reo-gold-set', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
    })
    if (!res.ok) { const j = await res.json().catch(() => ({})); setErr(j.error || 'Save failed'); setSaving(false); return }
    // reflect locally + advance to next pending
    setReviews(rs => rs.map((r, j) => j === idx ? { ...r, status, gold: body.gold ?? null, reviewer_note: note } : r))
    setCounts(c => ({ ...c, [status]: (c[status] || 0) + 1, pending: Math.max(0, (c.pending || 1) - (cur.status === 'pending' ? 1 : 0)) }))
    setSaving(false)
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 pt-24 pb-16">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: NAVY }}>Dimensions Gold Set</h1>
          <p style={{ color: SLATE, fontSize: 13, marginTop: 4 }}>Review my proposed Domain › Aspect labels. Fix, delete, or add — your version becomes the truth.</p>
        </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: 20 }}>
        ...
        {/* Observations editor */}
        <div style={{ fontSize: 12, fontWeight: 800, color: SLATE, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Labels ({working.length})</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {working.map((o, i) => (
            <div key={i} style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', padding: 8, borderRadius: 10, border: '1px solid #eef2f7', background: '#fafbfc' }}>
              ...
              <button onClick={() => removeObs(i)} title="Remove" style={{ border: 'none', background: 'transparent', color: RED, fontSize: 18, cursor: 'pointer', lineHeight: 1, padding: '0 4px' }}>×</button>
            </div>
          ))}
        </div>
        <button onClick={addObs} style={{ marginTop: 10, fontSize: 13, fontWeight: 700, color: HERMES, background: '#fff', border: `1px dashed ${HERMES}`, borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>+ Add label</button>
        ...
        <div style={{ marginTop: 18 }}>
          ...
          <textarea value={note} onChange={e => setNote(e.target.value)} rows={2}
            placeholder="e.g. 'route hot-plate warnings to Operations, not Service' or 'this one's a Mixed I'd split differently'"
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 18, flexWrap: 'wrap' }}>
          <button disabled={saving} onClick={() => save('approved')}
            style={{ fontSize: 14, fontWeight: 700, color: '#fff', background: GREEN, border: 'none', borderRadius: 10, padding: '9px 16px', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>✓ Looks right</button>
          <button disabled={saving} onClick={() => save('edited')}
            style={{ fontSize: 14, fontWeight: 700, color: '#fff', background: HERMES, border: 'none', borderRadius: 10, padding: '9px 16px', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>Save my edits</button>
          <button disabled={saving} onClick={() => save('skipped')}
            style={{ fontSize: 13, fontWeight: 600, color: SLATE, background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '9px 14px', cursor: 'pointer' }}>Skip</button>
