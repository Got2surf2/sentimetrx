...
export default function ClaraPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f6fdfe', display: 'flex', flexDirection: 'column', fontFamily: "'Be Vietnam Pro', 'Nunito Sans', system-ui, sans-serif" }}>
      {/* Header */}
      <header style={{
        background: 'linear-gradient(135deg, #2f3c4c, #1e2a36)',
        padding: '16px 24px',
        display: 'flex',
        alignItems: 'center',
        gap: 12,
        ...
        <div style={{
          ...
          background: 'linear-gradient(135deg, #8ae0e5, #5bbfc4)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: '1rem', fontWeight: 700, color: '#2f3c4c',
        }}>
          C
        </div>
        <div>
          ...
          <div key={i} style={{
            ...
            {msg.role === 'assistant' && (
              <div style={{
                ...
                background: 'linear-gradient(135deg, #8ae0e5, #5bbfc4)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '0.875rem', color: '#2f3c4c', fontWeight: 700,
              }}>C</div>
            )}
            <div style={{
              maxWidth: '80%',
              ...
              background: msg.role === 'user' ? '#2f3c4c' : 'white',
              color: msg.role === 'user' ? 'white' : '#1a1a1a',
              fontSize: '0.9rem',
              lineHeight: 1.6,
              border: msg.role === 'assistant' ? '1px solid #d8eff0' : 'none',
        ...
        {loading && (
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div style={{
              ...
              background: 'white', border: '1px solid #d8eff0',
              display: 'flex', gap: 6, alignItems: 'center',
            }}>
              <span className="clara-typing-dot" style={{ animationDelay: '0ms' }} />
              <span className="clara-typing-dot" style={{ animationDelay: '200ms' }} />
              <span className="clara-typing-dot" style={{ animationDelay: '400ms' }} />
            </div>
          </div>
        )}
            ...
            {SUGGESTIONS.map((s, i) => (
              <button key={i} onClick={() => sendMessage(s)}
                style={{
                  ...
                  background: 'white', border: '1.5px solid #d8eff0',
                  color: '#566476', fontSize: '0.8125rem', fontWeight: 500,
                  cursor: 'pointer', fontFamily: 'inherit',
                  transition: 'all 0.15s',
                }}
                onMouseEnter={e => { (e.target as HTMLElement).style.borderColor = '#8ae0e5'; (e.target as HTMLElement).style.color = '#2f3c4c' }}
                onMouseLeave={e => { (e.target as HTMLElement).style.borderColor = '#d8eff0'; (e.target as HTMLElement).style.color = '#566476' }}
              >
                {s}
              </button>
            ))}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <textarea
            ...
            placeholder="Ask Clara about Craniometrix..."
            rows={1}
            style={{
              flex: 1, resize: 'none',
              padding: '10px 16px', borderRadius: 24,
              border: '1.5px solid #d8eff0', outline: 'none',
              fontSize: '0.9rem', fontFamily: 'inherit',
              lineHeight: 1.5, maxHeight: 120,
              background: '#f6fdfe',
            }}
            onFocus={e => (e.target as HTMLElement).style.borderColor = '#8ae0e5'}
            onBlur={e => (e.target as HTMLElement).style.borderColor = '#d8eff0'}
            onInput={e => {
          ...
          <button
            ...
            style={{
              ...
              background: input.trim() && !loading ? '#2f3c4c' : '#d8eff0',
      ...
      <style>{`
        ...
        .clara-typing-dot {
          ...
          background: #8ae0e5; display: inline-block;
          animation: claraDotPulse 1.4s infinite ease-in-out both;
        }
      `}</style>
    </div>
  )
