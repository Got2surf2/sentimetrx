 ...
 *
 * Single test on purpose: one login, then both the UI smoke and the API round-
 * trip ride the same authenticated context — repeated sign-ins trip auth rate
 * limiting.
 */
...
test.describe('Saved Views', () => {
  ...
  test('ViewsBar renders + view/snapshot CRUD round-trip', async ({ page }) => {
    // Log in once — the page.request calls below reuse this cookie context.
    ...
    // UI smoke — the views bar mounts on the workspace.
    await page.goto(`/analyze/${datasetId}`)
    await expect(page.getByTestId('views-bar')).toBeVisible({ timeout: 30_000 })

    // API round-trip through the authed context.
    const base = `/api/datasets/${datasetId}/views`
    const stamp = String(Date.now())

    // Mutating verbs are CSRF-guarded by proxy.ts (Origin host must match the
    // request host). page.request omits Origin, so set it explicitly — the real
    // browser UI sends it automatically.
    ...
    // Create a view.
    res = await page.request.post(base, { ...H, data: { kind: 'view', name: `_e2e_view_${stamp}`, filter_config: { filters: {} } } })
    ...
    // Rename (update) works.
    res = await page.request.patch(`${base}/${view.id}`, { ...H, data: { name: `_e2e_view_${stamp}_renamed` } })
    expect(res.status()).toBe(200)

    // Freeze a snapshot — default 30-day TTL is stamped.
    res = await page.request.post(base, { ...H, data: { kind: 'snapshot', name: `_e2e_snap_${stamp}`, frozen: { filteredRowCount: 0, analytics: { totalRows: 0, computedAt: '', fieldSummaries: {} } } } })
    expect(res.status()).toBe(201)
    ...
    res = await page.request.patch(`${base}/${snap.id}`, { ...H, data: { name: 'nope' } })
    expect(res.status()).toBe(400)

    // But the retention clock is mutable: keep forever.
    res = await page.request.patch(`${base}/${snap.id}`, { ...H, data: { expires_at: null } })
    expect(res.status()).toBe(200)
    ...
    expect((await page.request.delete(`${base}/${view.id}`, H)).status()).toBe(200)
    expect((await page.request.delete(`${base}/${snap.id}`, H)).status()).toBe(200)
  })
})
