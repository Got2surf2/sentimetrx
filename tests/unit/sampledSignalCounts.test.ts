...
function makeService(pages: Record<string, unknown>[], calls: Record<string, unknown>[] = [], rpcName = 'sampled_signal_counts') {
  let i = 0
  return {
...
describe('sampledNumericFieldStats — keyset pager (sql/163)', () => {
  it('accumulates sum/n/min/max across pages and derives the mean', async () => {
    const calls: Record<string, unknown>[] = []
    const svc = makeService([
      { n_scanned: 5000, n: 4000, sum: 16000, min: 1, max: 5, last_hash: 10, last_id: 20 },
      { n_scanned: 5000, n: 3000, sum: 15000, min: 2, max: 4, last_hash: 30, last_id: 40 },
    ], calls, 'sampled_numeric_field_stats')
  ...
  it('passes the alias map through and handles a rating-less sample', async () => {
    const calls: Record<string, unknown>[] = []
    const svc = makeService([
      { n_scanned: 3000, n: 0, sum: null, min: null, max: null, last_hash: null, last_id: null },
    ], calls, 'sampled_numeric_field_stats')
  ...
  it('throws on RPC error so callers fall back to the exact aggregates', async () => {
    const svc = makeService([], [], 'sampled_numeric_field_stats')
    await expect(sampledNumericFieldStats(svc, 'd1', 'rating', null)).rejects.toThrow('sampled_numeric_field_stats failed')
  })
})
