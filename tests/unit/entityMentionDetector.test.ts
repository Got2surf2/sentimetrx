...
describe('detectEntityMentions', () => {
  it('returns empty for empty / whitespace input', async () => {
    ...
    expect(await detectEntityMentions({} as any, BOT_ID, '')).toEqual([])
    expect(await detectEntityMentions({} as any, BOT_ID, '   ')).toEqual([])
  })

  it('matches a single-word canonical with word boundaries', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('apopka', 'Apopka')])
    expect(await detectEntityMentions({} as any, BOT_ID, 'I live in Apopka.')).toEqual(['apopka'])
  })

  it('is case-insensitive', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('apopka', 'Apopka')])
    expect(await detectEntityMentions({} as any, BOT_ID, 'apopka resident here')).toEqual(['apopka'])
    expect(await detectEntityMentions({} as any, BOT_ID, 'APOPKA gets ignored too?')).toEqual(['apopka'])
  })

  it('does not match across word boundaries', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('apopka', 'Apopka')])
    // "Apopkans" should NOT match the canonical "Apopka" — word boundary.
    expect(await detectEntityMentions({} as any, BOT_ID, 'the Apopkans are organizing')).toEqual([])
  })

  it('matches multi-word entities', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('plymouth_sorrento', 'Plymouth Sorrento')])
    expect(await detectEntityMentions({} as any, BOT_ID, 'when does Plymouth Sorrento get widened?')).toEqual(['plymouth_sorrento'])
  })
  ...
  it('matches plural variants via entityVariants expansion', async () => {
    ...
    expect(await detectEntityMentions({} as any, BOT_ID, 'the new Ordinances passed last week')).toEqual(['ordinance'])
  })

  it('matches singular for catalog plurals', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('brussels_sprouts', 'Brussels Sprouts')])
    expect(await detectEntityMentions({} as any, BOT_ID, 'I had one Brussels Sprout')).toEqual(['brussels_sprouts'])
    expect(await detectEntityMentions({} as any, BOT_ID, 'I love Brussels Sprouts')).toEqual(['brussels_sprouts'])
  })

  it('dedupes when the same entity appears multiple times in one message', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('apopka', 'Apopka')])
    const slugs = await detectEntityMentions({} as any, BOT_ID, 'Apopka, Apopka, Apopka — three times in Apopka')
  ...
  it('returns multiple distinct entity slugs in first-seen order', async () => {
    ...
    expect(await detectEntityMentions({} as any, BOT_ID, 'I am at Terminal C, need to get to Terminal A')).toEqual(['terminal_c', 'terminal_a'])
  })
  ...
  it('uses longest-first precedence (multi-word wins over substring)', async () => {
    ...
    expect(await detectEntityMentions({} as any, BOT_ID, 'I am at Terminal A waiting')).toEqual(['terminal_a'])
  })
  ...
  it('matches an alias term', async () => {
    ...
    // leading "the".)
    expect(await detectEntityMentions({} as any, BOT_ID, 'tell me about Foundations')).toEqual(['foundations_project'])
  })

  it('skips terms shorter than 2 characters', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('x', 'X')])
    expect(await detectEntityMentions({} as any, BOT_ID, 'X is a thing')).toEqual([])
  })

  it('invalidateEntityCache forces a reload', async () => {
    __setEntityCacheForTest(BOT_ID, [entry('apopka', 'Apopka')])
    expect(await detectEntityMentions({} as any, BOT_ID, 'Apopka talk')).toEqual(['apopka'])
    ...
    expect(await detectEntityMentions({} as any, BOT_ID, 'Apopka talk')).toEqual([])
    expect(await detectEntityMentions({} as any, BOT_ID, 'Orlando talk')).toEqual(['orlando'])
  })
})
