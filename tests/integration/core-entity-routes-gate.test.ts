...
describe('townhall/themes/custom — POST (cross-org write fix)', () => {
  ...
  it('404 when the pulseiq_sessions hall belongs to another org (non-admin)', async () => {
    ctx.authUser = { id: 'u1' }
    ctx.results['users'] = withAdmin('orgA', false)
    ctx.results['pulseiq_sessions'] = { data: { id: 'h1', org_id: 'orgB' }, error: null }
  ...
  it('404 when the legacy session belongs to another org (non-admin)', async () => {
    ctx.authUser = { id: 'u1' }
    ctx.results['users'] = withAdmin('orgA', false)
    ctx.results['pulseiq_sessions'] = { data: null, error: null }
  ...
  it('owning-org caller may push a custom topic (new substrate)', async () => {
    ctx.authUser = { id: 'u1' }
    ctx.results['users'] = withAdmin('orgA', false)
    ctx.results['pulseiq_sessions'] = { data: { id: 'h1', org_id: 'orgA' }, error: null }
