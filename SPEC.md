...
| *(taxonomy embed)* | Per-row taxonomy verdicts live **inside `dataset_rows_flat.data` under the reserved `_tx` key** (sql/151, 2026-07-04): `_tx.f[field-key]` = axes/alerts/assertions block per analyzed open-end combination. The former sidecar tables `dataset_row_taxonomy` + `dataset_row_field_taxonomy` were **dropped** (sql/152) after a 155K-verdict backfill. Read via the `taxonomy_*` RPCs; the tsv trigger + schema detection + exports all skip `_tx`. Since sql/158 the embed also carries an 8th **`emotion` axis** (disappointment / blame / churn-intent language flags, `lib/emotionFlags.ts`). Closed vocab in `lib/taxonomyVocabulary.ts`. |
...
- Storage: the **`data._tx` embed** in `dataset_rows_flat` (sql/151, keyed per analyzed field-combination via `taxonomyFieldKey`); the former sidecar tables were dropped in sql/152. All readers — Dimensions tab, theme-card chips, Comments dimension filter, Charts/Stats, decks, admin viewer — go through the embed / `taxonomy_*` RPCs. See § Database Tables.
...
- **Emotion-language axis** (2026-07-06, `TAXONOMY.md §2a`): the keyword pass also emits an 8th `emotion` axis — `disappointment` / `blame` / `churn intent` expressed-language flags with verbatim evidence spans (`lib/emotionFlags.ts`; regret dark until the LLM tier — prototype results §2a.1). Rides every Dimensions surface incl. the Comments facet and an Operational Review deck tile; zero-suppressed on datasets where it never fires.
...
- **Public display config**: `GET /api/bots/[id]/public` (unauthenticated, CORS-open — same posture as `/chat`) returns ONLY safe display fields (`name`, `avatarLetter`, `subtitle`, `greeting`, `suggestions`, `placeholder`) so external embeds (e.g. the datanautix.com chat widget) render the agent's identity at runtime instead of hardcoding it. Requires `status='active'`; never exposes the system prompt, KB, or guardrails. See `docs/BOTS.md`.
...
- Admin-only deck routes (gated by `requireAdmin`): pitch, rollup, architecture, engineering-reality, agent-capabilities, entity-analysis, pulseiq, signal-tiers, restaurant-expansion, nowocats-approach, mco-listening, community-feedback, review-intelligence. The decks index (`/admin/decks`) groups by audience and sorts most-recently-updated first. Downloads logged to `deck_download_log`.
...
- **Manifest** (`app/manifest.ts`): Next.js App Router file-convention manifest, served as `/manifest.webmanifest`. `start_url=/m`, `display=standalone`, theme color `#e8622a`. Three icon sizes (180/192/512 PNG, generated from `public/favicon.svg` via `sips`) plus a maskable 512 for Android adaptive icons.
...
| Agents (bots) | `/api/bots`, `/api/bots/[id]/{chat,public,analyze,conversations/*,intents-stats,knowledge,knowledge/[chunkId],export,history,questions,questions/[questionId],questions/export.csv,ui-hints}`, `/api/bots/{deep-crawl,fetch-url,research,import}`, `/api/bot-chat`, `/api/clara-chat`, `/api/nora-chat` |
