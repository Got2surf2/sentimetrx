...
function footer(slide: any, pptx: any, datasetName: string) {
  ...
  slide.addText('Generated by Datanautix', {
  ...
  if (spec.secondStat || spec.decisionsMade !== undefined) {
    ...
    if (spec.secondStat) {
      drawStat(PAD + half + 0.4, half, spec.secondStat.value, spec.secondStat.label,
               spec.secondStat.color || DN.gold, spec.secondStat.sub)
    } else {
      drawStat(PAD + half + 0.4, half, (spec.decisionsMade as number).toLocaleString(), 'decisions made', DN.gold,
