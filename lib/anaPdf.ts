...
// each answer's "Provenance — how this analysis and report was derived" trail so every figure in the
...
export function composeAnaFindingsHtml(opts: AnaPdfOpts): string {
  ...
  const sections = opts.exchanges.map(function(ex, i) {
    const logic = (ex.logic || []).length > 0
      ? '<div class="logic avoid-break"><div class="logic-title">Provenance &mdash; how this analysis and report was derived</div><ol>' +
