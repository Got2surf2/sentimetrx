...
export function mapLegacyLabel(raw: string): MappingResult {
  ...
  if (serviceMatch) {
    ...
    // Touchpoint role embedded in label (e.g. 'service - manager')
    if (['server', 'manager', 'host', 'bartender', 'busser', 'chef', 'sommelier', 'cashier'].includes(sub)) {
      result.assertions.push({ axis: 'touchpoint', sub, severity: 'normal' })
      return result
    }
    result.assertions.push({ axis: 'touchpoint', sub: 'server', severity: 'normal' })
    return result
  ...
  // ── Beverage ─────────────────────────────────────────────────────────────
  const bevMatch = canon.match(/^beverage - (.+)$/)
  if (bevMatch) {
  ...
  if (['server', 'manager', 'host', 'bartender', 'busser', 'chef', 'sommelier', 'cashier'].includes(canon)) {
    result.assertions.push({ axis: 'touchpoint', sub: canon, severity: 'normal' })
    return result
  }
