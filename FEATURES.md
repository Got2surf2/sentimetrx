...
- **Collections + PDF everywhere (2026-09-04)**: the Data Story appears in every Reports menu (datasets and collections) with HTML + PDF formats; a collection's story fans out to the members and uses them as the story's segments (per-member profiles and cross-member comparisons).
...
- **Per-field & multi-field, reactive**: classifies the selected open-end(s) (combined, like Themes), stored per `(row, field-key)` in the row blob's **`data._tx` embed** (sql/151; the former sidecar tables were dropped in sql/152); selecting fields **auto-classifies** and the whole view updates (no button)
...
- **Emotion-language flags** (shipped 2026-07-06): an 8th `emotion` axis — *disappointment / blame / churn-intent* language detected by a validated keyword pass (`lib/emotionFlags.ts`), every hit carrying a verbatim evidence span. Expressed-language framing only ("contains disappointment language", never "felt"); client-facing rates denominated on 1–3★ comments; suppressed entirely when nothing fires (ideation-genre datasets never show "0% emotion"). Surfaces: Dimensions pill/cards, TextMine clouds + Comments facet, Charts/Stats `__dim_emotion__`, Operational Review deck tile with the disappointment×churn co-occurrence KPI. Regret stays dark until the LLM tier (prototype 2026-07-06: ~75–80% precision, `docs/TAXONOMY.md §2a.1`)
...
- **Backup outcome ledger (`org_snapshot_runs`, sql/192, 2026-08-16)** — one durable row per (org, snapshot day), upserted as the resumable cron hops. Four statuses: `ok` · **`incomplete`** (a manifest was written but a table failed to read — **NOT a usable backup**) · `partial` (out of time mid-org, manifest pending) · `failed`. **`/admin/health` shows a Per-Tenant Backups card** for the latest run: N/M orgs clean, plus every org that failed *and* every org with **no run recorded at all**. Before this the outcome lived only in the cron's HTTP response, `console.error` and an aggregate Sentry count, so once Vercel's logs rotated there was no way to say which tenant had no backup.
...
- **Advanced-research capability deck** (`lib/pptx/advancedResearchDeck.ts`, admin `/api/advanced-research-deck?client=<name>`): fixed-content, client-agnostic first-meeting overview of the four advanced-research capabilities. 10 slides — cover → four-capability map (Conversational Surveys · Town Hall · PulseIQ as collection modes, all feeding Analytics as the shared insight engine) → a drill-down per product (why conversational beats traditional; live-conversation theme extraction; virtual focus-group scale; text analytics · charting · statistics · predictive modeling) → one-platform through-line → close. No fabricated data (only the house's established/attributed 40–50% and 3–5× figures, the latter labeled illustrative). Optional `?client=<name>` injects a prospect name on the cover + footer only. Surfaced on `/admin/decks` (Client & Prospect).
- **NEPA / Forest Service demo suite** (GOV·NEPA, since 2026-07-10): two admin-gated deck routes on `/admin/decks` — `/api/nepa-comment-deck` (Blue Mountains Forest Plan Revision pitch, `lib/pptx/blueMountainsNepaDeck.ts`, real-corpus funnel/issues/PII slides) and `/api/nepa-cara-deck` ("If we built CARA today", `lib/pptx/nepaCaraReimaginedDeck.ts`). Companion pieces: the public `blue-mountains` info+concern-capture agent (curated KB + tagged DEIS chunks; seeder `scripts/oneoff/_blue_mountains_create_agent.ts`, DEIS ingest `_nepa_deis_ingest.ts`) and a KB-grounded generate-response demo reusing the public `/review/[token]` loop (`lib/agentDraft.ts` `draftAnswerFromKB`). The comment corpus is pulled from the Forest Service CARA public reading room and analyzed offline by the committed `scripts/oneoff/_nepa_*` pipeline (pull → dedup/template-delta → issue/stance coding → question extraction/answerability → PII scan → shared-link dashboard). No in-app NEPA product yet — the greenfield comment-analytics dashboard is scoped, owner sign-off pending.
...
- **Question Log** (since 2026-05-21/22) — durable record of user turns the bot couldn't answer (`deflect`, `kb_miss`, `ai_uncertain`); admin UI at `/bots/[id]/questions` with All / Unanswered tabs + status mutation + notes; CSV export with PII redaction by default (superadmin `?reveal=1` unmasks). Driver: NOWOCATS PM-2 legal-defensibility; generic across all agents.
  - **Answer → knowledge base (closed loop, since 2026-06-26)** — each question has an **Answer** box; saving feeds the answer into the agent's knowledge base (insert + embed one chunk) and marks the question answered, so the agent retrieves it on the next matching question — no rebuild. **Idempotent per question = a correction loop**: re-saving updates + re-embeds the same chunk, so fixing a wrong/weak answer re-trains the agent in place. Forced-neutral sentiment so RAG never suppresses a human-authored answer; **✓ In knowledge base** badge once promoted. Each question also shows **what the agent actually replied** (context for what to correct) with a one-click **"Accept the agent's reply as the answer"** button. `POST /api/bots/[id]/questions/[questionId]/answer`. A **guided Review mode** (one question at a time, conversation on screen, AI-drafted answer prefilled), a **near-duplicate guard** (warns before adding a ~identical chunk), and a **KB Health view** (`/bots/[id]/knowledge/health` — inventory + prune by source/age) keep the base tight as answers feed in.
...
- **Live-data context injection — `config.liveContext`** (since 2026-07-11) — per-turn deterministic injection of live external-API data into the system prompt, gated by a config flag (generalizes the bot-id-gated MCO pattern; see BOTS.md §6). First value: `'wildfire'` → `lib/wildfireLiveContext.ts` pulls, keyless + fail-soft: NIFC/WFIGS active incidents (distance/acres/containment), fire perimeters (IRWIN-joined distance-to-fire-EDGE + inside-perimeter urgent banner), NWS point alerts, AirNow AQI, and U.S. Drought Monitor class (context-never-verdict). Regex intent + ZIP/city location detection with cross-turn carry-forward; safety rails baked into the injected block (never declare safe, no spread prediction, 911-first, no constructed local-agency URLs). First consumer: the **Ember** wildfire information agent (slug `wildfire`, seeded via `scripts/oneoff/_wildfire_create_agent.ts`).
...
*Gated by `organizations.features.townhall`. UI label: **PulseIQ**. Internal name: `townhall` (legacy substrate) + `pulseiq_sessions` (phase-3 substrate, post-convergence). See `docs/TOWNHALL.md` and `docs/CONVERGENCE.md`.*

> **Convergence Phase 5 complete (2026-05-22)**. PulseIQ runs on a unified conversational substrate shared with Agents (`conversations` + `conversation_turns`) plus PulseIQ-specific cohort tables (`pulseiq_sessions`, `pulseiq_topics`, `pulseiq_session_conversations`). The chat handler is `lib/chatCore.handleChatTurn` — same code path as `/api/bots/[id]/chat`, with a town-hall context branch that pulls topics from `pulseiq_topics`, picks the next via `lib/pickNextTopic`, and tags both turns of the pair with the chosen `topic_id`. The facilitator dashboard (`/api/townhall/sessions` list + `/api/townhall/sessions/[id]` detail) accepts both substrates via `lib/townHallAdapter.ts`. Participant + facilitator-mutating routes (`join`, `live`, `themes/[id]`, `themes/custom`, `sessions/[id]` PATCH/DELETE, `sessions/[id]/analyze`) are all substrate-aware. **NOWOCATS** is the first town hall on the new substrate (Sarina bot, early June launch); **Vindman** follows post-launch using the Sir O'Gate bot. Each event = one `pulseiq_sessions` row; multi-event rollup combines per-event datasets in Analytics. Legacy `townhall_sessions/_themes/_turns` tables remain in service for pre-convergence sessions and will be dropped in Tier 5 cleanup. Two known follow-ups not yet wired: `/api/townhall/responses` POST (post-session psycho/demo upsert) and bot-level analyze `town_hall_id` attribution — neither blocks NOWOCATS launch. Prod env flags required to activate: `TOWNHALL_VIA_AGENT_HANDLER=true` + `DUAL_WRITE_PHASE3=true`.
...
- **Agent Study** (`/bots/[id]/report`) — comprehensive health + study report (volume, depth, focuses, entities, open questions, narrative insights) with HTML / PDF / PPTX export. See `docs/BOTS.md § Agent Study`.
- **What We Heard** (`/bots/[id]/readout`, "What We Heard" link on each agent card; since 2026-06-26. UI label "What We Heard"; internal slug/route/table stay `readout`) — content-first companion to the Agent Study (the card's "Report") answering "what did people **ask**" (questions by theme) and "what did they **raise** beyond the questions" (themed volunteered observations/concerns/suggestions). Hybrid theming (declared-focus scaffold + emergent themes mined from the data), grounded executive summary, with interactive HTML / PDF / Datanautix-branded PPTX export. Generic per-agent template (optional `bot.config.readoutTitle`); NOWOCATS/Sarina is the first instance. See `docs/BOTS.md § Agent Conversation Readout`.
